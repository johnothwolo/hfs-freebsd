...
//		gBlkListEntries += BLOCK_LIST_INCREMENT;
		tmp = realloc(gBlockList, (gBlkListEntries + BLOCK_LIST_INCREMENT) * sizeof(uint64_t));
		if (tmp == NULL) {
			pfatal("Can't allocate memory for block list (%llu entries).\n", gBlkListEntries);
		}
		gBlockList = (uint64_t*)tmp;
	}
...
#define PRSTAT \
	...
	if (ioctl(fd, DIOCGSECTORSIZE, &devBlockSize) == -1) {
		devBlockSize = 512;
	}

	if (ioctl(fd, DIOCGMEDIASIZE, &diskSize) == -1) {
		diskSize = 0;
	} else
		devBlockTotal = diskSize / devBlockSize;
...
getblocklist(const char *filepath)
{
	...
	gBlockList = (uint64_t *) malloc(blockListCount * sizeof(uint64_t));
	if (gBlockList == NULL)
