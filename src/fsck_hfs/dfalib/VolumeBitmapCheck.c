...
struct dk_extent {
    uint64_t               offset;
    uint64_t               length;
} gTrimExtents[kMaxTrimExtents];
struct {
    struct dk_extent *     extents;
...
static void TrimFlush(void)
{
	...
	if (gTrimData.extentsCount == 0)
	{
        ...
        for (int i = 0; i < gTrimData.extentsCount; i++) {
            // dk_unmap->options is currently unused in darwin
            arg[0] = gTrimData.extents[i].offset;
            arg[1] = gTrimData.extents[i].length;
            err = ioctl(fsreadfd, DIOCGDELETE, arg);
            if (err < 0) {
                if (errno == EOPNOTSUPP && !IsTrimSupported()) {
                    perror("TRIM/UNMAP not supported by driver");
                } else
                    perror("ioctl(DIOCGDELETE) failed");
                break;
            }
        }
    }
...
int IsTrimSupported(void)
{
    ...
    if (ioctl(fsreadfd, DIOCGATTR, &arg) == 0)
