...
static swapper_t swappedEndian = {
	"swapped endian",
	^(uint16_t x) { return (uint16_t)bswap16(x); },
	^(uint32_t x) { return (uint32_t)bswap32(x); },
	^(uint64_t x) { return (uint64_t)bswap64(x); }
};
...
//	fprintf(stderr, "%s(%p, %p, %zu)\n", __FUNCTION__, info, buffer, length);
	...
	while (nread < length) {
		...
		if (n == -1) {
			warn("pread(%d, %p, %zu, %lu)", info->jfd, ptr, amt, info->current);
			goto done;
		}
		if (n != amt) {
			if (debug) {
	     ...
	     int (^do_write_b)(off_t, void*, size_t))
{
	...
	if (ioctl(jfd, DIOCGSECTORSIZE, &jBlkSize) == -1) {
	...
	} else if (bswap32(jhdr.endian) == ENDIAN_MAGIC) {
