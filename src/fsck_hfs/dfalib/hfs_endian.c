...
)
{
    ...
    if (fileID == kHFSExtentsFileID) {
    	...
    	for (i = 0; i < srcDesc->numRecords; i++) {
    		...
    		if ((char *)srcRec + sizeof(uint32_t) > nextRecord) {
    		...
    		if (srcKey->attrNameLen > kHFSMaxAttrNameLen || keyLength < (kHFSPlusAttrKeyMinimumLength + sizeof(uint16_t)*srcKey->attrNameLen)) {
