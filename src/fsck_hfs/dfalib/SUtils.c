...
UpdateVolumeEncodings(SVCB *volume, TextEncoding encoding)
{
	...
	volume->vcbEncodingsBitmap |= (uint64_t)(1ULL << index);
...
OSErr	FlushVolumeControlBlock( SVCB *vcb )
{
	...
	{		
		...
		volumeHeader->extentsFile.totalBlocks = (uint32_t)(fcb->fcbPhysicalSize / vcb->vcbBlockSize);
		...
		volumeHeader->catalogFile.totalBlocks = (uint32_t)(fcb->fcbPhysicalSize / vcb->vcbBlockSize);
		...
		volumeHeader->allocationFile.totalBlocks = (uint32_t)(fcb->fcbPhysicalSize / vcb->vcbBlockSize);
		...
		if (vcb->vcbAttributesFile != NULL)	//	Only update fields if an attributes file existed and was open
		{
			...
			volumeHeader->attributesFile.totalBlocks = (uint32_t)(fcb->fcbPhysicalSize / vcb->vcbBlockSize);
		}
	}
	else
	{
