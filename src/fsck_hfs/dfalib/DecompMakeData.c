...
struct UniCharClassAndRepl {
	...
	uint16_t	combClass;
	uint16_t	action;
	uint16_t	matchAndReplacement[3];
};
...
static uint8_t	classRanges[kMaxRangeCount][kLoFieldEntryCount];
static uint8_t	replRanges[kMaxRangeCount][kLoFieldEntryCount];
static uint16_t rangesKey[kMaxRangeCount];							// remembers starting Unicode for range
static uint16_t replacementData[kMaxReplaceDataCount];

int main(int argc, char *argv[]) {
	uint32_t						entryIndex, rangeIndex;
	const UniCharClassAndRepl *		classAndReplPtr;
	int32_t							rangeCount;
	uint32_t						replDataCount;
	...
	for (classAndReplPtr = uCharClassAndRepl; classAndReplPtr->uChar != 0; classAndReplPtr++) {
		uint32_t	matchAndReplacementCount, matchAndReplacementIndex;
		uint16_t	shiftUChar = classAndReplPtr->uChar + kShiftUniCharOffset;
	...
	for (entryIndex = 0; entryIndex < kHiFieldEntryCount; entryIndex++) {
		...
		if ((entryIndex + 1) % kIndexValuesPerLine == 0)		// end of line, print starting UniChar value
			plog("// uChar 0x%04X-\n", (uint16_t)(((entryIndex + 1 - kIndexValuesPerLine) << kLoFieldBitSize) - kShiftUniCharOffset) );
	}
	plog("};\n\n");
	
	// print filled in class ranges
	plog("static const uint8_t combClassRanges[][kLoFieldEntryCount] = {\n", kLoFieldEntryCount);
	...
	plog("static const uint8_t replaceRanges[][kLoFieldEntryCount] = {\n", kLoFieldEntryCount);
	for (rangeIndex = 0; rangeIndex < rangeCount; rangeIndex++) {
		plog("\t{\t");
		for (entryIndex = 0; entryIndex < kLoFieldEntryCount; entryIndex++) {
			char *	formatPtr = (entryIndex + 1 < kLoFieldEntryCount)? "%3d,": "%3d";
	...
	plog("static const uint16_t replaceData[] = {\n");
