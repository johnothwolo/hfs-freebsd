...
typedef struct attributeInfo {
	...
	int16_t	recordType;
	uint32_t fileID;
	unsigned char attrname[EXTATTR_MAXNAMELEN+1];
	uint32_t totalBlocks;
	uint32_t calculatedTotalBlocks;
...
typedef struct SGlob {
	...
	UInt16		securityAttrName[EXTATTR_MAXNAMELEN];	/* Store security attribute name in UTF16, to avoid frequent conversion */
...
extern 	OSErr 	GetFileNamePathByID(SGlobPtr GPtr, UInt32 fileID, char *fullPath, unsigned int *fullPathLen, char *fileName, unsigned int *fileNameLen, uint16_t *status);
 ...
 */
extern int utf_decodestr(const unsigned char *, size_t, uint16_t *, size_t *, size_t);
extern int utf_encodestr(const uint16_t *, size_t, unsigned char *, size_t *, size_t);
