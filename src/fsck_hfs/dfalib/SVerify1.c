...
#define SW16(x)	be16toh(x)
#define	SW32(x)	be32toh(x)
#define	SW64(x)	be64toh(x)
 ...
 * Output: Nothing
 */
static void setLastAttrAllocInfo(SGlobPtr GPtr, uint32_t totalBlocks, 
				uint64_t logicalSize, uint32_t calculatedTotalBlocks)
{
...
static int CheckLastAttrAllocation(SGlobPtr GPtr) 
{
	...
	if (GPtr->lastAttrInfo.isValid == true) {
		if (GPtr->lastAttrInfo.totalBlocks != 
			...
			bytes = (uint64_t)GPtr->lastAttrInfo.calculatedTotalBlocks * 
					(uint64_t)GPtr->calculatedVCB->vcbBlockSize;
...
{
	int i;
	uint64_t blk, blk1, blk2;
	uint64_t offset;

	offset = (uint64_t) startblk * (uint64_t) vcb->vcbBlockSize;
