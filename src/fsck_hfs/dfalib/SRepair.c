...
static 	int		BuildFolderRec( SGlob*, uint16_t theMode, UInt32 theObjID, Boolean isHFSPlus, CatalogRecord * theRecPtr );
...
static 	void 	BuildAttributeKey(uint32_t fileID, uint32_t startBlock, unsigned char *attrName, uint16_t attrNameLen, HFSPlusAttrKey *key);
...
OSErr FixBadLinkChainFirst(SGlobPtr GPtr, RepairOrderPtr p)
{
	...
	uint8_t	attrdata[FIRST_LINK_XATTR_REC_SIZE];
	...
	case kHFSPlusFolderRecord:	// directory hard link
		...
		attrRec->attrSize = (uint32_t)(1 + strlen((char*)&attrRec->attrData[0]));
...
OSErr FixHardLinkBadDate(SGlobPtr GPtr, RepairOrderPtr p)
{
	...
	if (retval == 0) {
		if (rec.recordType != kHFSPlusFileRecord) {
			...
			rec.hfsPlusFile.createDate = (uint32_t)p->correct;
...
#define DPRINT(where, fmt, ...) \
	...
	record.hfsPlusFolder.folderCount = (uint32_t)p->correct;
...
FixIllegalNames( SGlobPtr GPtr, RepairOrderPtr roPtr )
{
    ...
    {
        int					myLength;
        uint16_t *			myPtr = (uint16_t *) oldNamePtr;
...
static OSErr FixAttrSize(SGlobPtr GPtr, RepairOrderPtr p)
{
	...
	if (p->type == E_PEOAttr) {
		if ((uint32_t)p->incorrect == record.forkData.theFork.totalBlocks) {
			record.forkData.theFork.totalBlocks = (uint32_t)p->correct;	
			doReplace = true;

			/* Make sure that new block count is large enough to 
			 * cover the current LEOAttr.  If not, truncate it. 
			 */
			bytes = p->correct * (uint64_t)GPtr->calculatedVCB->vcbBlockSize;
				...
				UInt16 *recordSize, UInt32 *foundExtentIndex)
{
	...
	attrname = malloc (EXTATTR_MAXNAMELEN + 1);
	if (!attrname) {
		result = memFullErr;
		goto out;
	}
	...
	while (1) {
		...
		(void) utf_encodestr(key->attrName, key->attrNameLen * 2, attrname, &attrnamelen, EXTATTR_MAXNAMELEN + 1);
 ...
 */
OSErr GetFileNamePathByID(SGlobPtr GPtr, UInt32 fileID, char *fullPath, unsigned int *fullPathLen, char *fileName, unsigned int *fileNameLen, uint16_t *status)
{
	OSErr err = noErr;
	Boolean isHFSPlus;
	UInt16 recordSize;
...
FixMissingDirectory( SGlob *GPtr, UInt32 theObjID, UInt32 theParID )
{
    ...
    if ( isHFSPlus )
    {
        ...
        for ( i = 0; i < myName.ustr.length; i++ )
            myName.ustr.unicode[ i ] = (uint16_t) myString[ i ];
    }
    else
    {
...
UInt32 CreateDirByName(SGlob *GPtr , const u_char *dirName, const UInt32 parentID)
{
    ...
    if ( isHFSPlus )
    {
        ...
        myName.ustr.length = nameLen;
        for ( i = 0; i < myName.ustr.length; i++ )
            myName.ustr.unicode[ i ] = (uint16_t) dirName[ i ];
    }
    else
    {
        myName.pstr[0] = nameLen;
 ...
 */
static int
BuildFolderRec( SGlob *GPtr, uint16_t theMode, UInt32 theObjID, Boolean isHFSPlus, CatalogRecord * theRecPtr )
{
	UInt16				recSize;
	UInt32 				createTime;
 ...
 */
static void BuildAttributeKey(uint32_t fileID, uint32_t startBlock, 
			unsigned char *attrName, uint16_t attrNameLen, HFSPlusAttrKey *key)
{
