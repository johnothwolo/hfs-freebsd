...
static int  CheckCatalogName(uint16_t charCount, const uint16_t *uniChars,
                             uint32_t parentID, Boolean thread);
static int  CheckCatalogName_HFS(uint16_t charCount, const u_char *filename,
                                 uint32_t parentID, Boolean thread);
...
static 	OSErr	UniqueDotName( 	SGlobPtr GPtr, 
                                ...
                                Boolean isHFSPlus );
static Boolean 	FixDecomps(	uint16_t charCount, const uint16_t *inFilename, HFSUniStr255 *outFilename );
...
CheckDirectory(const HFSPlusCatalogKey * key, const HFSPlusCatalogFolder * dir)
{
	...
	gCIS.encodings |= (uint64_t)(1ULL << MapEncodingToIndex(dir->textEncoding & 0x7F));
...
CheckFile(const HFSPlusCatalogKey * key, const HFSPlusCatalogFile * file)
{
	...
	gCIS.encodings |= (uint64_t)(1ULL << MapEncodingToIndex(file->textEncoding & 0x7F));
 ...
 */
static int
CheckCatalogName(uint16_t charCount, const uint16_t *uniChars, uint32_t parentID, Boolean thread)
{
	OSErr 				result;
	uint16_t *			myPtr;
 ...
 */
static int
CheckCatalogName_HFS(uint16_t charCount, const u_char *filename, uint32_t parentID, Boolean thread)
{
	u_char *			myPtr;
	RepairOrderPtr 		roPtr;
	int					myLength;
             	...
             	Boolean isHFSPlus )
{
    ...
    if ( isHFSPlus )
    {
        ...
        for ( i = 0; i < theNewNamePtr->ustr.length; i++ )
            theNewNamePtr->ustr.unicode[ i ] = (uint16_t) *(myPtr + i);
    }
    else
    {
    ...
    for ( newChar = 0x30; newChar < 0x7F; newChar++ )
    {
        ...
        if ( isHFSPlus )
        {
            theNewNamePtr->ustr.unicode[ nameLen ] = (uint16_t) newChar;
 ...
 */
static Boolean 
FixDecomps(	uint16_t charCount, const uint16_t *inFilename, HFSUniStr255 *outFilename ) 
{
    // input filename: address of curr input char,
	const uint16_t *	inNamePtr		= inFilename;
    // and of last input char.
	const uint16_t *	inNameLastPtr	= &inFilename[charCount - 1];	
    // output filename buffer: addr of next output char,
	uint16_t *	outNamePtr			= outFilename->unicode;	
    // and of last possible output char.
	uint16_t *	outNameLastPtr		= &outFilename->unicode[kHFSPlusMaxFileNameChars - 1];	
	uint16_t *	outNameCombSeqPtr	= NULL;	// if non-NULL, start of output combining seq we are processing.
	uint32_t	maxClassValueInSeq	= 0;
	Boolean		didModifyName		= 0;

	while (inNamePtr <= inNameLastPtr) {
		uint16_t	shiftUniChar;	// this must be 16 bits for the kShiftUniCharOffset wraparound to work
		int32_t		rangeIndex;
		uint32_t	shiftUniCharLo;
		uint32_t	replDataIndex;
		uint32_t	currCharClass;
			...
			while (copyCount-- > 0) {
				...
				if ( currCharClass == 0 ) {
					...
					*outNameCombSeqPtr = *replDataPtr++;
					...
					uint16_t *	outNameCombCharPtr;
					uint32_t	combCharClass;
