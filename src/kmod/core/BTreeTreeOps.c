...
static int   AddNewRootNode	(BTreeControlBlockPtr		 btreePtr,
									 NodeDescPtr				 leftNode,
									 NodeDescPtr				 rightNode );

static int   CollapseTree		(BTreeControlBlockPtr		 btreePtr,
									 BlockDescriptor			*blockPtr );

static int   RotateLeft		(BTreeControlBlockPtr		 btreePtr,
									 NodeDescPtr				 leftNode,
									 NodeDescPtr				 rightNode,
...
static boolean_t	   RotateRecordLeft	(BTreeControlBlockPtr		 btreePtr,
									 NodeDescPtr				 leftNode,
									 NodeDescPtr				 rightNode );

static int	   SplitLeft		(BTreeControlBlockPtr		 btreePtr,
...
static	int	InsertLevel		(BTreeControlBlockPtr		 btreePtr,
...
static OSErr		InsertNode 		(BTreeControlBlockPtr		 btreePtr,
									 ...
									 u_int16_t					*newIndex,
									 BlockDescriptor			*leftNode,
									 boolean_t					*updateParent,
									 boolean_t					*insertParent,
									 boolean_t					*rootSplit );
									 
static u_int16_t		GetKeyLength	(const BTreeControlBlock *btreePtr,
									 const BTreeKey *key,
									 boolean_t forLeafNode );
...
int	InsertLevel (BTreeControlBlockPtr		 btreePtr,
						 TreePathTable				 treePathTable,
						 InsertKey					*primaryKey,
						 InsertKey					*secondaryKey,
						 BlockDescriptor			*targetNode,
								 ...
								 u_int16_t					 level )
{
	...
	u_int32_t			targetNodeNum;
	boolean_t				deleteRequired;
	boolean_t				updateRequired;
...
static u_int16_t	GetKeyLength ( const BTreeControlBlock *btreePtr, const BTreeKey *key, boolean_t forLeafNode )
{
	u_int16_t length;
