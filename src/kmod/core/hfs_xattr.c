...
static int  read_attr_data(struct hfsmount *hfsmp, struct uio *uio, size_t datasize, HFSPlusExtentDescriptor *extents);

static int  write_attr_data(struct hfsmount *hfsmp, struct uio *uio, size_t datasize, HFSPlusExtentDescriptor *extents);
 ...
 */
int
hfs_vnop_getnamedstream(struct vop_getnamedstream_args* ap)
{
 ...
 */
int
hfs_vnop_makenamedstream(struct vop_makenamedstream_args* ap)
{
	struct vnode* vp = ap->a_vp;
	struct vnode* *svpp = ap->a_svpp;
	struct cnode *cp;
 ...
 */
int
hfs_vnop_removenamedstream(struct vop_removenamedstream_args* ap)
{
 ...
 */
int
hfs_vnop_getxattr(struct vop_getextattr_args *ap)
/*
...
*/
{
	...
	if (vp == cp->c_vp) {
		...
		if (strcmp(ap->a_name, XATTR_FINDERINFO_NAME) == 0) {
			...
			result = uiomove((caddr_t)&finderinfo , (int)bufsize, uio);
		...
		if (strcmp(ap->a_name, XATTR_RESOURCEFORK_NAME) == 0) {
			...
			if (uio == NULL) {
				*ap->a_size = (size_t)VTOF(rvp)->ff_size;
				...
				if (decmpfs_hide)
					uio_size = uio->uio_resid;
#endif /* HFS_COMPRESSION */
				result = VOP_READ(rvp, uio, 0, ap->a_cred);
#if HFS_COMPRESSION
				if (decmpfs_hide &&
				    (result == 0) &&
				    (uio->uio_resid == uio_size)) {
					/*
					 ...
					 * (hence uio->uio_resid == uio_size, i.e. the uio is untouched)
					 * 
					 * In that case, we try again with the decmpfs_ctx context
					 * to get the actual data
					 */
					result = VOP_READ(rvp, uio, 0, decmpfs_ctx);
				}
#endif /* HFS_COMPRESSION */
			}
...
int hfs_xattr_read(struct vnode* vp, const char *name, void *data, size_t *size)
{
	...
	struct vop_getextattr_args args = {
		.a_uio = &auio,
        .a_attrnamespace = 0,
		.a_name = name,
		.a_size = size,
 ...
 */ 
int hfs_getxattr_internal (struct cnode *cp, struct vop_getextattr_args *ap,
		...
		case kHFSPlusAttrInlineData: {
			...
			*ap->a_size = recp->attrData.attrSize;
			if (uio && recp->attrData.attrSize != 0) {
				if (*ap->a_size > (size_t)uio->uio_resid) {
		...
		case kHFSPlusAttrForkData: {
			...
			*ap->a_size = recp->forkData.theFork.logicalSize;
			...
			if (has_overflow_extents(&recp->forkData.theFork)) {
				...
				attrlen = (int) recp->forkData.theFork.logicalSize;
 ...
 */
int
hfs_vnop_setxattr(struct vop_setextattr_args *ap)
/*
...
//	time_t orig_ctime=VTOC(vp)->c_ctime;
    ...
    result = extattr_check_cred(vp, ap->a_attrnamespace, ap->a_cred, ap->a_td, VWRITE);
    if (result) {
        return result;
    }
...
//	nspace_snapshot_event(vp, orig_ctime, NAMESPACE_HANDLER_METADATA_WRITE_OP, NSPACE_REARM_NO_ARG);
	...
	if (strcmp(ap->a_name, XATTR_FINDERINFO_NAME) == 0) {
		...
		if ((size_t)uio->uio_resid != attrsize) {
			return (ERANGE);
		}
		/* Grab the new Finder Info data. */
		if ((result = (int)uiomove(fi.cdata, (int)attrsize, uio))) {
			return (result);
		}
        ...
        // always replace no matter what.
#if XATTR_CREATE_AND_REPLACE
		...
		if (fdFlags & htobe16(kFinderInvisibleMask)) {
	...
	if (strcmp(ap->a_name, XATTR_RESOURCEFORK_NAME) == 0) {
		...
		result = VOP_WRITE(rvp, uio, 0, ap->a_cred);
		...
		if (openunlinked) {
			...
			result = hfs_fsync (rvp, MNT_NOWAIT, 0, curthread);
			hfs_unlock (cp);
		}
	...
	if (attrsize > 0 &&
		...
		result = uiomove((caddr_t)user_data_ptr, (int)attrsize, uio);
		if (result) {
			goto exit;
		}
	}
...
{
	struct vop_setextattr_args args = {
		.a_vp 	= vp,
		.a_name = name,
	};
...
int hfs_setxattr_internal (struct cnode *cp, const void *data_ptr, size_t attrsize,
						   struct vop_setextattr_args *ap, struct hfsmount *hfsmp,
						   u_int32_t fileid) 
{
	struct uio *uio = ap->a_uio;
	struct vnode *vp = ap->a_vp;
	...
	if (attrsize > hfsmp->hfs_max_inline_attrsize) {
		...
		btdata.itemSize = (int) (sizeof(HFSPlusAttrData) - 2 + attrsize + ((attrsize & 1) ? 1 : 0));
		...
		recp->attrData.attrSize = (int)attrsize;
		...
		if (attrsize > 0) {
			if (data_ptr) {
				...
				result = uiomove((caddr_t)&recp->attrData.attrData, (int)attrsize, uio);
			}
 ...
 */
int
hfs_vnop_removexattr(struct vop_deleteextattr_args *ap)
/*
	struct vop_removeextattr_args {
		struct vnodeop_desc *a_desc;
		struct vnode* a_vp;
		char * a_name;
		int a_options;
...
//	nspace_snapshot_event(vp, orig_ctime, NAMESPACE_HANDLER_METADATA_DELETE_OP, NSPACE_REARM_NO_ARG);
	...
	if (strcmp(ap->a_name, XATTR_RESOURCEFORK_NAME) == 0) {
		...
		result = hfs_truncate(rvp, (off_t)0, IO_NDELAY, 0, ap->a_td);
		if (result == 0) {
			cp->c_touch_chgtime = TRUE;
			cp->c_flag |= C_MODIFIED;
			result = hfs_update(vp, 0);
 ...
 */
int
hfs_vnop_listxattr(struct vop_listextattr_args *ap)
/*
...
*/
{
	...
	*ap->a_size = 0;
	...
	if (bcmp(finderinfo, emptyfinfo, sizeof(emptyfinfo)) != 0) {
		...
		} else if ((size_t)uio->uio_resid < sizeof(XATTR_FINDERINFO_NAME)) {
			result = ERANGE;
			goto exit;
		} else {
	...
	if (S_ISREG(cp->c_mode) && hfs_has_rsrc(cp)) {
...
#endif /* HFS_COMPRESSION */
		{
			...
			} else if ((size_t)uio->uio_resid < sizeof(XATTR_RESOURCEFORK_NAME)) {
				result = ERANGE;
				goto exit;
			} else {
	 ...
	 */
	if (uio && (uio->uio_segflg == UIO_USERSPACE)) {
...
listattr_callback(const HFSPlusAttrKey *key, __unused const HFSPlusAttrData *data, struct listattr_callback_state *state)
{
	...
	if (state->uio == NULL) {
		...
		if (bytecount > state->uio->uio_resid) {
			state->result = ERANGE;
			return (0);	/* stop */
		}
		result = uiomove((caddr_t) attrname,(int)bytecount, state->uio);
		if (result) {
			state->result = result;
			return (0);	/* stop */
		}
 ...
 */
static int
read_attr_data(struct hfsmount *hfsmp, struct uio *uio, size_t datasize, HFSPlusExtentDescriptor *extents)
{
	struct vnode* evp = hfsmp->hfs_attrdata_vp;
	int bufsize;
	int64_t iosize;
	int attrsize;
	int blksize;
	...
	for (i = 0; (attrsize > 0) && (bufsize > 0) && (extents[i].startBlock != 0); ++i) {
        ...
        uio->uio_offset = (u_int64_t)extents[i].startBlock * (u_int64_t)blksize;
 ...
 */
__unused static int
write_attr_data(struct hfsmount *hfsmp, struct uio *uio, size_t datasize, HFSPlusExtentDescriptor *extents)
{
	struct vnode* evp = hfsmp->hfs_attrdata_vp;
	off_t filesize;
	int bufsize;
	...
	for (i = 0; (attrsize > 0) && (bufsize > 0) && (extents[i].startBlock != 0); ++i) {
        ...
        // FIXME: xattr write support.
//        result = cluster_write(<#struct vnode *#>, <#struct buf *#>, <#u_quad_t#>, <#int#>, <#int#>);
...
//			(void) ubc_msync(hfsmp->hfs_attrdata_vp, start, end, &start, UBC_INVALIDATE);
            u_int bcount = (u_int) round_page(end - start);
            struct vnode *vp = hfsmp->hfs_attrdata_vp;
            vm_page_t page = vm_page_lookup(vp->v_object, OFF_TO_IDX(start));
            ret = vnode_pager_generic_putpages(vp, &page, bcount, vnode_pager_putpages_ioflags(IO_SYNC), 0);
		}
	}
