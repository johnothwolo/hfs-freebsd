...
struct BlockDescriptor{
	void		*buffer;
	void		*blockHeader;
	daddr_t	 blockNum;	/* logical block number (used by hfs_swap_BTNode) */
	ByteCount	 blockSize;
	boolean_t		 blockReadFromDisk;
...
enum {
		...
		kForceReadBlock		= 0x00000002,	//ÄÄ how does this relate to Read/Verify? Do we need this?
...
typedef	int	(* GetBlockProcPtr)		(FileReference				 fileRefNum,
											 u_int32_t					 blockNum,
											 GetBlockOptions			 options,
											 BlockDescriptor			*block );
							 

typedef	int	(* ReleaseBlockProcPtr)	(FileReference				 fileRefNum,
											 BlockDescPtr				 blockPtr,
											 ReleaseBlockOptions		 options );

typedef	int	(* SetEndOfForkProcPtr)	(FileReference				 fileRefNum,
											 FSSize						 minEOF,
											 FSSize						 maxEOF );
								 
typedef	int	(* SetBlockSizeProcPtr)	(FileReference				 fileRefNum,
											 ByteCount					 blockSize,
											 ItemCount					 minBlockCount );

int		SetEndOfForkProc ( FileReference fileRefNum, FSSize minEOF, FSSize maxEOF );
...
extern int	BTOpenPath(FCB *filePtr, KeyCompareProcPtr keyCompareProc);

extern int	BTClosePath			(FCB		 				*filePtr );


extern int	BTSearchRecord		(FCB		 				*filePtr,
									 BTreeIterator				*searchIterator,
									 FSBufferDescriptor			*btRecord,
									 u_int16_t					*recordLen,
									 BTreeIterator				*resultIterator );

extern int	BTIterateRecord		(FCB		 				*filePtr,
...
extern int BTIterateRecords(FCB *filePtr, BTreeIterationOperation operation, BTreeIterator *iterator,
		 IterateCallBackProcPtr	 callBackProc, void * callBackState);

extern int	BTInsertRecord		(FCB		 				*filePtr,
									 BTreeIterator				*iterator,
									 FSBufferDescriptor			*btrecord,
									 u_int16_t					 recordLen );

extern int	BTReplaceRecord		(FCB		 				*filePtr,
									 BTreeIterator				*iterator,
									 FSBufferDescriptor			*btRecord,
									 u_int16_t					 recordLen );

extern int	BTUpdateRecord		(FCB		 				*filePtr,
									 BTreeIterator				*iterator,
									 IterateCallBackProcPtr		 callBackProc,
									 void						*callBackState );

extern int	BTDeleteRecord		(FCB		 				*filePtr,
									 BTreeIterator				*iterator );

extern int	BTGetInformation	(FCB		 				*filePtr,
									 u_int16_t					 vers,
									 BTreeInfoRec				*info );

extern int BTIsDirty(FCB *filePtr);

extern int	BTFlushPath			(FCB		 				*filePtr );

extern int BTReloadData		(FCB *filePtr);

extern int	BTInvalidateHint	(BTreeIterator				*iterator );

extern int	BTGetLastSync		(FCB		 				*filePtr,
									 u_int32_t					*lastfsync );

extern int	BTSetLastSync		(FCB		 				*filePtr,
									 u_int32_t					lastfsync );

extern int	BTHasContiguousNodes(FCB		 				*filePtr);

extern int BTGetUserData(FCB *filePtr, void * dataPtr, int dataSize);

extern int BTSetUserData(FCB *filePtr, void * dataPtr, int dataSize);
