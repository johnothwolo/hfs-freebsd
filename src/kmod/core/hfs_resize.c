...
static int hfs_file_extent_overlaps(struct hfsmount *hfsmp, u_int32_t allocLimit,
										struct HFSPlusCatalogFile *filerec, bool *overlaps);
static int hfs_reclaimspace(struct hfsmount *hfsmp, u_int32_t allocLimit, u_int32_t reclaimblks, struct thread *td);
static int hfs_extend_journal(struct hfsmount *hfsmp, u_int32_t sector_size, u_int64_t sector_count, struct thread *td);

/*
 * Extend a file system.
 */
int
hfs_extendfs(struct hfsmount *hfsmp, u_int64_t newsize, struct thread *td)
{
	...
	daddr_t  prev_fs_alt_sector;
	daddr_t	   bitmapblks;
	int  lockflags = 0;
	int  error;
	int64_t oldBitmapSize;
	
	boolean_t  usedExtendFileC = false;
...
//		error = vnode_authorize(devvp, NULL, KAUTH_VNODE_READ_DATA | KAUTH_VNODE_WRITE_DATA, td);
//		if (error)
//			return (error);
	}
	if (VNOP_IOCTL(devvp, HFSTOCP(hfsmp), DKIOCGETBLOCKSIZE, (caddr_t)&sector_size, 0)) {
		return (ENXIO);
	}
	if (sector_size != hfsmp->hfs_logical_block_size) {
		return (ENXIO);
	}
	if (VNOP_IOCTL(devvp, HFSTOCP(hfsmp), DKIOCGETBLOCKCOUNT, (caddr_t)&sector_count, 0)) {
		return (ENXIO);
	}
	...
	error = VNOP_IOCTL(devvp, HFSTOCP(hfsmp), DKIOCGETPHYSICALBLOCKSIZE, (caddr_t)&phys_sector_size, 0);
	if (error) {
	...
	if ((newsize <= oldsize) || (newsize % sector_size) || (newsize % phys_sector_size)) {
        printf("hfs_extendfs: invalid size (newsize=%lu, oldsize=%lu)\n", newsize, oldsize);
		return (EINVAL);
	}
	uint64_t cnt = newsize / vcb->blockSize;
	if (cnt > 0xFFFFFFFF) {
		printf ("hfs_extendfs: current blockSize=%u too small for newsize=%lu\n", hfsmp->blockSize, newsize);
		return (EOVERFLOW);
	}
	...
	if (hfs_resize_debug) {
		printf ("hfs_extendfs: old: size=%lu, blkcnt=%u\n", oldsize, hfsmp->totalBlocks);
		printf ("hfs_extendfs: new: size=%lu, blkcnt=%u, addblks=%u\n", newsize, newblkcnt, addblks);
	}
	 ...
	 */
	if (hfs_resize_debug) {
		printf ("hfs_extendfs: old: partition_avh_sector=%lu, fs_avh_sector=%lu\n",
	...
	if (hfs_resize_debug) {
		printf ("hfs_extendfs: new: partition_avh_sector=%lu, fs_avh_sector=%lu\n",
				hfsmp->hfs_partition_avh_sector, hfsmp->hfs_fs_avh_sector);
	}
	...
	if (bitmapblks > 0) {
		...
		if (error == 0) {
			 ...
			 */
			bytesAdded = 0;
			error = AddFileExtent(vcb, fp, vcb->totalBlocks, (u_int) bitmapblks);
			if (error) {
		 ...
		 */
		{
			...
			blkcnt = bitmapblks;
			while (blkcnt > 0) {
				error = (int)bread(vp, blkno, vcb->blockSize, NOCRED, &bp);
				if (error) {
					if (bp) {
						brelse(bp);
					}
		 ...
		 */
		if (!usedExtendFileC) {
			error = BlockMarkAllocated(vcb, vcb->totalBlocks, (u_int) bitmapblks);
			if (error) {
	...
	if (prev_fs_alt_sector) {
		if (bread(hfsmp->hfs_devvp,
			...
			bzero((char *)bp->b_data + HFS_ALT_OFFSET(hfsmp->hfs_physical_block_size), kMDBSize);
	...
	if (hfsmp->hfs_attrdata_vp) {
		...
		if (vget(hfsmp->hfs_attrdata_vp, LK_EXCLUSIVE) == 0) {
			...
			vnode_pager_setsize(hfsmp->hfs_attrdata_vp, attr_fp->ff_size);
			vput(hfsmp->hfs_attrdata_vp);
		}
	}
	...
	error = hfs_extend_journal(hfsmp, sector_size, sector_count, td);
	if (error) {
 ...
 */
int
hfs_truncatefs(struct hfsmount *hfsmp, u_int64_t newsize, struct thread *td)
{
	u_int64_t oldsize;
	u_int32_t newblkcnt;
	...
	int lockflags = 0;
	int transaction_begun = 0;
	boolean_t updateFreeBlocks = false;
	boolean_t disable_sparse = false;
	int error = 0;
	...
	oldsize = (u_int64_t)hfsmp->totalBlocks * (u_int64_t)hfsmp->blockSize;
	newblkcnt = (int) newsize / hfsmp->blockSize;
	reclaimblks = hfsmp->totalBlocks - newblkcnt;
    
	if (hfs_resize_debug) {
		printf ("hfs_truncatefs: old: size=%lu, blkcnt=%u, freeblks=%u\n", oldsize, hfsmp->totalBlocks, hfs_freeblks(hfsmp, 1));
		printf ("hfs_truncatefs: new: size=%lu, blkcnt=%u, reclaimblks=%u\n", newsize, newblkcnt, reclaimblks);
	}
	...
	if ((newsize < HFS_MIN_SIZE) ||
		...
		printf ("hfs_truncatefs: invalid size (newsize=%lu, oldsize=%lu)\n", newsize, oldsize);
		error = EINVAL;
		goto out;
	}
	...
	if (hfs_isallocated(hfsmp, hfsmp->allocLimit, reclaimblks)) {
		...
		error = hfs_reclaimspace(hfsmp, hfsmp->allocLimit, reclaimblks, td);
		if (error != 0) {
	 ...
	 */
	if (hfs_resize_debug) {
		printf ("hfs_truncatefs: old: partition_avh_sector=%lu, fs_avh_sector=%lu\n",
				hfsmp->hfs_partition_avh_sector, hfsmp->hfs_fs_avh_sector);
	}
	...
	if (hfs_resize_debug) {
		printf ("hfs_truncatefs: new: partition_avh_sector=%lu, fs_avh_sector=%lu\n",
				hfsmp->hfs_partition_avh_sector, hfsmp->hfs_fs_avh_sector);
	}
	...
	if (hfsmp->hfs_attrdata_vp) {
		...
		if (vget(hfsmp->hfs_attrdata_vp, LK_EXCLUSIVE) == 0) {
			...
			vnode_pager_setsize(hfsmp->hfs_attrdata_vp, fp->ff_size);
			vput(hfsmp->hfs_attrdata_vp);
		}
	}
...
static int
hfs_invalidate_block_numbers_callback(struct buf * bp, void *args_in)
{
	daddr_t blkno;
...
static void
hfs_invalidate_sectors(struct vnode *vp, daddr_t sectorStart, daddr_t sectorCount)
{
	struct hfs_inval_blk_no args;
                ...
                __unused struct thread *td)
{
	...
	if (cp != hfsmp->hfs_allocation_cp && cp->c_lockowner != curthread)
	 ...
	 */
	if (!vnode_issystem (vp) && (vp->v_type & VREG) && cp_fs_protected (hfsmp->hfs_mp)) {
		cpenabled = 1;
	}
#endif
	 ...
	 */
	vfs_ioattr(hfsmp->hfs_mp, hfsmp->hfs_cp, &ioSize);
	bufferSize = ioSize;
	buffer = hfs_malloc(bufferSize);
	
	/* Get a buffer for doing the I/O */
    bp = geteblk((int)ioSize, GB_NOWAIT_BD);
    bgetvp(hfsmp->hfs_devvp, bp);
	bp->b_data = buffer;
	
	resid = (off_t) blockCount * (off_t) hfsmp->blockSize;
	srcSector = (daddr_t) oldStart * hfsmp->blockSize / hfsmp->hfs_logical_block_size;
	destSector = (daddr_t) newStart * hfsmp->blockSize / hfsmp->hfs_logical_block_size;
	while (resid > 0) {
		ioSize = MIN(bufferSize, (size_t) resid);
		ioSizeSectors = (int) ioSize / hfsmp->hfs_logical_block_size;
		
		/* Prepare the buffer for reading */
        bp->b_iocmd = BIO_READ;
		bp->b_bufsize = ioSize;
		bp->b_bcount = ioSize;
		bp->b_blkno = srcSector;
		bp->b_lblkno = srcSector;
		...
		/* Do the read */
		BO_STRATEGY(&hfsmp->hfs_devvp->v_bufobj, bp);
        err = bufwait(bp);;
		if (err) {
#if CONFIG_PROTECT
        ...
        bp->b_bufsize = ioSize;
        bp->b_bcount = ioSize;
        bp->b_blkno = destSector;
        bp->b_lblkno = destSector;
        
        // MARK: fua not implemented in freebsd vfs
//		if (vp->v_vflag & VV_SYSTEM && journal_uses_fua(hfsmp->jnl))
//			buf_markfua(bp);
...
//		vnode_startwrite(hfsmp->hfs_devvp);
	...
	if (!err)
		hfs_invalidate_sectors(vp, (daddr_t)oldStart*sectorsPerBlock, (daddr_t)blockCount*sectorsPerBlock);
 ...
 */
static int
hfs_reclaim_extent(struct hfsmount *hfsmp, const u_long allocLimit, struct hfs_reclaim_extent_info *extent_info, struct thread *td)
{
	int error = 0;
	int index;
	struct cnode *cp;
	...
	if (oldStartBlock < allocLimit) {
		newBlockCount = (int) allocLimit - oldStartBlock;
...
relocate_full_extent:
	...
	error = hfs_copy_extent(hfsmp, extent_info->vp, oldStartBlock,
                            newStartBlock, newBlockCount, td);
	if (error) {
...
hfs_reclaim_file(struct hfsmount *hfsmp, struct vnode *vp, u_int32_t fileID,
                 u_int8_t forktype, u_long allocLimit, struct thread *td)
{
	int error = 0;
	...
	extent_info->is_sysfile = vp->v_vflag & VV_SYSTEM;
	if ((vp->v_type & VDIR) && (cp->c_flag & C_HARDLINK)) {
	...
	for (i = 0; i < kHFSPlusExtentDensity; ++i) {
		...
		error = hfs_reclaim_extent(hfsmp, allocLimit, extent_info, td);
		if (error) {
...
hfs_journal_relocate_callback(void *_args)
{
	...
	error = bread(hfsmp->hfs_devvp,
                           ...
                           hfsmp->blockSize, args->td->td_ucred, &bp);
	if (error) {
	...
	jibp = (JournalInfoBlock*) bp->b_data;
 ...
 */
static int
hfs_relocate_journal_file(struct hfsmount *hfsmp, u_int32_t jnl_size, int resize_type, struct thread *td)
{
	int error;
	int journal_err;
	int lockflags;
	...
	if (hfsmp->jvp == hfsmp->hfs_devvp) {
		...
		hfsmp->jnl_size = (int) newBlockCount * hfsmp->blockSize;
	}
 ...
 */
static int
hfs_reclaim_journal_file(struct hfsmount *hfsmp, u_int32_t allocLimit, struct thread *td)
{
	int error = 0;
	...
	error = hfs_relocate_journal_file(hfsmp, (u_int)hfs_blk_to_bytes(blockCount, hfsmp->blockSize),
									  HFS_RESIZE_TRUNCATE, td);
	if (error == 0) {
 ...
 */
static int
hfs_reclaim_journal_info_block(struct hfsmount *hfsmp, u_int32_t allocLimit, struct thread *td)
{
	int error;
	int journal_err;
	int lockflags;
	...
	error = bread(hfsmp->hfs_devvp,
                           (uint64_t)hfsmp->vcbJinfoBlock * (hfsmp->blockSize/hfsmp->hfs_logical_block_size),
                           hfsmp->blockSize, td->td_ucred, &old_bp);
	if (error) {
	...
	new_bp = getblk(hfsmp->hfs_devvp,
                        (uint64_t)newBlock * (hfsmp->blockSize/hfsmp->hfs_logical_block_size),
                        hfsmp->blockSize, 0, 0, 0);
	bcopy((char*)old_bp->b_data, (char*)new_bp->b_data, hfsmp->blockSize);
	brelse(old_bp);
//	if (journal_uses_fua(hfsmp->jnl))
//		buf_markfua(new_bp);
	error = bwrite(new_bp);
 ...
 */
static int
hfs_extend_journal(struct hfsmount *hfsmp, u_int32_t sector_size, u_int64_t sector_count, struct thread *td)
{
	int error = 0;
	...
	if (hfs_resize_debug) {
		printf ("hfs_extend_journal: journal old=%u, new=%ld\n", hfsmp->jnl_size, calc_journal_size);
	}
    
	/* Extend the journal to the new calculated size */
	error = hfs_relocate_journal_file(hfsmp, (int) calc_journal_size, HFS_RESIZE_EXTEND, td);
	if (error == 0) {
 ...
 */
static int
hfs_reclaim_xattr(struct hfsmount *hfsmp, struct vnode *vp, u_int32_t fileID, u_int32_t allocLimit, struct thread *td)
{
	int error = 0;
	struct hfs_reclaim_extent_info *extent_info;
	int i;
 ...
 */
static int
hfs_reclaim_xattrspace(struct hfsmount *hfsmp, u_int32_t allocLimit, struct thread *td)
{
	int error = 0;
	FCB *fcb;
 ...
 */
static int
hfs_reclaim_filespace(struct hfsmount *hfsmp, u_int32_t allocLimit, struct thread *td)
{
	int error;
	FCB *fcb;
	struct BTreeIterator *iterator = NULL;
	...
	while (1) {
		...
		datafork = VTOF(vp);
		if ((datafork && datafork->ff_blocks > 0) || (vp->v_type & VDIR)) {
		...
		if (((VTOC(vp)->c_blocks - (datafork ? datafork->ff_blocks : 0)) > 0) || (vp->v_type & VDIR)) {
			if ((vp->v_type & VDIR)) {
 ...
 */
static int
hfs_reclaimspace(struct hfsmount *hfsmp, u_int32_t allocLimit, u_int32_t reclaimblks, struct thread *td)
{
	int error = 0;
	 ...
	 * fail.
	 */
	error = hfs_reclaim_journal_file(hfsmp, allocLimit, td);
	if (error) {
	...
	error = hfs_reclaim_journal_info_block(hfsmp, allocLimit, td);
	if (error) {
	...
	error = hfs_reclaim_filespace(hfsmp, allocLimit, td);
	if (error) {
	...
	error = hfs_reclaim_xattrspace(hfsmp, allocLimit, td);
	if (error) {
...
again:;
	...
	TAILQ_FOREACH(range, &hfsmp->hfs_reserved_ranges[HFS_LOCKED_BLOCKS], rl_link) {
		if (rl_overlap(range, hfsmp->allocLimit, RL_INFINITY) != RL_NOOVERLAP) {
			...
			tsleep(hfs_reclaimspace, PINOD, "waiting on reserved blocks", 100 * 1000000);
			goto again;
		}
	}
	hfs_systemfile_unlock(hfsmp, lockf);
