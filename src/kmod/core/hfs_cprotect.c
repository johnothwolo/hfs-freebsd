 ...
 */
static int cp_root_major_vers(struct mount* mp);
...
static int cp_lock_vnode_callback(struct vnode*, void *);
static int cp_vnode_is_eligible (struct vnode*);
...
int
cp_fs_protected (struct mount* mnt) 
{
...
int cp_vnode_transcode(struct vnode* vp, cp_key_t *k)
{
	struct cnode *cp;
	struct cprotect *entry = 0;
	int error = 0;
...
cp_getrootxattr(struct hfsmount* hfsmp, struct cp_root_xattr *outxattr) 
{
	...
	struct uio uio = uio_create(1, 0, UIO_SYSSPACE, UIO_READ);
...
cp_setrootxattr(struct hfsmount *hfsmp, struct cp_root_xattr *newxattr)
{
	...
	newxattr->flags = htole64(newxattr->flags);
	...
	if (newxattr->auto_roll_min_version || newxattr->auto_roll_max_version) {
		...
		newxattr->auto_roll_min_version = htole32(newxattr->auto_roll_min_version);
		newxattr->auto_roll_max_version = htole32(newxattr->auto_roll_max_version);
	} else if (newxattr->major_version == CP_VERS_4)
		xattr_size = offsetof(struct cp_root_xattr, auto_roll_min_version);
#endif

	newxattr->major_version = htole16(newxattr->major_version);
	newxattr->minor_version = htole16(newxattr->minor_version);
				...
				uint32_t fileid, int options)
{
...
#endif
	xattr->persistent_class		= htole32(entry->cp_pclass);
	xattr->key_os_version		= htole32(entry->cp_key_os_version);
	xattr->key_revision			= htole16(entry->cp_key_revision);

	uint16_t key_len = cpkp_pers_key_len(cpkp);
	xattr->key_len	= htole16(key_len);
	...
	if (rolling) {
		...
		roll_info->off_rsrc = htole64(entry->cp_key_roll_ctx->ckr_off_rsrc);

		key_len = cpkp_pers_key_len(&entry->cp_key_roll_ctx->ckr_keys);
		roll_info->key_len = htole16(key_len);
...
int
cp_get_root_major_vers(struct vnode* vp, uint32_t *level) 
{
	int err = 0;
	struct hfsmount *hfsmp = NULL;
...
static int
cp_root_major_vers(struct mount* mp)
{
	int err = 0;
...
{
	return !vp->v_vflag & VV_SYSTEM && ((vp->v_type & VREG) || (vp->v_type & VDIR));
}
...
void cp_device_locked_callback(struct mount* mp, cp_lock_state_t state)
{
	struct hfsmount *hfsmp;
...
int cp_set_auto_roll(hfsmount_t *hfsmp,
						 const hfs_key_auto_roll_args_t *args)
{
	...
	int ret2 = hfs_end_transaction(hfsmp);
...
int cp_handle_strategy(struct buf * bp)
{
	...
	off_rsrc_t off_rsrc = off_rsrc_make(bp->b_lblkno * GetLogicalBlockSize(vp),
										VNODE_IS_RSRC(vp));
	...
	if (!cpx_has_key(io_params.cpx)) {
		...
		int error = cp_handle_vnop(vp, io_op, 0);
		if (error) {
			/*
			 * We have to be careful here.  By this point in the I/O
			 * path, VM or the cluster engine has prepared a struct buf *
			 * with the proper file offsets and all the rest, so
			 * simply erroring out will result in us leaking this
			 * particular struct buf *.  We need to properly decorate the
			 * struct buf * just as buf_strategy would so as to make it
	...
	hfs_assert(bp->b_bcount <= io_params.max_len);
