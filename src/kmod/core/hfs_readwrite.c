...
#include <sys/fcntl.h>
#include <sys/unistd.h> // R_OK and co.
#include <sys/stat.h>
#include <sys/buf.h>
#include <sys/bio.h>
#include <sys/proc.h>
...
#define can_cluster(size) ((((size & (4096-1))) == 0) && (size <= (MAXPHYS/2)))
...
extern int hfs_vfs_vget (struct mount *mp, ino_t ino, struct vnode **vpp, struct thread *td);
...
static int  do_hfs_truncate(struct vnode *vp, off_t length, int flags, int skip, struct thread *td);
 ...
 */
int
hfs_vnop_read(struct vop_read_args *ap)
{
	/*
    ...
    ssize_t xfersize, blockOffset, logBlockSize, logBlockSizeBk, logBlockNo;
    struct buf *bp;
	off_t filesize;
	off_t filebytes;
	...
	__unused int io_throttle = 0;
	__unused int throttled_count = 0;
    int seqcount = 0;
...
//	retval = cluster_read(vp, uio, filesize, ap->a_ioflag |io_throttle);
    ...
    logBlockSizeBk = logBlockSize = GetLogicalBlockSize(vp);
    
    for (retval = 0, bp = NULL; uio->uio_resid > 0; bp = NULL) {
        if ((filebytes -= uio->uio_offset) <= 0)
            break;
        
        logBlockSize = logBlockSizeBk;
        logBlockNo = uio->uio_offset / logBlockSize;
        blockOffset = (logBlockSize - 1) & uio->uio_offset;
        xfersize = logBlockSize - blockOffset;
        
        if (uio->uio_resid < xfersize)
            xfersize = uio->uio_resid;
        if (filebytes < xfersize)
            xfersize = filebytes;
        
        if (uio->uio_offset + xfersize >= filesize){
            retval = bread(vp, logBlockNo, logBlockSize, NOCRED, &bp);
        } else if ((vp->v_mount->mnt_flag & MNT_NOCLUSTERR) == 0) {
            retval =  cluster_read(vp, filesize, logBlockNo, logBlockSize, NOCRED,
                                   blockOffset + uio->uio_resid,
                                   seqcount, 0, &bp);
        } else if (seqcount > 1) {
            daddr_t logBlockNext = logBlockNo + 1;
            int nextsize = (int) logBlockSize;
            retval = breadn(vp, logBlockNo, logBlockSize, &logBlockNext, &nextsize, 1, NOCRED, &bp);
        } else {
            retval = bread(vp, logBlockNo, logBlockSize, NOCRED, &bp);
        }
         ...
         */
        logBlockSize -= bp->b_resid;
        if (logBlockSize < xfersize) {
            if (logBlockSize == 0)
                break;
            xfersize = logBlockSize;
        }
        retval = uiomove((char *)bp->b_data + blockOffset, (int)xfersize, uio);
        if (retval)
            break;
        vfs_bio_brelse(bp, ap->a_ioflag);
    }
...
//		(int)uio->uio_offset, uio->uio_resid, (int)filesize,  (int)filebytes, 0);
	...
	if (hfsmp->hfc_stage == HFC_RECORDING && retval == 0) {
		...
		bytesread = start_resid - (uio)->uio_resid;
 ...
 */
static int hfs_zero_eof_page(struct vnode* vp, off_t zero_up_to)
{
	hfs_assert(VTOC(vp)->c_lockowner != curthread);
	hfs_assert(VTOC(vp)->c_truncatelockowner == curthread);

	struct filefork *fp = VTOF(vp);

	if (!(fp->ff_size & PAGE_MASK) || zero_up_to <= fp->ff_size) {
		// Nothing to do
		return 0;
	}

	zero_up_to = MIN(zero_up_to, (off_t)round_page(fp->ff_size));
 ...
 */
int
hfs_vnop_write(struct vop_write_args *ap)
{
	struct uio *uio = ap->a_uio;
	struct vnode *vp = ap->a_vp;
	struct cnode *cp;
	...
	int cnode_locked = 0;
	int partialwrite = 0;
	__unused int do_snapshot = 1;
	__unused time_t orig_ctime = VTOC(vp)->c_ctime;
	__unused int took_truncate_lock = 0;
	__unused int io_return_on_throttle = 0;
	__unused int throttled_count = 0;
...
again:
	...
	cred = ap->a_cred;
    if (cred && priv_check_cred(cred, PRIV_VFS_ADMIN) != 0)
	...
	if ((cp->c_truncatelockowner == HFS_SHARED_OWNER) &&
		...
		if (lck_rw_lock_shared_to_exclusive(&cp->c_truncatelock) == FALSE) {
			...
			cp->c_truncatelockowner = curthread;  
		}
	}
...
//				 (int)offset, uio->uio_resid, (int)fp->ff_size,
//				 (int)filebytes, 0);
...
#if QUOTA
	retval = hfs_chkdq(cp, (int64_t)(roundup(bytesToAdd, hfsmp->blockSize)), cred, 0);
	if (retval)
		goto exit;
#endif /* QUOTA */
...
//			(int)offset, uio->uio_resid, (int)fp->ff_size,  (int)filebytes, 0);
	}
	...
	if ((retval == ENOSPC) && (filebytes > offset)) {
		retval = 0;
		partialwrite = 1;
		uio->uio_resid = ((uio->uio_resid - bytesToAdd));
		resid -= bytesToAdd;
...
//		}
		 ...
		 * vnode_pager_setsize.  hfs_vnop_pageout relies on fp->ff_new_size or
		...
		if (filesize > fp->ff_size) {
			...
			vnode_pager_setsize(vp, filesize);
		}
...
//							   0, lflag | IO_NOZERODIRTY | io_return_on_throttle);
		...
		if (retval) {
			...
			if (filesize > origFileSize) {
				vnode_pager_setsize(vp, origFileSize);
			}
			goto ioerr_exit;
		}
...
ioerr_exit:
	...
	if (resid > uio->uio_resid) {
		...
		if (cp->c_mode & (S_ISUID | S_ISGID)) {
			cred = ap->a_cred;
			if (cred && priv_check_cred(cred, PRIV_VFS_ADMIN)) {
	...
	if (retval) {
		if (ioflag & IO_UNIT) {
			(void)hfs_truncate(vp, origFileSize, ioflag & IO_SYNC, 0, curthread);
			uio->uio_offset = (uio->uio_offset - (resid - uio->uio_resid));
			uio->uio_resid = resid;
			filebytes = (off_t)fp->ff_blocks * (off_t)hfsmp->blockSize;
		}
	} else if ((ioflag & IO_SYNC) && (resid > uio->uio_resid))
...
struct user32_access_t {
	...
	short     num_groups;       /* IN: number of groups user belongs to */
	int       num_files;        /* IN: number of files to process */
	void*     file_ids;         /* IN: array of file ids */
	void*     groups;           /* IN: array of groups */
	void*     access;           /* OUT: access info for each file (0 for 'has access') */
};
...
struct user32_ext_access_t {
	...
	uint32_t   map_size;        /* IN: size of the bit map */
	void*      file_ids;        /* IN: Array of file ids */
	void*      bitmap;          /* OUT: hash-bitmap of interesting directory ids */
	void*      access;          /* OUT: access info for each file (0 for 'has access') */
	uint32_t   num_parents;   /* future use */
	void*      parents;   /* future use */
};
...
struct user64_ext_access_t {
	...
	uint32_t      map_size;     /* IN: size of the bit map */
	void*         file_ids;     /* IN: array of file ids */
	void*         bitmap;       /* IN: array of groups */
	void*         access;       /* OUT: access info for each file (0 for 'has access') */
	uint32_t      num_parents;/* future use */
	void*         parents;/* future use */
};
...
do_access_check(struct hfsmount *hfsmp, int *err, struct access_cache *cache, HFSCatalogNodeID nodeID, 
    struct cnode *skip_cp, struct proc *theProcPtr, struct ucred* myp_ucred,
    struct thread *my_td,
    char *bitmap,
    uint32_t map_size,
    cnid_t* parents,
    uint32_t num_parents)
{
	...
	if (priv_check_cred(myp_ucred, PRIV_VFS_ADMIN) == 0) {
	...
	if ((cnattr.ca_recflags & kHFSHasSecurityMask) != 0) {
	    ...
	    if (vp->v_type == VDIR) {
            myErr = VOP_ACCESS(vp, VREAD | VEXEC, myp_ucred, my_td);
	    } else {
...
do_bulk_access_check(struct hfsmount *hfsmp, struct vnode *vp,
    struct vop_ioctl_args *ap, int arg_size, struct thread *td)
{
    /*
     ...
     */
    boolean_t check_leaf = true;
    
    struct user64_ext_access_t *user_access_structp;
    __unused struct user64_ext_access_t tmp_user_access;
    ...
    user_access_structp = (struct user64_ext_access_t *)ap->a_data;
        ...
        tmp_user_access.file_ids  = (accessp->file_ids);
        tmp_user_access.bitmap    = NULL;
        tmp_user_access.access    = (accessp->access);
        ...
        tmp_user_access.file_ids  = (accessp->file_ids);
        tmp_user_access.bitmap    = (accessp->bitmap);
        tmp_user_access.access    = (accessp->access);
        tmp_user_access.parents    = (accessp->parents);
    ...
    for (i = 0; i < num_files; i++) {
        ...
        if ((!parents) && (!priv_check_cred(cred, PRIV_VFS_ADMIN))) {
            access[i] = 0;
            continue;
        }
        ...
        if (check_leaf) {
            ...
            if ((cnattr.ca_recflags & kHFSHasSecurityMask) != 0) {
                ...
                if ((cvp)->v_type == VDIR) {
                    myErr = VOP_ACCESS(vp, VREAD | VEXEC, td->td_ucred, td);
                } else {
        ...
        myaccess = do_access_check(hfsmp, &error, &cache, catkey.hfsPlus.parentID,
                                   skip_cp, p, cred, td, bitmap, map_size, parents, num_parents);
 ...
 */
int
hfs_vnop_ioctl( struct vop_ioctl_args /* {
		struct vnode* a_vp;
		long  a_command;
		caddr_t  a_data;
		int  a_fflag;
	...
	} */ *ap)
{
	...
	struct ucred* cred = td->td_ucred;
	__unused struct proc* p = td->td_proc;
	struct statfs *vfsp;
	boolean_t is64bit;
	...
	case HFSIOC_GETPATH:
	{
		...
		vfsp = &(HFSTOVFS(hfsmp))->mnt_stat;
		if (priv_check_cred(cred, PRIV_VFS_ADMIN) &&
			cred->cr_uid != vfsp->f_owner) {
			return (EACCES);
		}
	...
	case HFSIOC_TRANSFER_DOCUMENT_ID:
	{
		...
		if (!(cp->c_bsdflags & UF_TRACKED)) {
			...
			if (f_extinfo->document_id == 0) {
				...
				if ((error = hfs_generate_document_id(hfsmp, &new_id, td)) == 0) {
					//
			...
			add_fsevent(FSE_DOCID_CHANGED, context,
				    ...
				    FSE_ARG_INO,   (ino_t)to_cp->c_fileid,    // dst inode #
...
fail_change_next_allocation:
	...
	case HFSIOC_SETBACKINGSTOREINFO: {
		...
		if (vp->v_mount == vnode_mount(di_vp)) {
			(void)vput(di_vp);
			file_drop(bsdata->backingfd);
			return (EINVAL);
		}
	...
	case UFSRESUME: {
		vfsp = &((vp)->v_mount)->mnt_stat;
		if (cred->cr_uid != vfsp->f_owner &&
			!priv_check_cred(cred, PRIV_VFS_ADMIN))
			return (EACCES);

		return hfs_thaw(hfsmp, curproc);
	}
...
//		 *
//		 * So, to serialize against listxattr (which will grab struct buf * meta references on
...
//	}
	...
	case FFSYNC: {
		...
		if (error == 0) {
			error = hfs_fsync(vp, MNT_WAIT, HFS_FSYNC_FULL, td);
			hfs_unlock(VTOC(vp));
		}
...
//	    break;
	...
	case HFS_SET_BOOT_INFO:
		if ((vp->v_vflag & VV_ROOT) == 0)
			return(EINVAL);
            if (!priv_check_cred(cred, PRIV_VFS_ADMIN) && (cred->cr_uid != HFSTOVFS(hfsmp)->mnt_stat.f_owner))
	...
	case HFSIOC_MARK_BOOT_CORRUPT:
		...
		if ((HFSTOVFS(hfsmp)->mnt_flag & MNT_ROOTFS) == 0 || ((vp->v_vflag & VV_ROOT) == 0)) {
			return EINVAL;
		}
		if (hfsmp->hfs_flags & HFS_READ_ONLY) {
			return (EROFS);
...
//	/* case HFS_DISABLE_METAZONE: _IO are the same */HFSIOC_DISABLE_METAZONE  xzzzC_DISABLE_METAZONE: {
...
//		 * in the 4K block size.  This can cause struct buf * collisions
...
#if 0
int
hfs_vnop_select(__unused struct vop_select_args *ap)
/*
 ...
 */
int
hfs_bmap(struct vnode *vp, daddr_t bn, struct vnode **vpp, daddr_t *bnp, unsigned int *runp)
{
	struct filefork *fp = VTOF(vp);
	struct hfsmount *hfsmp = VTOHFS(vp);
	int  retval = E_NONE;
	...
	if (retval == E_NONE) {
		...
		if (runp != NULL) {
			if (can_cluster(logBlockSize)) {
				...
				*runp =(u_int)(bytesContAvail < logBlockSize) ? 0 : (u_int)(bytesContAvail / logBlockSize) - 1;
			} else {
				*runp = 0;
			}
		}
 ...
 */
int
hfs_vnop_blktooff(struct vop_blktooff_args *ap)
/*
 ...
 */
int
hfs_vnop_offtoblk(struct vop_offtoblk_args *ap)
/*
...
{	
	if (ap->a_vp == NULL)
		return (EINVAL);
	*ap->a_lblkno = (daddr_t)(ap->a_offset / (off_t)GetLogicalBlockSize(ap->a_vp));
 ...
 * to the caller that the data should be zeroed (a_bnp == -1).  We
 ...
 *
 *   1. When a read occurs and we return with a_bnp == -1.
 *
 ...
 */
int
hfs_vnop_blockmap(struct vop_bmap_args *ap)
/*
...
*/
{
    ...
    vp = ap->a_vp;
    bo = &vp->v_bufobj;
    bsize = GetLogicalBlockSize(vp);
    a_foffset = ap->a_bn * bsize;
    
    
    /*
     * freebsd doesn't really give us the residual IO size in VOP_BMAP, like xnu does
     * so hfs can't check for invalid ranges
     * Therefore we go the long way to figure that out
     * TODO: find a more efficient approach
     */
    if ((bp = gbincore(bo, ap->a_bn)) == NULL)
        panic("Why is this null???");
    
    bytesContAvail = bp->b_bcount;
    ...
    if (ap->a_bop != NULL)
        *ap->a_bop = &hfsmp->hfs_devvp->v_bufobj;

	if ((vp->v_vflag & VV_SYSTEM) == 0 && vp->v_type != VLNK && !vnode_isswap(vp)) {
		if (cp->c_lockowner != curthread) {
		...
		if (cp->bmap_op == VREAD) {
			if (a_foffset >= fp->ff_size) {
				retval = ERANGE;
				goto exit;
			}

			overlaptype = rl_scan(&fp->ff_invalidranges, a_foffset,
								  a_foffset + (off_t)bytesContAvail - 1,
								  &invalid_range);
			switch(overlaptype) {
				...
				case RL_OVERLAPSTARTSBEFORE:
					...
					*ap->a_bnp = (daddr_t)-1;
					...
					if (((off_t)fp->ff_size > (invalid_range->rl_end + 1)) &&
						((size_t)(invalid_range->rl_end + 1 - a_foffset) < bytesContAvail)) {
						bytesContAvail = invalid_range->rl_end + 1 - a_foffset;
					}
				...
				case RL_OVERLAPENDSAFTER:
					...
					if (invalid_range->rl_start == a_foffset) {
						...
						*ap->a_bnp = (daddr_t)-1;
						 ...
						 */
						do {
							off_t rounded_start = round_page(invalid_range->rl_start);
							if ((off_t)bytesContAvail < rounded_start - a_foffset)
								break;
							if (rounded_start < invalid_range->rl_end + 1) {
								bytesContAvail = rounded_start - a_foffset;
								break;
							}
						} while ((invalid_range = TAILQ_NEXT(invalid_range, rl_link)));
					}
					break;
	...
	if (cp->c_cpentry) {
		...
		cp_io_params(hfsmp, cp->c_cpentry,
					 off_rsrc_make(a_foffset, VNODE_IS_RSRC(vp)),
					 direction, &io_params);
		...
		if (io_params.phys_offset != -1) {
			*ap->a_bnp = ((io_params.phys_offset + hfsmp->hfsPlusIOPosOffset)
...
retry:
	...
	if (cp->bmap_op == VWRITE && (fp->ff_unallocblocks != 0)) {
		if (hfs_start_transaction(hfsmp) != 0) {
			retval = EINVAL;
			goto exit;
		} else {
	...
	if (cp->bmap_op == VWRITE && (fp->ff_unallocblocks != 0)) {
		...
		retval = ExtendFileC(hfsmp, (FCB*)fp, 0, 0, kEFAllMask | kEFNoClumpMask, &actbytes);
	...
	retval = MapFileBlockC(hfsmp, (FCB *)fp, bytesContAvail, a_foffset,
	...
	if (retval) {
		...
		if ((MacToVFSError(retval) != ERANGE) ||
		    cp->bmap_op == VWRITE ||
		    (cp->bmap_op == VREAD && (fp->ff_unallocblocks == 0))) {
			goto exit;
		}
		...
		if (fp->ff_size - a_foffset < (off_t)bytesContAvail)
			bytesContAvail = fp->ff_size - a_foffset;
	...
	if (retval == 0) {
		if (cp->bmap_op == VWRITE) {
			...
			if (r && (a_foffset + (off_t)bytesContAvail) > r->rl_start) {
				 ...
				 * risk here is small.
				 */
				rl_remove(a_foffset, a_foffset + bytesContAvail - 1,
						  &fp->ff_invalidranges);
 ...
 */
int
hfs_vnop_strategy(struct vop_strategy_args *ap)
{
...
int
do_hfs_truncate(struct vnode *vp, off_t length, int flags, int truncateflags, struct thread *td)
{
...
//	KERNEL_DEBUG(HFSDBG_TRUNCATE | DBG_FUNC_START, (int)length, (int)fp->ff_size, (int)filebytes, 0, 0);
...
//			KERNEL_DEBUG(HFSDBG_TRUNCATE | DBG_FUNC_NONE, (int)length, (int)fp->ff_size, (int)filebytes, 0, 0);
		}
...
//		}
			...
			if ((vp->v_vflag & VV_SYSTEM) == 0 && retval == E_NONE) {
				if (length > (off_t)fp->ff_size) {
					...
					cp->c_zftimeout = (int) tv.tv_sec + ZFTIMELIMIT;
				}
			} else {
...
//            ISSET(flags, IO_NOZEROFILL) &&
		...
		if (fp->ff_unallocblocks > 0) {
			...
			finalblks = (u_int) (length + blksize - 1) / blksize;
	...
	retval = hfs_update(vp, 0);
	if (retval) {
//		KERNEL_DEBUG(HFSDBG_TRUNCATE | DBG_FUNC_NONE, -1, -1, -1, retval, 0);
	}

Err_Exit:

//	KERNEL_DEBUG(HFSDBG_TRUNCATE | DBG_FUNC_END, (int)length, (int)fp->ff_size, (int)filebytes, retval, 0);
...
int hfs_ubc_setsize(struct vnode* vp, off_t len, bool have_cnode_lock)
{
#if 0
	int error;

	/*
	 * Call vnode_pager_setsize to give the VM subsystem a chance to do
	 ...
	 */
	if (have_cnode_lock) {
		vnode_pager_setsize(vp, len);
		if (error == EAGAIN) {
			cnode_t *cp = VTOC(vp);

			if (cp->c_truncatelockowner != curthread)
...
hfs_truncate(struct vnode *vp, off_t length, int flags,
			 int truncateflags, struct thread *td)
{
	struct filefork *fp = VTOF(vp);
	off_t filebytes;
	u_int32_t fileblocks;
	...
	if (vnode_isswap(vp) && length
//        && !ISSET(flags, IO_NOAUTH)
        ) {
		return (EPERM);
	}
	...
	bool caller_has_cnode_lock = (cp->c_lockowner == curthread);
	...
	if (length < filebytes) {
		while (filebytes > length) {
			...
			error = do_hfs_truncate(vp, filebytes, flags, truncateflags, td);
			if (error)
				break;
		}
	} else if (length > filebytes) {
		struct ucred* cred = td->td_ucred;
		const bool keep_reserve = cred && priv_check_cred(cred, PRIV_VFS_ADMIN) != 0;
		...
		error = do_hfs_truncate(vp, length, flags, truncateflags, td);
	}
	/* Files that are changing size are not hot file candidates. */
	if (VTOHFS(vp)->hfc_stage == HFC_RECORDING) {
		fp->ff_bytesread = 0;
...
#if HFS_CONFIG_KEY_ROLL
	if (!error && cp->c_truncatelockowner == curthread) {
	...
	int err2 = hfs_ubc_setsize(vp, fp->ff_size, caller_has_cnode_lock);
	if (!error)
		error = err2;
 ...
 */
int
hfs_vnop_allocate(struct vop_allocate_args /* {
		struct vnode* a_vp;
		off_t a_length;
		u_int32_t  a_flags;
...
//	nspace_snapshot_event(vp, orig_ctime, ap->a_length == 0 ? NAMESPACE_HANDLER_TRUNCATE_OP|NAMESPACE_HANDLER_DELETE_OP : NAMESPACE_HANDLER_TRUNCATE_OP, NULL);
	...
	if (length > filebytes) {
		...
		if (total_bytes_added != 0 && orig_request_size < total_bytes_added)
			*(ap->a_len) = roundup(orig_request_size, (off_t)vcb->blockSize);
		...
		retval = hfs_truncate(vp, length, 0, 0, td);
		filebytes = (off_t)fp->ff_blocks * (off_t)vcb->blockSize;
 ...
 */
int
hfs_vnop_pagein(struct vop_pagein_args *ap)
/*
...
retry_pagein:
	 ...
	 * marked C_DELETED, it will call vnode_pager_setsize(0).  As a result, the unmount 
	 ...
	 * call vnode_pager_setsize" because doing that would just shift the problem to the
	...
	while (isize) {
		...
		if ( !vnode_isswap(vp) && VTOHFS(vp)->hfc_stage == HFC_RECORDING && error == 0) {
			...
			/* When ff_bytesread exceeds 32-bits, update it behind the cnode lock. */
			if ((fp->ff_bytesread + bytesread) > 0x00000000ffffffff && cp->c_lockowner != curthread) {
				hfs_lock(cp, HFS_EXCLUSIVE_LOCK, HFS_LOCK_ALLOW_NOEXISTS);
				took_cnode_lock = 1;
			}
			/*
 ...
 */
int
hfs_vnop_pageout(struct vop_pageout_args *ap)
/*
...
hfs_vnop_bwrite(struct buf *bp)
{
	...
	if ((VTOC(vp)->c_fileid == kHFSExtentsFileID) ||
		 ...
		 */
		if (((u_int16_t *)((char *)bp->b_data + bp->b_bcount - 2))[0] == 0x000e) {
			...
			block.blockSize = (u_int) bp->b_bcount;
...
// Pinning essentially moves frequently used files to a higher tier storage in a ssd+hdd configuration.
// We won't support that because this driver will probably never be used to boot FreeBSD.
int
hfs_pin_block_range(struct hfsmount *hfsmp, int pin_state, uint32_t start_block, uint32_t nblocks)
{
	...
	err = VNOP_IOCTL(hfsmp->hfs_devvp, HFSTOCP(hfsmp), ioc, (caddr_t)&pin, 0);
	return err;
}
 ...
 */
int
hfs_relocate(struct  vnode *vp, u_int32_t  blockHint, struct ucred* cred, struct  thread *p)
{
	struct  cnode *cp;
	struct  filefork *fp;
	struct  hfsmount *hfsmp;
	...
	if ((vp->v_vflag & VV_SYSTEM) == 0 && vnodetype != VLNK) {
		hfs_unlock(cp);
		hfs_lock_truncate(cp, HFS_EXCLUSIVE_LOCK, HFS_LOCK_DEFAULT);
	...
	datablks = (u_int) howmany(fp->ff_size, blksize);
	...
	if (vnodetype == VLNK)
		retval = EPERM;
	else if (vp->v_vflag & VV_SYSTEM)
		retval = hfs_clonesysfile(vp, headblks, datablks, blksize, cred, curproc);
	else
...
//		error = cluster_read(vp, auio, copysize, IO_DIRECT);
        // sync or nah?
        error = VOP_READ(vp, &auio, IO_DIRECT | IO_SYNC, curthread->td_ucred);
		if (error) {
			printf("hfs_clonefile: cluster_read failed - %d\n", error);
			break;
		}
        if (auio.uio_resid != 0) {
            printf("hfs_clonefile: cluster_read: uio_resid = %ld\n", (int64_t)auio.uio_resid);
			error = EIO;		
			break;
		}
        ...
        error = VOP_WRITE(vp, &auio, IO_DIRECT | IO_UNIT, curthread->td_ucred);
//		error = cluster_write(vp, auio, writebase + offset,
//		                      writebase + offset + iosize,
//                              auio.uio_offset, 0, IO_DIRECT | IO_SYNC);
		if (error) {
	...
	if ((blksize & PAGE_MASK)) {
        ...
        if(vm_object_sync(vp->v_object, writebase, writebase + offset, true, true) == true)
            panic("Not syncing...");
                 ...
                 struct ucred* cred, struct proc *p)
{
	...
	breadcnt = (int) bufsize / iosize;

    bufp = hfs_malloc(bufsize);

	start_blk = ((daddr_t)blkstart * blksize) / iosize;
	last_blk  = ((daddr_t)blkcnt * blksize) / iosize;
	blkno = 0;
	...
	while (blkno < last_blk) {
		...
		for (i = 0, blk = blkno; (i < breadcnt) && (blk < last_blk); ++i, ++blk) {
			error = (int)bread(vp, blk, iosize, cred, &bp);
			if (error) {
				printf("hfs_clonesysfile: meta_bread error %d\n", error);
				goto out;
			}
			if (bp->b_bcount != iosize) {
                printf("hfs_clonesysfile: b_bcount is only %ld\n", bp->b_bcount);
				goto out;
			}
			bcopy((char *)bp->b_data, offset, iosize);
		...
		for (i = 0; (i < breadcnt) && (blkno < last_blk); ++i, ++blkno) {
			bp = getblk(vp, start_blk + blkno, (int)iosize, 0, 0, 0);
			if (bp == NULL) {
                printf("hfs_clonesysfile: getblk failed on blk %ld\n", start_blk + blkno);
				error = EIO;
				goto out;
			}
			bcopy(offset, (char *)bp->b_data, iosize);
			error = (int)bwrite(bp);
			bp = NULL;
			if (error)
				goto out;
...
out:
	...
	error = hfs_fsync(vp, MNT_WAIT, 0, curthread);

	return (error);
}

int hfs_flush_invalid_ranges(struct vnode* vp)
{
	cnode_t *cp = VTOC(vp);

	hfs_assert(cp->c_lockowner == curthread);
	hfs_assert(cp->c_truncatelockowner == curthread);
...
//									IO_HEADZEROFILL | IO_NOZERODIRTY | IO_NOCACHE);
                u_int ext_size = (u_int) round_page(exts[i].end - 1 - exts[i].start);
                vm_page_t page = vm_page_lookup(vp->v_object, OFF_TO_IDX(exts[i].start));
                ret = vnode_pager_generic_putpages(vp, &page, ext_size,
                                                   vnode_pager_putpages_ioflags(IO_DIRECT), 0);
				if (ret) {
					hfs_lock_always(cp, HFS_EXCLUSIVE_LOCK);
					goto exit;
				}
