...
static int  hotfiles_evict (struct hfsmount *, struct thread *);
...
hfs_recording_start(struct hfsmount *hfsmp)
{
	...
	hotdata->maxblocks = (uint32_t) HFC_MAXIMUM_FILESIZE / HFSTOVCB(hfsmp)->blockSize;
	hotdata->hfsmp = hfsmp;
...
hfs_recording_init(struct hfsmount *hfsmp)
{
	 ...
	 * hot files, too.
	 */
	if ((HFSTOVFS(hfsmp))->mnt_flag & MNT_NOATIME) {
		hfsmp->hfc_stage = HFC_DISABLED;
		lck_mtx_unlock (&hfsmp->hfc_mutex);
		return EPERM;
	}
 ...
 */
int
hfs_hotfilesync(struct hfsmount *hfsmp, struct thread * ctx)
{
	if (hfsmp->hfc_stage) {
		struct timeval tv;
...
hfs_addhotfile_internal(struct vnode *vp)
{
	...
	if ((vp->v_type & VDIR) || vp->v_vflag & VV_SYSTEM || (cp->c_flag & (C_DELETED | C_NOEXISTS))) {
		return 0;
	}
	...
	if ((hfsmp->hfs_flags & HFS_CS_HOTFILE_PIN) && vnode_isfastdevicecandidate(vp)) {
        ...
        memcpy(pname, curproc->p_comm, sizeof(curproc->p_comm));
		...
		temperature = (uint32_t) ffp->ff_bytesread / ffp->ff_size;
...
hfs_hotfile_adjust_blocks(struct vnode *vp, int64_t num_blocks)
{
	...
	atomic_add_long((volatile u_long*)&hfsmp->hfs_hotfile_blk_adjust, num_blocks);
...
hfs_hotfile_cur_freeblks(hfsmount_t *hfsmp)
{
	...
	if (cur_blk_adjust) {
		atomic_add_long((volatile u_long*)&hfsmp->hfs_hotfile_blk_adjust, -cur_blk_adjust);
...
hotfiles_adopt(struct hfsmount *hfsmp)
{
	...
	for (i = listp->hfl_next; (i < last) && (blksmoved < HFC_BLKSPERSYNC); ++i) {
		...
		if (hfsmp->hfs_flags & HFS_CS_HOTFILE_PIN) {
			...
			// region of the disk
			//
			error = hfs_relocate(vp, hfsmp->hfs_hotfile_start, curthread->td_ucred, curthread);
		}
 ...
 */
static int
hotfiles_evict(struct hfsmount *hfsmp, struct thread * td)
{
	BTreeIterator * iterator = NULL;
	struct vnode *vp;
	HotFileKey * key;
...
//        vrecycle(vp); // FIXME: vrecycle
	...
	if (error) {
		if (!ignore_btree_errs) {
			printf("hfs: hfc_btree_open: BTOpenPath error %d; filesize %ld\n", error, VTOF(vp)->ff_size);
			error = MacToVFSError(error);
		} else {
			error = 0;
		}
...
hfc_btree_close(struct hfsmount *hfsmp, struct vnode *vp)
{
	...
	if (vget(vp, LK_EXCLUSIVE) == 0) {
        (void) hfs_fsync(vp, MNT_WAIT, 0, td);
...
//		.cn_pnlen = sizeof(filename),
    ...
    vattr_null(&va);
    va.va_type = VREG;
    va.va_mode = S_IFREG | S_IRUSR | S_IWUSR;
    va.va_uid = 0;
    va.va_gid = 0;
...
//		.cn_pnlen = sizeof(filename),
	...
	if (!(vp->v_type & VREG) || cp->c_linkcount != 1) {
		error = EFTYPE;
		goto out;
	}
	...
	if (VTOF(vp)->ff_size < nodesize) {
		 ...
		 */
		((FndrFileInfo *)&cp->c_finderinfo[0])->fdFlags |= SWAP_BE16 (kIsInvisible + kNameLocked);
...
//		vnode_setnoflush(vp);
//        vn_canvmio(<#struct vnode *vp#>) FIXME: this....
		...
		if (error == 0) {
			struct vop_write_args args;
			struct uio auio;
            struct iovec aiov;
            
            aiov.iov_base = buffer;
            aiov.iov_len = nodesize;
