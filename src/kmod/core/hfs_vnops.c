...
int hfs_makenode(struct vnode *dvp, struct vnode **vpp,
                        ...
                        struct thread * ctx);
int hfs_metasync(struct hfsmount *hfsmp, daddr_t node, struct thread *p);
...
#if FIFO
static int hfsfifo_read(struct vop_read_args *);
static int hfsfifo_write(struct vop_write_args *);
static int hfsfifo_close(struct vop_close_args *);

extern int (**fifo_vnodeop_p)(void *);
#endif /* FIFO */

int hfs_vnop_close(struct vop_close_args*);
int hfs_vnop_exchange(struct vop_exchange_args*);
int hfs_vnop_fsync(struct vop_fsync_args*);
int hfs_vnop_mkdir(struct vop_mkdir_args*);
int hfs_vnop_mknod(struct vop_mknod_args*);
int hfs_vnop_getattr(struct vop_getattr_args*);
int hfs_vnop_open(struct vop_open_args*);
int hfs_vnop_readdir(struct vop_readdir_args*);
int hfs_vnop_rename(struct vop_rename_args*);
int hfs_vnop_renamex(struct vop_rename_args*);
int hfs_vnop_rmdir(struct vop_rmdir_args*);
int hfs_vnop_symlink(struct vop_symlink_args*);
int hfs_vnop_setattr(struct vop_setattr_args*);
int hfs_vnop_readlink(struct vop_readlink_args *);
int hfs_vnop_pathconf(struct vop_pathconf_args *);
int hfs_vnop_mmap(struct vop_mmapped_args *ap);
int hfsspec_read(struct vop_read_args *);
int hfsspec_write(struct vop_write_args *);
int hfsspec_close(struct vop_close_args *);
 ...
 */
int
hfs_vnop_create(struct vop_create_args *ap)
{
	/*
	 ...
	 */
	trace_return (hfs_makenode(ap->a_dvp, ap->a_vpp, ap->a_cnp, ap->a_vap, curthread));
}
 ...
 */
int
hfs_vnop_mknod(struct vop_mknod_args *ap)
{
	...
	/* Create the vnode */
	error = hfs_makenode(dvp, vpp, ap->a_cnp, vap, curthread);
	if (error)
		trace_return (error);
...
hfs_uncompressed_size_of_compressed_file(struct hfsmount *hfsmp, struct vnode *vp, cnid_t fid, off_t *size, int skiplock)
{
	...
	if (putaway) {		/* did we use hfs_vget() to get this vnode? */
		vput(vp);	/* if so, release it and set it to null */
		vp = NULL;
	}
	return ret;
}
...
int
hfs_hides_rsrc(struct thread * ctx, struct cnode *cp, int skiplock)
{
	if (ctx == decmpfs_ctx)
		return 0;
...
int
hfs_hides_xattr(struct thread * ctx, struct cnode *cp, const char *name, int skiplock)
{
	if (ctx == decmpfs_ctx)
		return 0;
	if (!hfs_file_is_compressed(cp, skiplock))
 ...
 */
int
hfs_vnop_open(struct vop_open_args *ap)
{
	 ...
	 */
	if ((cp->c_bsdflags & APPEND) && !(vp->v_type & VDIR) &&
	    (ap->a_mode & (FWRITE | O_APPEND)) == FWRITE)
		trace_return (EPERM);

	if (vp->v_vflag & VV_SYSTEM)
		trace_return (EBUSY);  /* file is in use by the kernel */
...
#if CONFIG_PROTECT
	if (ISSET(ap->a_mode, FENCRYPTED) && cp->c_cpentry && (vp->v_type & VREG)) {
    ...
    vnode_create_vobject(vp, fp->ff_size, ap->a_td);
 ...
 * ambiguous lock requirement
 */
int
hfs_vnop_close(struct vop_close_args *ap)
{
...
//	struct thread *td = ap->a_td;
	...
	int busy;
    int tooktrunclock = 0;
    int lkstat, __unused error;
//	int knownrefs = 0;

    cp = VTOC(vp);
    hfsmp = VTOHFS(vp);
    
    lkstat = hfs_ambiguous_lock(VOP_ISLOCKED(vp), cp, HFS_LOCK_DEFAULT);

#if CONFIG_PROTECT
	if (cp->c_cpentry && ISSET(ap->a_fflag, FENCRYPTED) && (vp->v_type & VREG)) {
	 ...
	 */
#ifdef NAMEDSTREAMS
	if ((vp->v_type == VREG) && (cp->c_rsrc_vp) && (vnode_isnamedstream(cp->c_rsrc_vp))) {
		uint32_t blks;
			...
			if (cp->c_rsrc_vp) {
				...
				if (!vnode_isinuse(vp, knownrefs)){
					...
					if (blks < VTOF(vp)->ff_blocks){
						(void) hfs_truncate(vp, VTOF(vp)->ff_size, IO_NDELAY, 0, td);
					}
				}
			}
		}
...
//	}
	...
	} else if (vp->v_vflag & VV_SYSTEM && !busy) {
		vrecycle(vp);
	}
 ...
 */
int
hfs_vnop_getattr(struct vop_getattr_args *ap)
{
    struct vnode *vp = ap->a_vp;
    struct vattr *vap = ap->a_vap;
    struct hfsmount *hfsmp;
     ...
     */
    if ((cp->c_touch_acctime || cp->c_touch_chgtime || cp->c_touch_modtime) != 0) {
    ...
    if (v_type == VDIR) {
        data_size = (cp->c_entries + 2) * AVERAGE_HFSDIRENTRY_SIZE;
        {
            ...
            vap->va_nlink = (u_int64_t)nlink;
        }
        
    } else /* !VDIR */ {
        data_size = VCTOF(vp, cp)->ff_size;
        
        vap->va_nlink = (u_int64_t)cp->c_linkcount;
        
        {
            ...
            if (hide_size) {
                ...
                blocks = cp->c_blocks;
                vap->va_bytes = blocks * (u_int64_t)hfsmp->blockSize;
            }
            else
#endif
            {
    ...
    vap->va_birthtime.tv_sec = cp->c_itime;
    ...
    vap->va_blocksize = hfsmp->hfs_logBlockSize;
    vap->va_uid = cp->c_uid;
    vap->va_gid = cp->c_gid;
    vap->va_mode = cp->c_mode & ~S_IFMT;
    vap->va_flags = cp->c_bsdflags;
    vap->va_type = IFTOVT(cp->c_mode);
    ...
    vap->va_fsid = dev2udev(hfsmp->hfs_raw_dev);
    vap->va_rdev = (v_type == VBLK || v_type == VCHR) ? cp->c_rdev : 0;
        ...
        if (hide_size)
            ...
            if (uncompressed_size == -1) {
                ...
                vap->va_size = data_size;
            } else {
                /* use the uncompressed size we fetched above */
                vap->va_size = uncompressed_size;
            }
        } else
            vap->va_size = data_size;
#else
    ...
    
    {
        ...
        if (cp->c_desc.cd_cnid == kHFSRootFolderID)
            ...
            if (!document_id && hfs_should_generate_document_id(hfsmp, cp)) {
                ...
                error = hfs_generate_document_id(hfsmp, &new_document_id, curthread);
                if (error){
                    goto out;
                }
...
hfs_set_bsd_flags(struct hfsmount *hfsmp, struct cnode *cp,
				  ...
				  struct thread * ctx, int *compression_changedp)
{
	u_int16_t *fdFlags;
	...
	if ((new_bsd_flags & UF_TRACKED) && !(cp->c_bsdflags & UF_TRACKED)) {
		...
		if (fip->document_id == 0) {
			if (document_id != 0) {
				...
				add_fsevent(FSE_DOCID_CHANGED, ctx,
							...
							FSE_ARG_INO, (ino_t)0,                // src inode #
							FSE_ARG_INO, (ino_t)cp->c_fileid,     // dst inode #
		...
		if (fip->document_id) {
			...
			add_fsevent(FSE_DOCID_CHANGED, ctx,
						FSE_ARG_DEV, hfsmp->hfs_raw_dev,
						FSE_ARG_INO, (ino_t)cp->c_fileid,          // src inode #
						FSE_ARG_INO, (ino_t)0,                     // dst inode #
	...
	if (new_bsd_flags & UF_HIDDEN)
		*fdFlags |= htobe16(kFinderInvisibleMask);
	else
		*fdFlags &= ~htobe16(kFinderInvisibleMask);
...
int
hfs_vnop_setattr(struct vop_setattr_args *ap)
{
#if 0
	struct vattr *vap = ap->a_vap;
	struct vnode *vp = ap->a_vp;
	...
	if (VATTR_IS_ACTIVE(vap, va_flags) && (vap->va_flags & UF_TRACKED) && !(VTOC(vp)->c_bsdflags & UF_TRACKED)) {
		...
		//
		if (fip->document_id == 0) {
			if (hfs_generate_document_id(hfsmp, &document_id, td) != 0) {
				document_id = 0;
			}
		}
	}
 ...
 */
int
hfs_chmod(struct vnode *vp, int mode, __unused struct ucred* cred, __unused struct proc *p)
{
	...
	if (VTOHFS(vp)->hfs_flags & HFS_UNKNOWN_PERMS) {
		return (0);
	};
#endif
...
int
hfs_write_access(struct vnode *vp, struct ucred* cred, struct thread *td, accmode_t mode, boolean_t considerFlags)
{
	struct cnode *cp = VTOC(vp);
	int retval = 0;
	...
	if ((retval = hfs_owner_rights(VTOHFS(vp), cp->c_uid, cred, td, false)) == 0)
		trace_return ((cp->c_mode & S_IWUSR) == S_IWUSR ? 0 : EACCES);
 
	/* Otherwise, check the groups. */
	if (groupmember(cp->c_gid, cred)) {
		trace_return ((cp->c_mode & S_IWGRP) == S_IWGRP ? 0 : EACCES);
 	}
 
	/* Otherwise, check everyone else. */
	trace_return ((cp->c_mode & S_IWOTH) == S_IWOTH ? 0 : EACCES);
}

// MARK: BUG: vop_stdaccessx and vop_stdaccess call each other infinitely
static int
hfs_vnop_access(struct vop_access_args *ap)
{
    trace_return (hfs_write_access(ap->a_vp, ap->a_cred, ap->a_td, ap->a_accmode, true));
}
...
int
#if !QUOTA
hfs_chown(struct vnode *vp, uid_t uid, gid_t gid, __unused struct ucred* cred,
	__unused struct proc *p)
#else 
hfs_chown(struct vnode *vp, uid_t uid, gid_t gid, struct ucred* cred,
	__unused struct proc *p)
#endif
{
	register struct cnode *cp = VTOC(vp);
 ...
 * an iocount.
 */
static int hfs_flush_rsrc(struct vnode* vp, struct thread * ctx)
{
	cnode_t *cp = VTOC(vp);
...
static int __unused
hfs_exchangedata_getxattr (struct vnode *vp, uint32_t name_selector,  void **buffer, size_t *xattr_size) {
    ...
    //setup iovec
    iovec.iov_base = xattr_rawdata;
    iovec.iov_len = memsize;
    
    //now create the UIO
    uio.uio_iov = &iovec;
    uio.uio_iovcnt = 1;
    uio.uio_offset = 0;
    uio.uio_resid = memsize;
...
static int __unused
hfs_exchangedata_setxattr (struct hfsmount *hfsmp, uint32_t fileid,
		uint32_t name_selector, void *buffer, size_t xattr_size) {
	...
	struct vop_setextattr_args vsa = {
		.a_vp = NULL,
		.a_name = XATTR_NAMES[name_selector], 
...
#if 0
int
hfs_vnop_exchange(struct vop_exchange_args *ap)
{
	...
	if ((error = hfs_lockpair(from_vp, to_vp, HFS_EXCLUSIVE_LOCK)))
		goto exit;
...
int
hfs_vnop_mmap(struct vop_mmapped_args *ap)
{
	struct vnode *vp = ap->a_vp;
	__unused cnode_t *cp = VTOC(vp);
	__unused int error;
...
#if 0
static int hfs_vnop_mnomap(struct vop_mnomap_args *ap)
{
	struct vnode* vp = ap->a_vp;
...
/*
 * Mark the resource fork as needing a vnode_pager_setsize when we drop the
 * cnode lock later.
 */
static void hfs_rsrc_setsize(cnode_t *cp)
{
	/*
	 * We need to take an iocount if we don't have one.  vget
	 * will return ENOENT if the vnode is terminating which is what we
	 * want as it's not safe to call vnode_pager_setsize in that case.
	 */
 ...
 */
int __unused
hfs_move_data(cnode_t *from_cp, cnode_t *to_cp,
				  hfs_move_data_options_t options)
{	
 ...
 */
int
hfs_fsync(struct vnode *vp, int waitfor, hfs_fsync_mode_t fsyncmode, struct thread *td)
{
	struct cnode *cp = VTOC(vp);
	 ...
	 */
	waitdata = wait = (waitfor == MNT_WAIT);
...
datasync:
	...
	else if (fsync_default && vp->v_type == VLNK
             && vp->v_bufobj.bo_dirty.bv_cnt > 0 && VN_IS_DOOMED(vp)) {
		/*
...
metasync:
	if ((vp->v_type & VREG) && vp->v_vflag & VV_SYSTEM) {
		if (VTOF(vp)->fcbBTCBPtr != NULL) {
			microuptime(&tv);
			BTSetLastSync(VTOF(vp), (uint32_t)tv.tv_sec);
		}
		cp->c_touch_acctime = FALSE;
		...
		if ((retval == 0) && wait && fsync_default && cp->c_hint &&
   			...
   			hfs_metasync(VTOHFS(vp), (daddr_t)cp->c_hint, td);
		}
...
int
hfs_metasync(struct hfsmount *hfsmp, daddr_t node, struct thread *p)
{
	struct vnode*	vp;
	struct buf *	bp;
	int lockflags;
	 ...
	 *
	 * BLK_ONLYVALID causes getblk to return a
	 * struct buf * for the daddr_t specified only if it's
	 * currently resident in the cache... the size
	 * parameter to getblk is ignored when this flag
	 * is set
	 */
	bp = getblk(vp, node, 0, 0, 0, GB_NOCREAT | GB_UNMAPPED);
...
hfs_btsync(struct vnode *vp, int sync_transaction)
{
	...
	if ((vp->v_vflag & VV_SYSTEM) && (VTOF(vp)->fcbBTCBPtr != NULL))
		(void) BTSetLastSync(VTOF(vp), (u_int)tv.tv_sec);
	cp->c_touch_acctime = FALSE;
	cp->c_touch_chgtime = FALSE;
	cp->c_touch_modtime = FALSE;
 ...
 */
int
hfs_vnop_rmdir(struct vop_rmdir_args *ap)
{
	struct vnode *dvp = ap->a_dvp;
	struct vnode *vp = ap->a_vp;
...
//	nspace_snapshot_event(vp, orig_ctime, NAMESPACE_HANDLER_DELETE_OP, NULL);
	cp = VTOC(vp); // FIXME: redundant variable assignment?
	...
	if ((cp->c_bsdflags & UF_TRACKED) && ((struct FndrExtendedDirInfo *)((char *)&cp->c_attr.ca_finderinfo + 16))->document_id == 0) {
		...
		if (hfs_generate_document_id(VTOHFS(vp), &newid, curthread) == 0) {
              ...
              int skip_reserve, int only_unlink)
{
	...
	if (dcp == cp) {
		trace_return (EINVAL);	/* cannot remove "." */
	}
	if (cp->c_flag & (C_NOEXISTS | C_DELETED)) {
		return (0);
	}
	if (cp->c_entries != 0) {
		trace_return (ENOTEMPTY);
	}
 ...
 */
int
hfs_vnop_remove(struct vop_remove_args *ap)
{
	struct vnode *dvp = ap->a_dvp;
	struct vnode *vp = ap->a_vp;
    struct thread *td = curthread;
...
//    if (
        ...
        !vnode_isnamedstream(vp)
#endif
//        && ((ap->a_flags & VNODE_REMOVE_SKIP_NAMESPACE_EVENT) == 0)) {
...
relock:
	...
	if ((cp->c_bsdflags & UF_TRACKED) && ((struct FndrExtendedDirInfo *)((char *)&cp->c_attr.ca_finderinfo + 16))->document_id == 0) {
		...
		if (hfs_generate_document_id(VTOHFS(vp), &newid, td) == 0) {
			hfs_lockpair(dcp, cp, HFS_EXCLUSIVE_LOCK);
			((struct FndrExtendedDirInfo *)((char *)&cp->c_attr.ca_finderinfo + 16))->document_id = newid;
			add_fsevent(FSE_DOCID_CHANGED, curthread,
				    FSE_ARG_DEV,   VTOHFS(vp)->hfs_raw_dev,
	...
	error = hfs_removefile(dvp, vp, ap->a_cnp, 0, 0, 0, NULL, 0);
	 ...
	 * (which can potentially perform a vput and
...
//		} else {
			 ...
			 */
			if (((vp->v_type & VDIR) == 1) && (cp->c_linkcount == 1) && 
			    (allow_dirs == 0)) {
			    	trace_return (EPERM);
			}
			trace_return (hfs_unlink(hfsmp, dvp, vp, cnp, skip_reserve));
//		}
	}
...
//		}
	...
	/* 
	 * Do a vnode_pager_setsize to indicate we need to wipe contents if:
	...
	if (isdir == 0 && (!dataforkbusy || !rsrcforkbusy) && (defer_remove == 0)) {
		/*
		 * A vnode_pager_setsize can cause a pagein so defer it
...
//	if (hfsmp->jnl && vp->v_type == VLNK && (defer_remove == 0)) {
...
//	}
	...
	if (dataforkbusy || rsrcforkbusy || isbigfile || defer_remove) {
		...
		if (vp->v_type == VLNK && cp->c_datafork->ff_symlinkptr) {
 ...
 */
int hfs_vnop_rename (struct vop_rename_args *args)
{
#if 0
...
relock:
    ...
    if (error) {
         ...
         * This is ok because the vfs syscall will vput the vnodes
    ...
    if ((fcp->c_bsdflags & UF_TRACKED) && ((struct FndrExtendedDirInfo *)((char *)&fcp->c_attr.ca_finderinfo + 16))->document_id == 0) {
        ...
        if (hfs_generate_document_id(hfsmp, &newid, td) == 0) {
            ...
            add_fsevent(FSE_DOCID_CHANGED, curthread,
                    FSE_ARG_DEV,   hfsmp->hfs_raw_dev,
                    FSE_ARG_INO,   (ino_t)0,             // src inode #
                    FSE_ARG_INO,   (ino_t)fcp->c_fileid,  // dst inode #
                    FSE_ARG_INT32, newid,
                    FSE_ARG_DONE);
            hfs_unlock(fcp);
        } else {
    ...
    if (tcp && (tcp->c_bsdflags & UF_TRACKED) && ((struct FndrExtendedDirInfo *)((char *)&tcp->c_attr.ca_finderinfo + 16))->document_id == 0) {
        ...
        if (hfs_generate_document_id(hfsmp, &newid, td) == 0) {
            ...
            add_fsevent(FSE_DOCID_CHANGED, curthread,
                    FSE_ARG_DEV,   hfsmp->hfs_raw_dev,
                    FSE_ARG_INO,   (ino_t)0,             // src inode #
                    FSE_ARG_INO,   (ino_t)tcp->c_fileid,  // dst inode #
                    FSE_ARG_INT32, newid,
                    FSE_ARG_DONE);
            hfs_unlock(tcp);
        } else {
    ...
    if (tvp) {
        ...
        if (fvp != tvp) {
            ...
            if (ffip->document_id && tfip->document_id) {
                ...
                add_fsevent(FSE_DOCID_CHANGED, curthread,
                        FSE_ARG_DEV, hfsmp->hfs_raw_dev,
                        FSE_ARG_INO, (ino_t)tcp->c_fileid,           // src inode #
                        FSE_ARG_INO, (ino_t)0ULL,                    // dst inode #
                        FSE_ARG_INT32, (uint32_t)tfip->document_id,
                        FSE_ARG_DONE);
                add_fsevent(FSE_DOCID_CHANGED, curthread,
                        FSE_ARG_DEV, hfsmp->hfs_raw_dev,
                        FSE_ARG_INO, (ino_t)fcp->c_fileid,           // src inode #
                        FSE_ARG_INO, (ino_t)tcp->c_fileid,           // dst inode #
...
skip_rm:
     ...
     * To signal VFS, we return ERELOOKUP (which is also used for lookup restarts). This errno
...
out:
    ...
    const ino_t file_id = fcp->c_fileid;
    ...
    /* Now vput the resource forks vnodes if necessary */
    if (tvp_rsrc) {
        vput(tvp_rsrc);
        tvp_rsrc = NULL;
    }
    ...
    if (old_doc_vp) {
        ...
        const ino_t old_file_id = ocp->c_fileid;
 ...
 */
int
hfs_vnop_renamex(struct vop_rename_args *ap)
{
#if 0
	struct vnode *tvp = ap->a_tvp;
	struct vnode *tdvp = ap->a_tdvp;
 ...
 */
int
hfs_vnop_mkdir(struct vop_mkdir_args *ap)
{
	/***** HACK ALERT ********/
	ap->a_cnp->cn_flags |= MAKEENTRY;
	return hfs_makenode(ap->a_dvp, ap->a_vpp, ap->a_cnp, ap->a_vap, curthread);
}
 ...
 */
int
hfs_vnop_symlink(struct vop_symlink_args *ap)
{
	struct vnode **vpp = ap->a_vpp;
	struct vnode *dvp = ap->a_dvp;
	...
	if ((error = hfs_makenode(dvp, vpp, ap->a_cnp, ap->a_vap, curthread))) {
		goto out;
	}
	vp = *vpp;
	if ((error = hfs_lock(VTOC(vp), HFS_EXCLUSIVE_LOCK, HFS_LOCK_DEFAULT))) {
	 ...
	 */
	error = hfs_truncate(vp, len, 0/* FIXME: IO_NOZEROFILL*/, 0, curthread);
	...
	bp = getblk(vp, (daddr_t)0, roundup((int)fp->ff_size, hfsmp->hfs_physical_block_size), 0, 0, 0);
	if (hfsmp->jnl) {
		journal_modify_block_start(hfsmp->jnl, bp);
	}
	datap = (char *)bp->b_data;
	bzero(datap, bp->b_bufsize);
 ...
 */
int
hfs_vnop_readdir(struct vop_readdir_args *ap)
{
	struct vnode *vp = ap->a_vp;
    ...
    struct cookiedata cdata = {0};
	off_t offset;
	off_t startoffset;
	int error = 0;
	int eofflag = 0;
	caddr_t user_start = 0;
	size_t user_len = 0;
	size_t user_original_resid = 0;
	int index;
	unsigned int tag;
	int items;
	int lockflags;
	int extended;
    boolean_t nfs_cookies;
	cnid_t cnid_hint = 0;
	int bump_valence = 0;
    int lockstat;

	items = 0;
	startoffset = offset = uio->uio_offset;
    nfs_cookies = ap->a_ncookies != NULL;
    extended = ISHFSPLUS(hfsmp);
    lockstat = VOP_ISLOCKED(vp);
	...
	if (hfsmp->jnl && uio->uio_segflg == UIO_USERSPACE) {
		...
		if (user_len > (256 * 1024)) {
			...
			user_len = (256 * 1024);
			user_original_resid = uio->uio_resid;
            uio->uio_resid = (ssize_t)(256 * 1024);
		}

		if ((error = vslock(user_start, user_len)) != 0) {
			if (user_original_resid > 0) {
				uio->uio_resid = (user_original_resid);
    ...
    if (lockstat == LK_EXCLOTHER){
        if (user_start) {
            if (user_original_resid > 0) {
                uio->uio_resid = (user_original_resid);
                user_original_resid = 0;
            }
            vsunlock(user_start, user_len);
        }
        trace_return (EBUSY);
    }
    
    // if we're not exclusive, we're shared
    if (lockstat != LK_EXCLUSIVE) {
        hfs_lock_upgrade(cp);
	}
    
	/* Pick up cnid hint (if any). */
//	if (nfs_cookies) {
//		cnid_hint = (cnid_t)(uio->uio_offset >> 32);
//		uio->uio_offset &= 0x00000000ffffffffLL;
//		if (cnid_hint == __INT_MAX__) { /* searching pass the last item */
//			eofflag = 1;
//			goto out;
//		}
//	}
	...
	if (offset == 0 && !(cp->c_flag & C_DELETED)) {	
		...
		{
			struct dirent  dotentry[2];
			
			/* HFS Plus */
			struct dirent *entry = &dotentry[0];
			...
			entry->d_name[1] = '\0';
			entry->d_name[2] = '\0';
            entry->d_reclen = offsetof(struct dirent, d_name) + entry->d_namlen;
			...
			entry->d_name[2] = '\0';
            entry->d_reclen = offsetof(struct dirent, d_name) + entry->d_namlen;
			entry->d_off = offsetof(struct dirent, d_name) + entry->d_namlen;
	
			if ((error = (int) uiomove((caddr_t)dotentry, (int) uiosize, uio))) {
				goto out;
			}
    ...
    if (nfs_cookies){
        cdata.cd_ncookies = cp->c_entries - (int)index; // this should be the maximum no. of cookies we expect.
        cdata.cd_cookies = malloc(cdata.cd_ncookies * sizeof(cdata.cd_ncookies), M_TEMP, M_WAITOK);
        *ap->a_cookies = cdata.cd_cookies;
    }
    
	/* Pack the buffer with dirent entries. */
	error = cat_getdirentries(hfsmp, cp->c_entries, dirhint,
                              &cdata,
                              uio,
                              &items, &eofflag);

	if (user_original_resid > 0) {
		user_original_resid = user_original_resid - ((ssize_t)256*1024 - uio->uio_resid);
		uio->uio_resid = (user_original_resid);
		user_original_resid = 0;
	}
	...
	if (error != 0) {
        ...
        // cleanup cookies if they were requested.
        if (nfs_cookies) {
            free(cdata.cd_cookies, M_TEMP);
            *ap->a_ncookies = 0;
            *ap->a_cookies = NULL;
...
seekoffcalc:
	...
	if (nfs_cookies) {
        ...
        if (error == 0) {
            // doesn't matter if the buffer is too large, nfs will only iterate 'a_ncookies' number of times.
            if (cdata.cd_ncookies >= items)
                *ap->a_ncookies = items;
            else // if (cdata.cd_ncookies < items)
                panic("Corrupt Directory catalog entry?...");
        }
	}
 ...
 */
int
hfs_vnop_readlink(struct vop_readlink_args *ap)
{
	struct vnode *vp = ap->a_vp;
	struct cnode *cp;
	struct filefork *fp;
    ...
    if (VOP_ISLOCKED(vp) != LK_EXCLUSIVE) {
        panic("%s(): lock not owned", __func__);
    }
	...
	if (fp->ff_symlinkptr == NULL) {
		...
		bcopy((char *)bp->b_data, fp->ff_symlinkptr, (size_t)fp->ff_size);

		if (VTOHFS(vp)->jnl && (BUF_ISLOCKED(bp)) == 0) {
 ...
 */
int
hfs_vnop_pathconf(struct vop_pathconf_args *ap)
{
...
hfs_update(struct vnode *vp, int options)
{
	...
	if (((vp->v_vflag & VV_SYSTEM && (cp->c_cnid < kHFSFirstUserCatalogNodeID))) || 
...
hfs_makenode(struct vnode *dvp, struct vnode **vpp, struct componentname *cnp,
             struct vattr *vap, struct thread * ctx)
{
	struct cnode *cp = NULL;
	...
	*vpp = NULL;
	...
	if ((hfs_freeblks(hfsmp, 1) == 0) && (priv_check_cred(ctx->td_ucred, PRIV_VFS_ADMIN) != 0)) {
		error = ENOSPC;
		goto exit;
	}
...
//	if (VATTR_IS_ACTIVE(vap, va_create_time)) {
//		VATTR_SET_SUPPORTED(vap, va_create_time);
		attr.ca_itime = vap->va_ctime.tv_sec;
//	} else {
...
//	if (VATTR_IS_ACTIVE(vap, va_flags)) {
//		VATTR_SET_SUPPORTED(vap, va_flags);
		attr.ca_flags = (u_int) vap->va_flags;
//	}
	 ...
	 */
	hfs_write_dateadded (&attr, (u_int) attr.ca_atime);
...
//	VATTR_SET_SUPPORTED(vap, va_gid);
	 ...
	 */
   	if (hfsmp->hfs_flags & HFS_QUOTAS) {
		if ((error = hfs_quotacheck(hfsmp, 1, attr.ca_uid, attr.ca_gid, vfs_context_ucred(ctx)))) {
			goto exit;
		}
	}	
#endif
	 ...
	 */
	error = hfs_getnewvnode(hfsmp, dvp, cnp, &out_desc, gnv, &attr,
 ...
 */
int
hfsspec_read(struct vop_read_args *ap)
{
	/*
	 * Set access flag.
	 */
 ...
 */
int
hfsspec_write(struct vop_write_args *ap)
{
	/*
 ...
 */
int
hfsspec_close(struct vop_close_args *ap)
{
	struct vnode *vp = ap->a_vp;
	cnode_t *cp = VTOC(vp);
 ...
 */
static int
hfsfifo_read(struct vop_read_args *ap)
{
	/*
	 * Set access flag.
	 */
 ...
 */
static int
hfsfifo_write(struct vop_write_args *ap)
{
	/*
	 * Set update and change flags.
	 */
 ...
 */
static int
hfsfifo_close(struct vop_close_args *ap)
{
	struct vnode *vp = ap->a_vp;
	struct cnode *cp;
...
{
	trace_return (hfs_get_document_id_internal(finderinfo, mode));
}
 ...
 */
int
hfs_vnop_fsync(struct vop_fsync_args *ap)
{
	struct vnode* vp = ap->a_vp;
	...
	trace_return (hfs_fsync(vp, ap->a_waitfor, 0, ap->a_td));
}
...
struct vop_vector hfs_vnodeops = {
    ...
    .vop_access         =   hfs_vnop_access,
    .vop_bmap           =   hfs_vnop_blockmap,
    .vop_lookup         =   hfs_vnop_lookup,
    .vop_close          =   hfs_vnop_close,
    .vop_create         =   hfs_vnop_create,
    .vop_fsync          =   hfs_vnop_fsync,
    .vop_getpages       =   vnode_pager_local_getpages,
    .vop_getpages_async =   vnode_pager_local_getpages_async,
    .vop_getattr        =   hfs_vnop_getattr,
    .vop_inactive       =   hfs_vnop_inactive,
    .vop_ioctl          =   hfs_vnop_ioctl,
//    .vop_link           =   hfs_vnop_link,
    .vop_mkdir          =   hfs_vnop_mkdir,
    .vop_mknod          =   hfs_vnop_mknod,
    .vop_open           =   hfs_vnop_open,
    .vop_pathconf       =   hfs_vnop_pathconf,
    .vop_poll           =   vop_stdpoll,
//    .vop_print          =   hfs_vnop_print,
    .vop_read           =   hfs_vnop_read,
    .vop_readdir        =   hfs_vnop_readdir,
    .vop_readlink       =   hfs_vnop_readlink,
//    .vop_reallocblks    =   hfs_vnop_reallocblks,
    .vop_reclaim        =   hfs_vnop_reclaim,
//    .vop_remove         =   hfs_vnop_remove,
//    .vop_rename         =   hfs_vnop_rename,
//    .vop_rmdir          =   hfs_vnop_rmdir,
//    .vop_setattr        =   hfs_vnop_setattr,
    .vop_strategy       =   hfs_vnop_strategy,
//    .vop_symlink        =   hfs_vnop_symlink,
//    .vop_write          =   hfs_vnop_write,
//    .vop_deleteextattr  =   hfs_vnop_removexattr,
//    .vop_getextattr     =   hfs_vnop_getxattr,
//    .vop_listextattr    =   hfs_vnop_listxattr,
//    .vop_setextattr     =   hfs_vnop_setxattr,
//    .vop_getacl         =   hfs_vnop_getacl,
//    .vop_setacl         =   hfs_vnop_setacl,
//    .vop_aclcheck       =   hfs_vnop_aclcheck,
    .vop_vptofh         =   hfs_vptofh,
    .vop_lock1          =   hfs_vnop_lock1,
    .vop_unlock         =   hfs_vnop_unlock,
};
VFS_VOP_VECTOR_REGISTER(hfs_vnodeops);

#if FIFO
struct vop_vector hfs_fifoops = {
...
VFS_VOP_VECTOR_REGISTER(hfs_fifoops);
#endif
