...
#include <sys/endian.h>	/* OSAddAtomic */
...
MALLOC_DEFINE(M_HFS_JOURNAL, "HFS Journal", "HFS Journal");
...
static void finish_end_thread(void *arg);
static void write_header_thread(void *arg);
static int finish_end_transaction(transaction *tr, int (*callback)(void*), void *callback_arg);
static int end_transaction(transaction *tr, int force_it, int (*callback)(void*), void *callback_arg, boolean_t drop_lock, boolean_t must_wait);
...
#define CHECK_JOURNAL(jnl) \
	...
	if (   jnl->jhdr->start <= 0					\
		...
		panic("%s:%d: jhdr start looks bad (0x%lx max size 0x%lx)\n", \
		      __FILE__, __LINE__, jnl->jhdr->start, jnl->jhdr->size); \
	}								\
	if (   jnl->jhdr->end <= 0					\
	       || jnl->jhdr->end > jnl->jhdr->size) {			\
		panic("%s:%d: jhdr end looks bad (0x%lx max size 0x%lx)\n", \
...
#define CHECK_TRANSACTION(tr) \
	...
	if (tr->journal_start < 0) {					\
		panic("%s:%d: tr journal start looks bad: 0x%lx\n", __FILE__, __LINE__, tr->journal_start); \
	}								\
	if (tr->journal_end < 0) {					\
		panic("%s:%d: tr journal end looks bad: 0x%lx\n", __FILE__, __LINE__, tr->journal_end); \
	}								\
...
do_journal_io(journal *jnl, off_t *offset, void *data, size_t len, int direction)
{
	...
	if (*offset < 0 || *offset > jnl->jhdr->size) {
        panic("jnl: do_jnl_io: bad offset 0x%lx (max 0x%lx)\n", *offset, jnl->jhdr->size);
	}
...
again:
	...
	if (curlen <= 0) {
        panic("jnl: do_jnl_io: curlen == %ld, offset 0x%lx len %zd\n", curlen, *offset, len);
	}

	if (*offset == 0 && (direction & JNL_HEADER) == 0) {
        panic("jnl: request for i/o to jnl-header without JNL_HEADER flag set! (len %ld, data %p)\n", curlen, data);
	}
	 ...
	 * as a metadata struct buf * for the layers below us that may care. If we were to
    ...
    if (bp == NULL) {
        bp = malloc(sizeof(struct buf), M_HFS_JOURNAL, M_ZERO | M_NOWAIT);
    } else {
        bzero(bp, sizeof(struct buf));
    }
    
    pbgetvp(jnl->jdev, bp); // associate buffer with vnode
    
    bp->b_flags |= B_MANAGED | B_DIRECT | B_NOCACHE;
	bp->b_bufsize = curlen;
	bp->b_bcount = curlen;
	bp->b_data = (void*)data;
    ...
    bp->b_runningbufspace = (int)bp->b_bufsize;
    bp->b_iooffset = dbtob(bp->b_blkno);
    
#if unsupported
	if ((direction & JNL_WRITE) && (jnl->flags & JOURNAL_DO_FUA_WRITES)) {
		// FreeBSD doesn't look like it supports FUA in the kernel
	}
#endif
#if notneeded
...
#endif
//	DTRACE_IO1(journal__start, struct buf *, bp);
    ...
    err = bufwait(bp);
//	DTRACE_IO1(journal__done, struct buf *, bp);
#if notneeded
...
write_journal_header(journal *jnl, int updating_start, uint32_t sequence_num)
{
	...
	int ret=0;
	off_t jhdr_offset = 0;
    struct hfsmount *hfsmp = VFSTOHFS(jnl->jdev->v_mountedhere);
...
buffer_flushed_callback(struct buf *bp)
{
	...
	//	   bp, bp->b_lblkno, buf_blkno(bp), bp->b_vp, arg);

	// snarf out the bits we want
	bufsize = (int) bp->b_bufsize;
	tr      = (transaction *)bp->b_fsprivate2;
	...
	//
    amt_flushed += atomic_fetchadd_int((volatile u_int *) &tr->num_flushed, bufsize);
	...
	if (i >= sizeof(jnl->old_start)/sizeof(jnl->old_start[0])) {
        panic("jnl: buffer_flushed: did not find tr w/start @ %ld (tr %p, jnl %p)\n",
...
update_fs_block(journal *jnl, void *block_ptr, off_t fs_block, size_t bsize)
{
	...
	ret = bread(jnl->fsdev, (daddr_t)fs_block, bsize, NOCRED, &oblock_bp);
	if (ret != 0) {
		printf("jnl: %s: update_fs_block: error reading fs block # %ld! (ret %d)\n", jnl->jdev_name, fs_block, ret);
		...
		oblock_bp = getblk(jnl->fsdev, (daddr_t)fs_block, (int)bsize, 0, 0, 0);
		if (oblock_bp == NULL) {
			printf("jnl: %s: update_fs_block: getblk() for %ld failed! failing update.\n", jnl->jdev_name, fs_block);
			return -1;
		}
	}
	...
	memcpy((char *)oblock_bp->b_data, block_ptr, bsize);
#if notneeded
	if (vfs_isswapmount(jnl->fsmount)) {
		/*
		 * if we block waiting for memory, and there is enough pressure to
...
#endif
	ret = bufwrite(oblock_bp);
#if notneeded
	if (vfs_isswapmount(jnl->fsmount) && (was_vm_privileged == FALSE))
		set_vm_privilege(FALSE);
#endif
	if (ret != 0) {
		printf("jnl: %s: update_fs_block: failed to update block %ld (ret %d)\n", jnl->jdev_name, fs_block,ret);
		return ret;
	}
...
do_overlap(journal *jnl, struct bucket **buf_ptr, int blk_index, off_t block_num, size_t size, __unused size_t offset, int32_t cksum, int *num_buckets_ptr, int *num_full_ptr)
{
	...
	while (index < *num_full_ptr && block_end > (off_t)((*buf_ptr)[index].block_num*jhdr_size)) {
		if (block_end >= (off_t)(((*buf_ptr)[index].block_num*jhdr_size + (*buf_ptr)[index].block_size))) {
			...
			if (overlap > 0) {
				if (overlap % jhdr_size != 0) {
					panic("jnl: do_overlap: overlap of %ld is not multiple of %zd\n", overlap, jhdr_size);
				}
...
restart_replay:
	...
	printf("jnl: %s: replay_journal: from: %ld to: %ld (joffset 0x%lx)\n",
	...
	while (check_past_jnl_end || jnl->jhdr->start != jnl->jhdr->end) {
		...
		if (ret != (size_t)jnl->jhdr->blhdr_size) {
			printf("jnl: %s: replay_journal: Could not read block list header block @ 0x%lx!\n", jnl->jdev_name, offset);
			bad_blocks = 1;
		...
		if (checksum != orig_checksum) {
			if (check_past_jnl_end && in_uncharted_territory) {
				...
				if (blhdr_offset != jnl->jhdr->end) {
					printf("jnl: %s: Extra txn replay stopped @ %ld / 0x%lx\n", jnl->jdev_name, blhdr_offset, blhdr_offset);
				}
			...
			printf("jnl: %s: replay_journal: bad block list header @ 0x%lx (checksum 0x%x != 0x%x)\n",
		...
		if (   (last_sequence_num != 0)
			...
			if (check_past_jnl_end) {
				check_past_jnl_end = 0;
				printf("jnl: %s: 2: extra replay stopped @ %ld / 0x%lx (seq %d < %d)\n",
				       jnl->jdev_name, blhdr_offset, blhdr_offset, blhdr->binfo[0].u.bi.b.sequence_num, last_sequence_num);
				continue;
			}

			printf("jnl: %s: txn sequence numbers out of order in txn @ %ld / %lx! (%d < %d)\n",
			       jnl->jdev_name, blhdr_offset, blhdr_offset, blhdr->binfo[0].u.bi.b.sequence_num, last_sequence_num);
			bad_blocks = 1;
			goto bad_txn_handling;
		}
		...
		if (blhdr_offset >= jnl->jhdr->end && jnl->jhdr->start <= jnl->jhdr->end) {
			if (last_sequence_num == 0) {
				check_past_jnl_end = 0;
				printf("jnl: %s: pre-sequence-num-enabled txn's - can not go further than end (%ld %ld).\n",
			...
			printf("jnl: %s: examining extra transactions starting @ %ld / 0x%lx\n", jnl->jdev_name, blhdr_offset, blhdr_offset);
		}
		...
		for (i = 1; i < blhdr->num_blocks; i++) {
			if (blhdr->binfo[i].bnum < 0 && blhdr->binfo[i].bnum != (off_t)-1) {
				printf("jnl: %s: replay_journal: bogus block number 0x%lx\n", jnl->jdev_name, blhdr->binfo[i].bnum);
				bad_blocks = 1;
				goto bad_txn_handling;
			}
		...
		for (i = 1; i < blhdr->num_blocks; i++) {
			...
			if (number == (off_t)-1) {
				...
				if (check_block_checksums) {
					...
					if (ret != (size_t)size) {
						printf("jnl: %s: replay_journal: Could not read journal entry data @ offset 0x%lx!\n", jnl->jdev_name, offset);
						bad_blocks = 1;
						goto bad_txn_handling;
					}
					...
					if (blhdr->binfo[i].u.bi.b.cksum != 0 && disk_cksum != blhdr->binfo[i].u.bi.b.cksum) {
                        printf("jnl: %s: txn starting at %ld (%ld) @ index %3d bnum %ld (%d) with disk cksum != blhdr cksum (0x%.8x 0x%.8x)\n",
		...
		bad_txn_handling:
			...
			check_past_jnl_end = 0;
			last_sequence_num = 0;
			printf("jnl: %s: restarting journal replay (%ld - %ld)!\n", jnl->jdev_name, jnl->jhdr->start, jnl->jhdr->end);
			goto restart_replay;
		}
	...
	if (jnl->jhdr->start != jnl->jhdr->end) {
		printf("jnl: %s: start %ld != end %ld.  resetting end.\n", jnl->jdev_name, jnl->jhdr->start, jnl->jhdr->end);
...
size_up_tbuffer(journal *jnl, int tbuffer_size, int phys_blksz)
{
	...
	if (def_tbuffer_size == 0) {
		uint64_t memsize = 0, retval;
		size_t l = sizeof(memsize);
		kernel_sysctlbyname(curthread, "hw.physmem", &memsize, &l, NULL, 0, &retval, 0);

        if (retval != 0) {
            panic("%s: kernel_sysctlbyname failed.", __func__);
        }
		...
		if (memsize < (256*1024*1024)) {
			...
			def_tbuffer_size = DEFAULT_TRANSACTION_BUFFER_SIZE * (int)(memsize / (256*1024*1024));
		}
	}
	...
	if (jnl->tbuffer_size > (jnl->jhdr->size / 2)) {
		jnl->tbuffer_size = (int)(jnl->jhdr->size / 2);
	}
...
get_io_info(struct vnode *devvp, size_t phys_blksz, journal *jnl, struct thread *td)
{
	...
	if (readmaxcnt == 0) {
		readmaxcnt = 128 * 1024;
	} else if (readmaxcnt > __UINT32_MAX__) {
		readmaxcnt = __UINT32_MAX__;
	}
	...
	} else if (writemaxcnt > __UINT32_MAX__) {
		writemaxcnt = __UINT32_MAX__;
	}
			   ...
			   struct mount *fsmount)
{
	...
	if (journal_size < (256*1024) || journal_size > (MAX_JOURNAL_SIZE)) {
		printf("jnl: %s: create: journal size %ld looks bogus.\n", jdev_name, journal_size);
		goto jnl_fail;
	}
	...
	if (journal_size < min_size) {
		printf("jnl: %s: create: journal size (%ld) too small given sector size of (%u)\n",
	...
	if ((journal_size % phys_blksz) != 0) {
        printf("jnl: %s: create: journal size 0x%lx is not an even multiple of block size 0x%ux\n",
		       jdev_name, journal_size, phys_blksz);
		goto jnl_fail;
	}
			 ...
			 struct mount *fsmount)
{
	...
	if (journal_size < (256*1024) || journal_size > (1024*1024*1024)) {
		printf("jnl: %s: open: journal size %ld looks bogus.\n", jdev_name, journal_size);
		goto jnl_fail;
	}
	...
	/* Reject journals that are too small given the sector size of the device */
	if (journal_size < min_size) {
		printf("jnl: %s: open: journal size (%ld) too small given sector size of (%u)\n",
				jdev_name, journal_size, phys_blksz);
		goto jnl_fail;
	}
    
	if ((journal_size % phys_blksz) != 0) {
		printf("jnl: %s: open: journal size 0x%lx is not an even multiple of block size 0x%x\n",
		       jdev_name, journal_size, phys_blksz);
		goto jnl_fail;
	}
	...
	if (   jnl->jhdr->start <= 0
		...
		printf("jnl: %s: open: jhdr start looks bad (0x%lx max size 0x%lx)\n",
...
// XXXdbg    if ((jnl->jhdr->start % jnl->jhdr->jhdr_size) != 0) {
	if ((jnl->jhdr->start % 512) != 0) {
		printf("jnl: %s: open: journal start (0x%lx) not a multiple of 512?\n",
...
//XXXdbg    if ((jnl->jhdr->end % jnl->jhdr->jhdr_size) != 0) {
	if ((jnl->jhdr->end % 512) != 0) {
		printf("jnl: %s: open: journal end (0x%lx) not a multiple of block size (0x%x)?\n",
	...
	if (flags & JOURNAL_RESET) {
		printf("jnl: %s: journal start/end pointers reset! (s 0x%lx e 0x%lx)\n",
	...
	if ((off_t)(jnl->jhdr->blhdr_size/sizeof(block_info)-1) > (jnl->jhdr->size/jnl->jhdr->jhdr_size)) {
		printf("jnl: %s: open: jhdr size and blhdr size are not compatible (0x%lx, %d, %d)\n", jdev_name, jnl->jhdr->size,
                 ...
                 size_t        min_fs_block_size)
{
	...
	if (journal_size < (256*1024) || journal_size > (MAX_JOURNAL_SIZE)) {
		printf("jnl: %s: is_clean: journal size %ld looks bogus.\n", jdev_name, journal_size);
		ret = EINVAL;
		goto jnl_fail;
	}
    
	if ((journal_size % phys_blksz) != 0) {
		printf("jnl: %s: is_clean: journal size 0x%lx is not an even multiple of block size 0x%x\n",
		       jdev_name, journal_size, phys_blksz);
		ret = EINVAL;
		goto jnl_fail;
	}
...
journal_close(journal *jnl)
{
	...
	if ((jnl->flags & JOURNAL_INVALID) == 0) {
		...
		if (*start != *end) {
            printf("jnl: %s: close: buffer flushing didn't seem to flush out all the transactions! (0x%lx - 0x%lx)\n",
	...
	while (1) {
		...
		if (counter++ == 5000) {
			...
			panic("jnl: check_free_space: buffer flushing isn't working "
			      "(jnl @ %p s %ld e %ld f %ld [active start %ld]).\n", jnl,
		...
		for (i = 0; i < sizeof(jnl->old_start)/sizeof(jnl->old_start[0]); i++) {
			...
			while (jnl->old_start[i] & 0x8000000000000000LL) {
				if (lcl_counter++ > 10000) {
					panic("jnl: check_free_space: tr starting @ 0x%lx not flushing (jnl %p).\n",
					      jnl->old_start[i], jnl);
				}
...
journal_modify_block_start(journal *jnl, struct buf *bp)
{
	...
	//   bp, bp->b_vp, bp->b_lblkno, buf_blkno(bp), bp->b_bufsize, tr->total_bytes);
	...
	if ((bp->b_bufsize % jnl->jhdr->jhdr_size) != 0) {
		...
		if (VNOP_IOCTL(jnl->jdev, HFSTOCP(hfsmp), DKIOCGETBLOCKSIZE, (caddr_t)&phys_blksz, 0)) {
			...
			if (phys_blksz < 512) {
                panic("jnl: mod block start: phys blksz %d is too small (%ld, %d)\n",
				      phys_blksz, bp->b_bufsize, jnl->jhdr->jhdr_size);
			}

			if ((bp->b_bufsize % phys_blksz) != 0) {
				bad = 1;
			} else if (phys_blksz < (uint32_t)jnl->jhdr->jhdr_size) {
				jnl->jhdr->jhdr_size = phys_blksz;
			} else {
		...
		if (bad) {
            panic("jnl: mod block start: bufsize %ld not a multiple of block size %d\n",
	...
	if (tr->total_bytes+bp->b_bufsize >= (jnl->jhdr->size - jnl->jhdr->jhdr_size)) {
        panic("jnl: transaction too big (%d >= %ld bytes, bufsize %ld, tr %p bp %p)\n",
...
#if DEBUG
	const int f = bp->b_flags;
	hfs_assert(!ISSET(f, B_DELWRI) || BUF_ISLOCKED(bp));
#endif
    // lock buffer
...
journal_modify_block_abort(journal *jnl, struct buf *bp)
{
	...
	if (jnl->flags & JOURNAL_INVALID) {
    	/* Still need to brelse(). Callers assume we consume the bp. */
    	brelse(bp);
		return EINVAL;
	}
...
int
journal_modify_block_end(journal *jnl, struct buf *bp, void (*func)(struct buf *), void *arg)
{
	int		i = 1;
	...
	for (blhdr = tr->blhdr; blhdr; prev = blhdr, blhdr = (block_list_header *)((long)blhdr->binfo[0].bnum)) {
		...
		for (i = 1; i < blhdr->num_blocks; i++) {
			...
			if (blhdr->binfo[i].bnum != (off_t)-1) {
				tbuffer_offset += (blhdr->binfo[i].u.bp)->b_bufsize;
			} else {
	...
	if (blhdr == NULL
	    ...
	    && (prev->bytes_used+bp->b_bufsize) <= (uint32_t)tr->tbuffer_size) {
		blhdr = prev;
...
//		}
		...
		bsize = (int)bp->b_bufsize;

		blhdr->binfo[i].bnum = (off_t)(bp->b_blkno);
...
#endif
		if (func) {
            if (bp->b_iodone != NULL && bp->b_iodone != func) {
                
                panic("jnl: modify_block_end: old func %p / arg %p (func %p)", bp->b_iodone, bp->b_fsprivate2, func);
            }
            bp->b_iodone = func;
            bp->b_fsprivate2 = arg;
		}
	...
	for (blhdr = tr->blhdr; blhdr; blhdr = (block_list_header *)((long)blhdr->binfo[0].bnum)) {
		...
		for (i = 1; i < blhdr->num_blocks; i++) {
			if (bp == blhdr->binfo[i].u.bp) {
				...
				// this undoes the vref() in journal_modify_block_end()
				vp = bp->b_vp;
				vrele(vp);
				...
				blhdr->binfo[i].u.bi.bsize = (int) bp->b_bufsize;
...
journal_binfo_cmp(const void *a, const void *b)
{
	...
	//
	res = (uintptr_t)((bi_a->u.bp) - (bi_b->u.bp)->b_blkno);
 ...
 */
static int
end_transaction(transaction *tr, int force_it, int (*callback)(void*), void *callback_arg, boolean_t drop_lock, boolean_t must_wait)
{
	...
	if (jnl->jhdr->end <= 0 || jnl->jhdr->end > jnl->jhdr->size) {
		panic("jnl: end_transaction: end is bogus 0x%lx (sz 0x%lx)\n",
			  jnl->jhdr->end, jnl->jhdr->size);
	}
	if (tr->delayed_header_write == TRUE) {
		 ...
		 */
        kthread_add(&write_header_thread, jnl, NULL, &thread, 0, 0, "end_transaction");
...
#undef __CAST_AWAY_QUALIFIER
	...
	for (blhdr = tr->blhdr; blhdr; blhdr = next) {
		...
		for (i = 1; i < blhdr->num_blocks; i++) {
			...
			if (blhdr->binfo[i].bnum != (off_t)-1) {
				void (*func)(struct buf *);
				void  *arg;

				bp = blhdr->binfo[i].u.bp;

				if (bp == NULL) {
					panic("jnl: inconsistent binfo (NULL bp w/bnum %ld; jnl @ %p, tr %p)\n",
				 ...
				 */
				do {
                    err = BUF_LOCK(bp, LK_EXCLUSIVE | LK_NOWAIT | LK_CANRECURSE, NULL);
				} while (err == EBUSY);
					
				if (err)
					panic("could not acquire bp %p (err %d)\n", bp, err);

				if (!BUF_ISLOCKED(bp) && (bp->b_flags & B_DELWRI) == 0) {
					if (jnl->flags & JOURNAL_CLOSE_PENDING) {
						 ...
						 * issuing a 'buf_bawrite' on a stale struct buf * pointer leads
...
//				sbp = buf_create_shadow_priv(bp, FALSE, (uintptr_t)blkptr, 0, 0);
				 ...
				 */
				memcpy((char *)bp->b_data, blkptr, bsize);
	...
	if (must_wait == TRUE)
		 ...
		 * unlock_condition
		 */
        kthread_add(&finish_end_thread, tr, 0, &thread, 0, 0, "end_transaction");
...
static int
finish_end_transaction(transaction *tr, int (*callback)(void*), void *callback_arg)
{
	int		i, amt;
	int		ret = 0;
	off_t		end;
	...
	for (blhdr = tr->blhdr; blhdr; blhdr = (block_list_header *)((long)blhdr->binfo[0].bnum)) {
		...
		bparray = hfs_malloc(blhdr->num_blocks * sizeof(struct buf *));
		tbuffer_offset = jnl->jhdr->blhdr_size;
		...
		for (i = 1; i < blhdr->num_blocks; i++) {
			...
			/*
			 * finish preparing the shadow struct buf * before 
			...
			if (blhdr->binfo[i].bnum != (off_t)-1) {
				...
				if (vp == NULL && lblkno == blkno) {
                    printf("jnl: %s: end_tr: bad news! buffer w/null vp and l/blkno = %ld/%ld.  aborting the transaction.\n",
				...
				if ((lblkno == blkno) && (vp != jnl->fsdev)) {
                    ...
                    f_offset = GetLogicalBlockSize(vp) * lblkno;

					if (VOP_BMAP(vp, lblkno, NULL, &blkno, &contig_bytes, 0) != 0) {
		...
		if (jnl->write_header_failed == FALSE)
			ret = (int) write_journal_data(jnl, &end, blhdr, amt);
		else 
			ret_val = -1;
		/*
		...
		hfs_free(bparray, blhdr->num_blocks * sizeof(struct buf *));
	...
	if (tr->journal_start == 0 || tr->journal_end == 0) {
		panic("jnl: end_transaction: bad tr journal start/end: 0x%lx 0x%lx\n",
...
lock_condition(journal *jnl, boolean_t *condition, const char *condition_name)
{
	...
	while (*condition == TRUE)
        msleep(condition, &jnl->flock.mtx, PRIBIO, condition_name, 0);
	...
	for (blhdr = tr->blhdr; blhdr; blhdr = next) {
		...
		for (i = 1; i < blhdr->num_blocks; i++) {
			...
			if (buf_shadow(tbp)) {
				sbp = tbp;
                tbp->b_iodone = NULL;
                tbp->b_fsprivate2 = NULL;
			} else {
                BUF_ASSERT_LOCKED(tbp);

				sbp = NULL;
                
				do {
					err = BUF_LOCK(tbp, LK_EXCLUSIVE | LK_NOWAIT | LK_CANRECURSE, 0);
				} while (err == EBUSY);
			...
			if (bp_vp) {
				err = bread(bp_vp,
						       ...
						       &bp);
				if (err == 0) {
					if (sbp == NULL && bp != tbp && BUF_ISLOCKED(tbp)) {
					 ...
					 * mark the struct buf * clean and make sure it's locked in the cache
					 * note: if we found a shadow, the real struct buf * needs to be relocked
					 */
                    BUF_LOCK(bp, LK_EXCLUSIVE | LK_CANRECURSE, 0);
                    CLR(bp->b_flags, B_INVAL);
					brelse(bp);
					...
					printf("jnl: %s: abort_tr: could not find block %ld for vnode!\n",
					       jnl->jdev_name, blhdr->binfo[i].bnum);
					if (bp) {
						brelse(bp);
					}
...
int journal_relocate(journal *jnl, off_t offset, off_t journal_size, int32_t tbuffer_size,
	int (*callback)(void *), void *callback_arg)
{
	int		ret;
	transaction	*tr;
	size_t i = 0;
	...
	if ((offset % jnl->jhdr->jhdr_size) != 0) {
		printf("jnl: %s: relocate: offset 0x%lx is not an even multiple of block size 0x%x\n",
		       jnl->jdev_name, offset, jnl->jhdr->jhdr_size);
		return EINVAL;
	}
	if ((journal_size % jnl->jhdr->jhdr_size) != 0) {
		printf("jnl: %s: relocate: journal size 0x%lx is not an even multiple of block size 0x%x\n",
