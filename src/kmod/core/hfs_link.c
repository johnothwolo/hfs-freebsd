...
hfs_makelink(struct hfsmount *hfsmp, struct vnode *src_vp, struct cnode *cp, 
		struct cnode *dcp, struct componentname *cnp, struct thread * ctx)
{
	...
	if (retval && newlink) {
		 ...
		 * Note that we're calling vget here, which should simply add an iocount if possible, without
		 * doing much checking.  It's safe to call this because we are protected by the cnode lock, which
		 * ensures that anyone trying to reclaim it will block until we release it.  vget will usually 
		 ...
		 * 
		 * As for the iocount we're about to add, we can't necessarily always call vput here.  
		 ...
		 * require the cnode lock and cause us to double-lock panic.  We can only call vput if we know
 ...
 *  IN struct thread *  a_context;
 */
int
hfs_vnop_link(struct vop_link_args *ap)
{
...
//	nspace_snapshot_event(vp, VTOC(vp)->c_ctime, NAMESPACE_HANDLER_LINK_CREATE, NULL);
	...
	cp->c_flag |= C_MODIFIED;
	cp->c_touch_chgtime = TRUE;
	error = hfs_makelink(hfsmp, vp, cp, tdcp, cnp, curthread);
	if (error) {
		cp->c_linkcount--;
...
hfs_unlink(struct hfsmount *hfsmp, struct vnode *dvp, struct vnode *vp, struct componentname *cnp, int skip_reserve)
{
	 ...
	 */
	cndesc.cd_flags = (vp->v_type & VDIR) ? CD_ISDIR : 0;
...
hfs_privatedir_init(struct hfsmount * hfsmp, enum privdirtype type)
{
	...
	if (error == 0) {
		if (type == FILE_HARDLINKS) {
			hfsmp->hfs_metadata_createdate = (int) priv_attrp->ca_itime;
		}
...
setfirstlink(struct hfsmount * hfsmp, cnid_t fileid, cnid_t firstlink)
{
	...
	dataptr->attrSize = (int) (1 + strlen((char *)&dataptr->attrData[0]));
...
getfirstlink(struct hfsmount * hfsmp, cnid_t fileid, cnid_t *firstlink)
{
	...
	*firstlink = (int) strtoul((char*)&dataptr->attrData[0], NULL, 10);
out:
	hfs_free(iterator, sizeof(*iterator));
