...
void
BuildCatalogKey(HFSCatalogNodeID parentID, const CatalogName *cName, boolean_t isHFSPlus, CatalogKey *key)
{
	if ( isHFSPlus )
	{
		    ...
		    CatalogKey *key)
{
    ...
    if ( name == NULL)
        ...
        nameLength = (uint32_t) strlen((const char *)name);
...
void
CopyCatalogName(const CatalogName *srcName, CatalogName *dstName, boolean_t isHFSPlus)
{
	u_int32_t	length = 0;
