...
int
cat_preflight(struct hfsmount *hfsmp, catops_t ops, cat_cookie_t *cookie, __unused struct proc* p)
{
	int lockflags = 0;
	int result;

	if (hfsmp->hfs_catalog_cp->c_lockowner != curthread)
...
void
cat_postflight(struct hfsmount *hfsmp, cat_cookie_t *cookie, __unused struct proc* p)
{
	int lockflags = 0;
                  ...
                  struct cat_desc *outdescp, struct cat_attr *attrp, struct cat_fork *forkp)
{
	...
	result =  ConvertUnicodeToUTF8Mangled((ByteCount)unicodelen, unicode,
	...
	case kHFSPlusFileRecord: {
		...
		if (state->s_rsrcfork) {
			...
			file->resourceFork.clumpSize =
					(uint32_t)howmany(forkp->cf_bytesread, blksize);
		}
		if (state->s_datafork) {
			forkp = state->s_datafork;
		...
		if (S_ISBLK(attrp->ca_mode) || S_ISCHR(attrp->ca_mode)) {
			file->bsdInfo.special.rawDevice = (uint32_t)attrp->ca_rdev;
		} 
		else {
			/* 
...
cat_makealias(struct hfsmount *hfsmp, u_int32_t inode_num, struct HFSPlusCatalogFile *crp)
{
	...
	bp = getblk(hfsmp->hfs_devvp, blkno, roundup(kHFSAliasSize, hfsmp->hfs_logical_block_size), 0, 0, 0);
	if (hfsmp->jnl) {
		journal_modify_block_start(hfsmp->jnl, bp);
	}

	/* Generate alias content */
	alias = (char *)bp->b_data;
	bzero(alias, bp->b_bufsize);
	...
	*valptr = htobe32(hfsmp->localCreateDate);
...
struct packdirentry_state {
    boolean_t       cbs_nfscookies; // if this is an nfs_dport request
    struct cookiedata *cbs_cdata;
	 ...
	 */
	struct dirent * cbs_direntry;
	struct dirent * cbs_prevdirentry;
	u_int32_t      cbs_previlinkref;
	boolean_t        cbs_hasprevdirentry;
	boolean_t        cbs_eof;
};
...
getdirentries_callback(const CatalogKey *ckp, const CatalogRecord *crp, struct packdirentry_state *state)
{
	...
	size_t uiosize = 0;
	caddr_t uioaddr;
	boolean_t stop_after_pack = false;
    struct cookiedata *cdata = state->cbs_cdata;
	...
	if (state->cbs_nfscookies && stop_after_pack) {
...
encodestr:
		...
		if (result == ENAMETOOLONG) {
			...
			result = ConvertUnicodeToUTF8Mangled(cnp->ustr.length * sizeof(UniChar),
							     cnp->ustr.unicode, (int) maxnamelen,
...
//		state->cbs_prevdirentry->d_seekoff = (state->cbs_index + 3) | ((u_int64_t)cnid << 32);
    ...
    if ((uiosize > (size_t)state->cbs_uio->uio_resid) ||
	    (ilinkref != 0 && state->cbs_nlinks == state->cbs_maxlinks)) {
		return (0);	/* stop */
	}

	if (!state->cbs_nfscookies || state->cbs_hasprevdirentry) {
		state->cbs_result = uiomove(uioaddr, (int)uiosize, state->cbs_uio);
		if (state->cbs_result == 0) {
            
            if (state->cbs_nfscookies){
                *cdata->cd_cookies = (int) state->cbs_index;
                cdata->cd_cookies++;
            }
	...
	return (state->cbs_result == 0  && state->cbs_uio->uio_resid >= SMALL_DIRENTRY_SIZE);
}
                           ...
                           struct packdirentry_state *state)
{
	...
	/* If this entry won't fit then we're done */
	if (uiosize > (size_t)uio_resid(state->cbs_uio)) {
		return (0);	/* stop */
	}
...
cat_getdirentries(struct hfsmount *hfsmp, u_int32_t entrycnt, directoryhint_t *dirhint,
                  struct cookiedata *cdata, struct uio *uio,
                  int * items, int * eofflag)
{
	FCB* fcb;
	BTreeIterator * iterator;
	CatalogKey * key;
	...
	extended = state.cbs_nfscookies = cdata->cd_cookies != NULL;
	...
	index = dirhint->dh_index + 1;
	if (extended) {
		state.cbs_direntry = (struct dirent *)((char *)iterator + sizeof(BTreeIterator));
	...
	if (state.cbs_nlinks > 0) {
		...
		for (i = 0; i < state.cbs_nlinks; ++i) {
			...
			if (uio->uio_segflg == UIO_USERSPACE) {
                ...
                bcopy(&fileid, (void*) (caddr_t) address, sizeof(fileid));
			}
		}
	}
		...
		if (type == S_IFDIR) {
			...
			*recordSize = sizeof(HFSPlusCatalogFolder);
			...
			if (type == S_IFBLK || type == S_IFCHR) {
				bsdp->special.rawDevice = (uint32_t) attrp->ca_rdev;
			} else {
				bsdp->special.linkCount = 1;
			}
	...
	int isdir, struct cat_desc *descp)
{
	...
	descp->cd_nameptr = (const u_int8_t *)vfs_addname((char *)nameptr, (u_int)utf8len, 0, 0);
...
getbsdattr(struct hfsmount *hfsmp, const struct HFSPlusCatalogFile *crp, struct cat_attr * attrp)
{
	...
	if (isDirectory) {
		...
		if (((const HFSPlusCatalogFolder *)crp)->userInfo.frFlags & htobe16(kFinderInvisibleMask))
		...
		/* Keep UF_HIDDEN bit in sync with Finder Info's invisible bit */
		if (crp->userInfo.fdFlags & htobe16(kFinderInvisibleMask))
			attrp->ca_flags |= UF_HIDDEN;
