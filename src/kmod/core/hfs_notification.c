...
void hfs_generate_volume_notifications(struct hfsmount *hfsmp) 
{
	...
	fsid.val[0] = (int) dev2udev(hfsmp->hfs_raw_dev);
    fsid.val[1] = mp->mnt_vfc->vfc_typenum;
	...
	if (state == 4 && !(hfsmp->hfs_notification_conditions & VQ_VERYLOWDISK)) {
		...
		if (backing_vp && vget(backing_vp, 0) != 0)
			backing_vp = NULL;
		hfs_unlock_mount(hfsmp);

		if (backing_vp) {
			struct statfs *sfs = &((backing_vp)->v_mount)->mnt_stat;
			printf("hfs: set VeryLowDisk: vol:%s, backingstore b_avail:%ld, tag:%d\n",
				   hfsmp->vcbVN, sfs->f_bavail, (backing_vp->v_nchash));
			vput(backing_vp);
		}
#endif
