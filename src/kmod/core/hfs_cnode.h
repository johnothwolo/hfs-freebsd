...
struct cnode {
	...
	uint64_t		magic;
#endif
    int bmap_op; // informs VOP_BMAP what operation is taking place.
};
 ...
 */
#define C_NEED_RVNODE_PUT   0x0000001  /* Need to do a vput on c_rsrc_vp after the unlock */
#define C_NEED_DVNODE_PUT   0x0000002  /* Need to do a vput on c_vp after the unlock */
...
#define C_NEED_DATA_SETSIZE 0x0001000  /* Do a vnode_pager_setsize(0) on c_rsrc_vp after the unlock */
#define C_NEED_RSRC_SETSIZE 0x0002000  /* Do a vnode_pager_setsize(0) on c_vp after the unlock */
...
int hfs_hides_rsrc(struct thread * ctx, struct cnode *cp, int skiplock);
int hfs_hides_xattr(struct thread * ctx, struct cnode *cp, const char *name, int skiplock);
#endif
...
extern int  hfs_filedone(struct vnode *vp, struct thread *td, 
 ...
 * usually be held in these cases.  This includes calls to vnode_pager_setsize
...
int hfs_set_bsd_flags(struct hfsmount *, struct cnode *,
				  u_int32_t, u_int32_t, struct thread *, int *);
