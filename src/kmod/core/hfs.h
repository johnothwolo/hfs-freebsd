...
enum {
    ...
    UF_TRACKED    = 0x00000040,    /* renames and deletes are tracked */
};

enum { MAXMNTLEN = 256 };
enum { MNT_JOURNALED = MNT_SUJ };
...
#define HFS_MAX_FILES	(__UINT32_MAX__ - kHFSFirstUserCatalogNodeID)
...
typedef struct hfsmount {
	...
	daddr_t     hfs_logical_block_count;  /* Number of logical blocks on the disk, as reported by ioctl(DKIOCGETBLOCKCOUNT) */
	 ...
	 */
	daddr_t	hfs_partition_avh_sector;	/* location of Alt VH w.r.t partition size */
	daddr_t	hfs_fs_avh_sector;	/* location of Alt VH w.r.t filesystem size */
	...
	/* vcb stuff */
	u_int8_t		 	vcbVN[MAXMNTLEN];		/* volume name in UTF-8 */
	...
	struct thread*			 hfs_global_lockowner;
	...
	union {
		 ...
		 * unfreeze. 
		 */
		struct proc* hfs_freezing_proc;
	};

	struct thread*		hfs_downgrading_thread; /* thread who's downgrading to rdonly */
    ...
    time_t	hfs_sync_req_oldest;

    /* Records the syncer thread so that we can avoid the syncer
       queing more syncs. */
    struct thread*		hfs_syncer_thread;
...
#define HFS_META_DELAY_TS \
	(struct timespec){ 0, HFS_META_DELAY * /*NSEC_PER_USEC*/ 1000ull }
...
******************************************************************************/
//int hfs_vnop_readdirattr(struct vop_readdirattr_args *);  /* in hfs_attrlist.c */
//int hfs_vnop_getattrlistbulk(struct vop_getattrlistbulk_args *);  /* in hfs_attrlist.c */

int hfs_vnop_inactive(struct vop_inactive_args *);        /* in hfs_cnode.c */
int hfs_vnop_reclaim(struct vop_reclaim_args *);          /* in hfs_cnode.c */
...
int hfs_vnop_read(struct vop_read_args *);           /* in hfs_readwrite.c */
int hfs_vnop_write(struct vop_write_args *);         /* in hfs_readwrite.c */
int hfs_vnop_ioctl(struct vop_ioctl_args *);         /* in hfs_readwrite.c */
//int hfs_vnop_select(struct vop_select_args *);       /* in hfs_readwrite.c */
int hfs_vnop_strategy(struct vop_strategy_args *);   /* in hfs_readwrite.c */
int hfs_vnop_allocate(struct vop_allocate_args *);   /* in hfs_readwrite.c */
//int hfs_vnop_pagein(struct vop_pagein_args *);       /* in hfs_readwrite.c */
//int hfs_vnop_pageout(struct vop_pageout_args *);     /* in hfs_readwrite.c */
int hfs_vnop_bwrite(struct buf *bp);       /* in hfs_readwrite.c */
//int hfs_vnop_blktooff(struct vop_blktooff_args *);   /* in hfs_readwrite.c */
//int hfs_vnop_offtoblk(struct vop_offtoblk_args *);   /* in hfs_readwrite.c */
int hfs_vnop_blockmap(struct vop_bmap_args *);   /* in hfs_readwrite.c */
int hfs_flush_invalid_ranges(struct vnode* vp);		  /* in hfs_readwrite.c */

int hfs_vnop_getxattr(struct vop_getextattr_args *);        /* in hfs_xattr.c */
int hfs_vnop_setxattr(struct vop_setextattr_args *);        /* in hfs_xattr.c */
int hfs_vnop_removexattr(struct vop_deleteextattr_args *);  /* in hfs_xattr.c */
int hfs_vnop_listxattr(struct vop_listextattr_args *);      /* in hfs_xattr.c */

#if NAMEDSTREAMS
extern int hfs_vnop_getnamedstream(struct vop_getnamedstream_args*);
extern int hfs_vnop_makenamedstream(struct vop_makenamedstream_args*);
extern int hfs_vnop_removenamedstream(struct vop_removenamedstream_args*);
#endif
...
******************************************************************************/
extern int  hfs_relocate(struct  vnode *, u_int32_t, struct ucred*, struct  thread *);
...
extern int hfs_truncate(struct vnode *, off_t, int, int, struct thread *);
...
extern int hfs_bmap(struct vnode *, daddr_t, struct vnode **, daddr_t *, unsigned int *);

extern int hfs_ubc_setsize(struct vnode* vp, off_t len, bool have_cnode_lock);


/*****************************************************************************
	Functions from hfs_resize.c 
******************************************************************************/
int hfs_extendfs(struct hfsmount *hfsmp, u_int64_t newsize, struct thread *td);
int hfs_truncatefs(struct hfsmount *hfsmp, u_int64_t newsize, struct thread *td);
...
extern struct vop_vector hfs_fifoops;

// unlike in macOS, this is a vnop in freebsd
extern int hfs_vptofh(struct vop_vptofh_args *);

extern int hfs_reload(struct mount *mp, struct thread *td, int flags);

extern int hfs_own_mount(const struct mount *mp);
...
extern int  hfs_extendfs(struct hfsmount *, u_int64_t, struct thread *);
extern int  hfs_truncatefs(struct hfsmount *, u_int64_t, struct thread *);
...
OSErr	hfs_MountHFSPlusVolume(struct hfsmount *hfsmp, HFSPlusVolumeHeader *vhp,
		off_t embeddedOffset, u_int64_t disksize, struct proc *p, void *args, struct ucred* cred);

OSErr hfs_ValidateHFSPlusVolumeHeader(struct hfsmount *hfsmp, HFSPlusVolumeHeader *vhp);

extern int hfsUnmount(struct hfsmount *hfsmp, struct thread *);
...
extern int hfs_generate_document_id(struct hfsmount *hfsmp, uint32_t *docid, struct thread *td);
...
extern void hfs_syncer(void *arg);
...
extern int hfs_flush(struct hfsmount *hfsmp, hfs_flush_mode_t mode);
...
int hfs_chown(struct vnode *vp, uid_t uid, gid_t gid, struct ucred* cred, struct proc *p);

int hfs_vnop_create(struct vop_create_args *ap);

int hfs_vnop_remove(struct vop_remove_args*);
...
extern int hfs_fsync(struct vnode *, int, hfs_fsync_mode_t, struct thread *);
 ...
 */
#define HFS_XATTR_MAXSIZE	__INT32_MAX__
...
int hfs_xattr_read(struct vnode* vp, const char *name, void *data, size_t *size);
int hfs_getxattr_internal(cnode_t *, struct vop_getextattr_args *,
                          struct hfsmount *, u_int32_t);
int hfs_xattr_write(struct vnode* vp, const char *name, const void *data, size_t size);
int hfs_setxattr_internal(struct cnode *, const void *, size_t, 
                          struct vop_setextattr_args *, struct hfsmount *, u_int32_t);
...
int hfs_first_link(hfsmount_t *hfsmp, cnode_t *cp, cnid_t *link_id);
 ...
 ******************************************************************************/
extern int hfs_get_fsinfo(struct hfsmount *hfsmp, void *a_data);
