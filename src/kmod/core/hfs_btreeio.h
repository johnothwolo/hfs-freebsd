...
extern int SetBTreeBlockSize(FileReference vp, ByteCount blockSize, 
				ItemCount minBlockCount);

extern int GetBTreeBlock(FileReference vp, u_int32_t blockNum, 
				GetBlockOptions options, BlockDescriptor *block);

extern int ReleaseBTreeBlock(FileReference vp, BlockDescPtr blockPtr, 
				ReleaseBlockOptions options);

extern int ExtendBTreeFile(FileReference vp, FSSize minEOF, FSSize maxEOF);
