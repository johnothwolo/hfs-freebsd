...
static void packcommonattr(struct attrblock *abp, struct hfsmount *hfsmp,
			...
			struct cat_attr * cap, struct thread *ctx);
...
static u_int32_t hfs_real_user_access(struct vnode* vp, struct thread * ctx);

static void get_vattr_data_for_attrs(struct attrlist *, struct vattr *,
    struct hfsmount *, struct vnode *, struct cat_desc *, struct cat_attr *,
    struct cat_fork *, struct cat_fork *, struct thread *);

static void vattr_data_for_common_attrs(struct attrlist *, struct vattr *,
    struct hfsmount *, struct vnode *, struct cat_desc *, struct cat_attr *,
    struct thread *);

static void vattr_data_for_dir_attrs(struct attrlist *, struct vattr *,
    struct hfsmount *, struct vnode *, struct cat_desc *, struct cat_attr *);

static void vattr_data_for_file_attrs(struct attrlist *, struct vattr *,
...
static int hfs_readdirattr_internal(struct vnode *, struct attrlist *,
    struct vattr *, struct uio, uint64_t, int, uint32_t *, int *, int *,
    struct thread *);
...
hfs_vnop_getattrlistbulk(ap)
	struct vop_getattrlistbulk_args /* {
 ...
 * This either fills in a vattr structure or fills in an attrbute buffer
 * Currently the difference in behaviour required for the two vnops is keyed
 * on whether the passed in vattr pointer is null or not. If the pointer
 * is null we fill in buffer passed and if it is not null we fill in the fields
 * of the vattr structure.
 */
int
hfs_readdirattr_internal(struct vnode *dvp, struct attrlist *alist,
    struct vattr *vap, struct uio uio, uint64_t options, int maxcount,
    uint32_t *newstate, int *eofflag, int *actualcount, struct thread * ctx)
{
	struct cnode *dcp;
	...
	*(eofflag) = 0;
	...
	if ((uio->uio_resid <= 0) || (uio_iovcnt(uio) > 1))
		return (EINVAL);
	...
	index = uio->uio_offset & HFS_INDEX_MASK;
	tag = uio->uio_offset & ~HFS_INDEX_MASK;
	...
	for (i = 0; i < (int)ce_list->realentries; ++i) {
		...
		if (!vap) {
			*((u_int32_t *)attrptr) = 0;
			...
			// LP64todo - fix this!
			if (uio->uio_resid < 0 ||
				currattrbufsize > (u_int32_t)uio->uio_resid) {
				break;
			} else {
				...
				*actualcount += 1;
				...
				if ((--maxcount <= 0) ||
				    // LP64todo - fix this!
				    uio->uio_resid < 0 ||
				    ((u_int32_t)uio->uio_resid < (fixedblocksize + HFS_AVERAGE_NAME_SIZE))){
					break;
				}
			}
		} else {
			size_t orig_resid = (size_t)uio->uio_resid;
			size_t resid;
	...
	struct thread * ctx)
{
	...
	boolean_t is_64_bit = proc_is64bit(ctx->td_proc);
	uid_t cuid = 1;
	int isroot = 0;
	...
	if (ATTR_CMN_OWNERID & attr) {
		...
		if (!isroot) {
			if (hfsmp->hfs_flags & HFS_UNKNOWN_PERMS)
				nuid = cuid;
...
hfs_attrblksize(struct attrlist *attrlist)
{
	...
	attrgroup_t a;
	int sizeof_timespec;
	boolean_t is_64_bit = proc_is64bit(curproc);
 ...
 */
static u_int32_t
hfs_real_user_access(struct vnode* vp, struct thread * ctx)
{
...
DerivePermissionSummary(uid_t obj_uid, gid_t obj_gid, mode_t obj_mode,
		struct mount *mp, struct ucred* cred, __unused struct proc *p)
{
	...
	if (! (VFSTOHFS(mp)->hfs_flags & HFS_UNKNOWN_PERMS)) {
		int is_member;

		if (groupmember(obj_gid, cred)) {
...
/*
 * ===========================================================================
 * Support functions for filling up a vattr structure based on attributes
 * requested.
 * ===========================================================================
 */
void
get_vattr_data_for_attrs(struct attrlist *alp, struct vattr *vap,
...
static void
copy_name_attr(struct vattr *vap, struct vnode *vp, const u_int8_t *name,
    int namelen)
{
	char *mpname;
	size_t mpnamelen;
...
static void
vattr_data_for_common_attrs( struct attrlist *alp, struct vattr *vap,
...
static void
vattr_data_for_dir_attrs(struct attrlist *alp, struct vattr *vap,
    struct hfsmount *hfsmp, struct vnode *vp, struct cat_desc * descp,
    struct cat_attr * cattrp)
{
	...
	if (ATTR_DIR_MOUNTSTATUS & attr) {
		/*
		 * There is not vattr for mount point status.
		 * XXX. Should there be ?
		 */
		u_int32_t mstatus = 0;
...
static void
vattr_data_for_file_attrs(struct attrlist *alp, struct vattr *vap,
