...
static boolean_t ExtentsAreIntegral(
	const HFSPlusExtentRecord extentRecord,
	u_int32_t	mask,
	u_int32_t	*blocksChecked,
	boolean_t		*checkedLastExtent);
	...
	size_t			*availableBytes)	// number of contiguous bytes (up to numberOfBytes)
{
	...
	temp += (daddr_t)startBlock * (daddr_t)sectorsPerBlock;
	...
	*startSector = temp;
	...
	if (availableBytes)
	{
		...
		if (tmpOff <= 0) {
            ...
            hfs_corruption_debug("MapFileBlockC: tmpOff <= 0 (%ld)\n", tmpOff);
			return EINVAL;
		}
...
static OSErr DeallocateFork(
	...
	u_int8_t			forkType,
	HFSPlusExtentRecord	catalogExtents,
	boolean_t *		recordDeleted) /* true if a record was deleted */
{
	OSErr				err;
	u_int32_t			numReleasedAllocationBlocks;
	boolean_t				releasedLastExtent;
...
should_pin_blocks(hfsmount_t *hfsmp, FCB *fcb)
{
	...
	//
	if ((fcb->ff_cp->c_vp->v_iflag) & VV_SYSTEM) {
		return 1;
	}
	...
	int64_t			*actualBytesAdded)	// number of bytes actually allocated
{
	...
	u_int32_t			nextBlock;
	u_int32_t			startBlock;
	boolean_t				allOrNothing;
	boolean_t				forceContig;
	boolean_t				wantContig;
	boolean_t				useMetaZone;
	boolean_t				needsFlush;
	int					allowFlushTxns;
	...
	*actualBytesAdded = 0;
	...
	if (err == noErr) {
		...
		fcb->ff_blocks = (uint32_t) peof / volumeBlockSize;
		...
		if (availbytes <= 0) {
			...
			if (wantContig && (availbytes < bytesToAdd)) {
				...
				err = BlockAllocate(
						  vcb,
						  startBlock,
						  howmany((uint32_t) MIN(bytesToAdd, availbytes), volumeBlockSize),
						  howmany((uint32_t) MIN(maximumBytes, availbytes), volumeBlockSize),
						  ba_flags,
						  &actualStartBlock,
						  &actualNumBlocks);
			}
...
OSErr TruncateFileC (
	...
	boolean_t			truncateToExtent)	// if true, truncate to end of extent containing newPEOF

{
	...
	int64_t             temp64;
	u_int8_t			forkType;
	boolean_t				extentChanged;	// true if we actually changed an extent
	boolean_t				recordDeleted;	// true if an extent record got deleted
	...
	//
	nextBlock = (uint32_t) howmany(peof, vcb->blockSize);	// number of allocation blocks to remain in file
	 ...
	 */
	numBlocks = (uint32_t) (peof / vcb->blockSize);
	if (!deleted) {
