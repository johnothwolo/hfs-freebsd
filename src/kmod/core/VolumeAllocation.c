...
Internal routines:
	...
	ReleaseBitmapRange
					Release and invalidate a struct buf * corresponding to the bitmap
...
static boolean_t add_free_extent_cache(struct hfsmount *hfsmp, u_int32_t startBlock, u_int32_t blockCount);
...
static int get_more_bits(bitmap_context_t *bitmap_ctx);
static int bit_count_set(void *bitmap, int start, int end);
static int bit_count_clr(void *bitmap, int start, int end);
static int hfs_bit_count(bitmap_context_t *bitmap_ctx, int (*fn)(void *, int ,int), uint32_t *bit_count);
static int hfs_bit_count_set(bitmap_context_t *bitmap_ctx, uint32_t *count);
static int hfs_bit_count_clr(bitmap_context_t *bitmap_ctx, uint32_t *count);
static int update_summary_table(bitmap_context_t *bitmap_ctx, uint32_t start, uint32_t count, bool set);
...
static void hfs_unmap_free_extent(struct hfsmount *hfsmp, u_int32_t startingBlock, u_int32_t numBlocks)
{
	...
	if (hfsmp->jnl != NULL) {
		...
		if ((offset >= device_sz) || ((offset + length) > device_sz)) {
            printf("hfs_unmap_free_ext: ignoring trim vol=%s @ off %lu len %ld \n", hfsmp->vcbVN, offset, length);
			err = EINVAL;
		}
...
static int hfs_issue_unmap (struct hfsmount *hfsmp, struct jnl_trim_list *list) 
{
	...
	if (list->extent_count > 0 && list->extents != NULL) {
		...
		error = VNOP_IOCTL(hfsmp->hfs_devvp, HFSTOCP(hfsmp), DKIOCUNMAP, (caddr_t)&unmap, 0);
...
hfs_trim_callback(void *arg, uint32_t extent_count, const dk_extent_t *extents)
{
	...
	for (i=0; i<extent_count; ++i) {
		...
		startBlock = (u_int) (extents[i].offset - hfsmp->hfsPlusIOPosOffset) / hfsmp->blockSize;
		numBlocks = (u_int) extents[i].length / hfsmp->blockSize;
...
/*
 ...
 ;				cause struct buf * collision and potential data corruption.
 ;  
 ; Input Arguments:
 ;	hfsmp			- The volume containing the allocation blocks.
 ;________________________________________________________________________________
								 ...
								 int list)
{
	...
	if (rl_len(range) > 0) {
		if (list == HFS_TENTATIVE_BLOCKS)
			 ...
			 */
			dk_extent_t extent = {
				.offset = (hfs_blk_to_bytes((u_int)range->rl_start, hfsmp->blockSize)
						   + hfsmp->hfsPlusIOPosOffset),
				.length = hfs_blk_to_bytes((u_int)rl_len(range), hfsmp->blockSize)
			};
			dk_unmap_t unmap = {
				.extents = &extent,
				.extentsCount = 1,
			};
			VNOP_IOCTL(hfsmp->hfs_devvp, HFSTOCP(hfsmp), DKIOCUNMAP, (caddr_t)&unmap, 0);
			hfs_assert(hfsmp->lockedBlocks >= rl_len(range));
			hfsmp->lockedBlocks -= rl_len(range);
		}
		hfs_release_summary(hfsmp, (u_int)range->rl_start, (u_int)rl_len(range));
		add_free_extent_cache(hfsmp, (u_int)range->rl_start, (u_int)rl_len(range));
	}
...
int hfs_block_alloc(hfsmount_t *hfsmp,
						HFSPlusExtentDescriptor *extent,
						hfs_block_alloc_flags_t flags,
						hfs_alloc_extra_args_t *ap)
{
...
// MARK:UNUSED: Currently unused:
// MARK: HFS_ALLOC_TENTATIVE, x, HFS_ALLOC_LOCKED, HFS_ALLOC_COMMIT, HFS_ALLOC_TRY_HARD
// MARK: HFS_ALLOC_ROLL_BACK (because hfs_ext_replace() is unused).
						  ...
						  hfs_alloc_extra_args_t *ap)
{
	...
	OSErr			err = 0;
	boolean_t			updateAllocPtr = false;		//	true if nextAllocation needs to be updated
	boolean_t forceContiguous = false;
	boolean_t forceFlush;
	...
	if (ISSET(flags, HFS_ALLOC_COMMIT)) {
		extent->startBlock = (u_int)(*ap->reservation_in)->rl_start;
		extent->blockCount = (u_int)rl_len(*ap->reservation_in);
		goto mark_allocated;
	}
    ...
    // MARK:UNSED: This flag is not set anywhere when the function is called.
    // MARK:UNSED: Furthermore, the reservation aren't set either (they're all null)
	if (ISSET(flags, HFS_ALLOC_USE_TENTATIVE)) {
		...
		if (range && range->rl_start != -1) {
			 ...
			 */
			uint32_t count = min(min(maxBlocks, (u_int)rl_len(range)), freeBlocks);

			if (count >= minBlocks) {
				extent->startBlock = (u_int)range->rl_start;
	...
	//
	if (forceContiguous) {
		err = BlockFindContig(hfsmp, startingBlock, minBlocks, maxBlocks, flags,
                              member_nowarn(HFSPlusExtentDescriptor, u_int, extent, startBlock),
                              member_nowarn(HFSPlusExtentDescriptor, u_int, extent, blockCount));
		/*
		 ...
		 */
		err = BlockFindKnown(hfsmp, maxBlocks,
                             member_nowarn(HFSPlusExtentDescriptor, u_int, extent, startBlock),
                             member_nowarn(HFSPlusExtentDescriptor, u_int, extent, blockCount));
		...
		if (err == dskFulErr) {
			 ...
			 */
			err = BlockFindAny(hfsmp, startingBlock, hfsmp->allocLimit, maxBlocks, flags, true,
					member_nowarn(HFSPlusExtentDescriptor, u_int, extent, startBlock),
                    member_nowarn(HFSPlusExtentDescriptor, u_int, extent, blockCount));
		}
		if (err == dskFulErr) {
			 ...
			 */
			if (hfsmp->hfs_flags & HFS_SUMMARY_TABLE) {
				err = BlockFindAny(hfsmp, 1, hfsmp->allocLimit, maxBlocks, flags, false,
                                   member_nowarn(HFSPlusExtentDescriptor, u_int, extent, startBlock),
                                   member_nowarn(HFSPlusExtentDescriptor, u_int, extent, blockCount));
			}
			else {
				err = BlockFindAny(hfsmp, 1, startingBlock, maxBlocks, flags, false,
                                   member_nowarn(HFSPlusExtentDescriptor, u_int, extent, startBlock),
                                   member_nowarn(HFSPlusExtentDescriptor, u_int, extent, blockCount));
			}	
...
mark_allocated:
	...
	if (ISSET(hfsmp->hfs_flags, HFS_CS) && extent->blockCount != 0
		...
		if (ISSET(flags, HFS_ALLOC_FAST_DEV)) {
			...
			int err2 = VNOP_IOCTL(hfsmp->hfs_devvp, HFSTOCP(hfsmp), _DKIOCCSMAP, (caddr_t)&cm, 0);
			...
			if (err2 || cm.cm_bytes_mapped < cm.cm_extent.length) {
                printf("hfs: _DKIOCCSMAP error: %d, bytes_mapped: %lu\n",
				...
				switch (rl_overlap(range, start, end)) {
					case RL_OVERLAPCONTAINSRANGE:
						...
						if (start - range->rl_start > range->rl_end - end) {
							...
							hfs_release_summary(hfsmp, end + 1, (u_int)range->rl_end - end);
							const uint32_t old_end = (u_int)range->rl_end;
							range->rl_end = start - 1;
							add_free_extent_cache(hfsmp, end + 1, old_end - end);
						} else {
							// Discard the head
							hfsmp->tentativeBlocks -= end + 1 - range->rl_start;
							hfs_release_summary(hfsmp, (u_int)range->rl_start,
												start - (u_int)range->rl_start);
							const uint32_t old_start = (u_int)range->rl_start;
...
#define LEFT_MASK(bit)	htobe64(0xffffffffffffffffull << (64 - bit))
#define RIGHT_MASK(bit)	htobe64(0xffffffffffffffffull >> bit)
...
static struct buf * process_reservations(hfsmount_t *hfsmp, struct buf * bp, off_t offset,
								  hfs_block_alloc_flags_t flags,
								  bool always_copy)
{
	bool taken_copy = false;
	void *buffer = (void *)bp->b_data;
	const uint32_t nbytes = (u_int) bp->b_bcount;
	...
	for (int i = (ISSET(flags, HFS_ALLOC_IGNORE_TENTATIVE)
		...
		TAILQ_FOREACH(entry, &hfsmp->hfs_reserved_ranges[i], rl_link) {
			 ...
			 * buffer in that case.
			 */
			if (!taken_copy && (always_copy || BUF_ISLOCKED(bp))) {
                struct buf * new_bp = buf_copy(bp);
				brelse(bp);
				bp = new_bp;
			...
			case RL_MATCHINGOVERLAP:
				...
				return bp;
			case RL_OVERLAPISCONTAINED:
				a = (u_int) entry->rl_start;
				b = (u_int) entry->rl_end;
				break;
			case RL_OVERLAPSTARTSBEFORE:
				a = (u_int) offset;
				b = (u_int) entry->rl_end;
			...
			hfs_assert(a < bp->b_bcount * 8);
			hfs_assert(b < bp->b_bcount * 8);
			hfs_assert(b >= a);
							 ...
							 hfs_block_alloc_flags_t flags)
{
	...
	block = (daddr_t)(bit / (blockSize * kBitsPerByte));
	...
	err = (int)bread(vp, block, blockSize, NOCRED, &bp);
	...
	if (bp) {
		if (err) {
			...
			*buffer = NULL;
            ...
            bp->b_fsprivate1 = (void *)(uintptr_t)flags;

			*blockRef = (uintptr_t)bp;
			*buffer = (u_int32_t *)bp->b_data;
		}
	} else
		hfs_assert(err);
		...
		uint32_t iosize, uint32_t **buffer, struct buf **blockRef)
{
	 ...
	 */
	block = (daddr_t)(offset / hfsmp->vcbVBMIOSize);

	err = (int) bread(vp, block, iosize, NOCRED, &bp);
		...
		boolean_t			dirty)
{
	...
	if (bp) {
		if (dirty) {
			hfs_block_alloc_flags_t flags = (u_int)((uintptr_t)bp->b_fsprivate1);
								...
								hfs_block_alloc_flags_t flags)
{
	...
	if (extent->startBlock > 0 && extent->startBlock < hfsmp->allocLimit
		...
		err = BlockFindContiguous(hfsmp, extent->startBlock, extent->startBlock + max_blocks,
								  ...
								  member_nowarn(HFSPlusExtentDescriptor, u_int, extent, startBlock),
                                  member_nowarn(HFSPlusExtentDescriptor, u_int, extent, blockCount), flags);

		if (err != dskFulErr)
			return err;
	}

	err = BlockFindKnown(hfsmp, max_blocks,
                         member_nowarn(HFSPlusExtentDescriptor, u_int, extent, startBlock),
                         member_nowarn(HFSPlusExtentDescriptor, u_int, extent, blockCount));
		...
		u_int32_t		*actualNumBlocks)
{
	...
	boolean_t useMetaZone = (flags & HFS_ALLOC_METAZONE);
		...
		u_int32_t		*actualNumBlocks)
{
	...
	struct hfsmount *hfsmp = VCBTOHFS(vcb);
	boolean_t useMetaZone = (flags & HFS_ALLOC_METAZONE);
	boolean_t forceFlush = (flags & HFS_ALLOC_FLUSHTXN);
		...
		boolean_t 		do_validate)
{
	...
	uint32_t min_unmap = 0, max_unmap = __UINT32_MAX__;
...
FoundUsed:
		...
		if (ISSET(flags, HFS_ALLOC_IGNORE_TENTATIVE)) {
			...
			firstBlock = (u_int) free_extent.rl_start;
			foundBlocks = (u_int) rl_len(&free_extent);
		}
...
hfs_isallocated_internal(struct hfsmount *hfsmp, u_int32_t startingBlock, 
		u_int32_t numBlocks, boolean_t stop_on_first, u_int32_t *allocCount)
{
	u_int32_t  *currentWord;   // Pointer to current word within bitmap block
	u_int32_t  wordsLeft;      // Number of words left in this bitmap block
...
static int hfs_alloc_scan_range(struct hfsmount *hfsmp, u_int32_t startbit, 
	 ...
	 */
    completed_size = (u_int) blockRef->b_bcount;
...
static int get_more_bits(bitmap_context_t *bitmap_ctx)
{
	...
	if (tsleep(&unused_sleep_event, PINOD | PCATCH, "hfs_fsinfo", 0) == EINTR) {
		return EINTR;
	}
	...
	if (lock_elapsed >= HFS_FSINFO_MAX_LOCKHELD_TIME) {
		 ...
		 * at the same time, it can result in multiple struct buf * with
	 ...
	 * bytes though, so clip our ending bit to what we actually read in.
	 */
	last_bitmap_block = start_bit + ((u_int)bitmap_ctx->bp->b_bcount) * kBitsPerByte;
...
static int bit_count_set(void *bitmap, int start, int end)
{
	...
	if (end_bit) {
		x = ~be64toh(*p) | BIT_RIGHT_MASK(end_bit);
		count += clzll(x);
	}
...
int hfs_find_free_extents(struct hfsmount *hfsmp,
