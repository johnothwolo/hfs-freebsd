...
static int CheckAccess(ExtendedVCB *vcb, u_long searchBits, CatalogKey *key, struct thread *ctx);

static int InsertMatch(struct hfsmount *hfsmp, struct uio a_uio, CatalogRecord *rec,
			CatalogKey *key, struct attrlist *returnAttrList,
			void *attributesBuffer, void *variableBuffer,
			uint32_t * nummatches );

static boolean_t CompareRange(u_long val, u_long low, u_long high);
static boolean_t CompareWideRange(u_int64_t val, u_int64_t low, u_int64_t high);
...
static int
CheckAccess(ExtendedVCB *theVCBPtr, u_long searchBits, CatalogKey *theKeyPtr, struct thread *ctx)
{
	boolean_t				isHFSPlus;
	int					myErr;
	int					myResult; 	
...
CheckCriteria(	ExtendedVCB *vcb, 
				...
				struct thread *ctx)
{
	boolean_t matched, atleastone;
	boolean_t isHFSPlus;
	attrgroup_t searchAttributes;
	struct cat_attr c_attr;
	struct cat_fork datafork;
	struct cat_fork rsrcfork;
	int force_case_sensitivity = proc_is_forcing_hfs_case_sensitivity(ctx->td_proc);
 ...
 */
static int
InsertMatch(struct hfsmount *hfsmp, struct uio a_uio, CatalogRecord *rec,
            CatalogKey *key, struct attrlist *returnAttrList,
            void *attributesBuffer, void *variableBuffer, uint32_t * nummatches)
{
	...
	attrblk.ab_context = curthread;

	hfs_packattrblk(&attrblk, hfsmp, NULL, &c_desc, &c_attr, &datafork, &rsrcfork, curthread);
		...
		searchinfospec_t *searchInfo, void *attributeBuffer, int firstblock)
{
    ...
    is_64_bit = proc_is64bit(curproc);
