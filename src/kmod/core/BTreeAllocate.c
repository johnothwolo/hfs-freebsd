...
static int	GetMapNode (BTreeControlBlockPtr	  btreePtr,
...
int	AllocateNode (BTreeControlBlockPtr		btreePtr, u_int32_t	*nodeNum)
{
	int		 err;
...
int	FreeNode (BTreeControlBlockPtr		btreePtr, u_int32_t	nodeNum)
{
	int		 err;
	BlockDescriptor	 node;
	u_int32_t		 nodeIndex;
...
static
int	GetMapNode (BTreeControlBlockPtr	  btreePtr,
						BlockDescriptor			 *nodePtr,
						u_int16_t				**mapPtr, 			
						u_int16_t				 *mapSize )
{
...
u_int32_t		CalcMapBits	(BTreeControlBlockPtr	 btreePtr)
{
	...
	mapBits		= (uint32_t) M_HeaderMapRecordSize (btreePtr->nodeSize) << 3;
		...
		while (size--)
		{
			if (*pos != 0xFFFF)			/* Anything free in this word? */
			{
				     ...
				     ++bitNumber, mask >>= 1)
				{
					 ...
					 */
                    bp = getblk(vp, nodeNumber + bitNumber, btreePtr->nodeSize, 0, 0, 0);
					if (bp == NULL)
					{
