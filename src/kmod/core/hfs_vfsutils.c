		...
		__unused struct proc *p)
{
	...
	error = hfs_getnewvnode(hfsmp, NULL, NULL, &cndesc, gnv_dfl, &cnattr, &fork,
...
OSErr hfs_MountHFSPlusVolume(struct hfsmount *hfsmp, HFSPlusVolumeHeader *vhp,
	off_t embeddedOffset, u_int64_t disksize, __unused struct proc *p, void *args, struct ucred* cred)
{
	...
	vcb->hfsPlusIOPosOffset	= (u_int) embeddedOffset;
	 ...
	 */
	hfsmp->hfs_logBlockSize = BestBlockSizeFit(vcb->blockSize, MAXBSIZE, hfsmp->hfs_logical_block_size);
	vcb->vcbVBMIOSize = min(vcb->blockSize, MAXPHYS);
	...
	spare_sectors = hfsmp->hfs_logical_block_count -
	                (((daddr_t)vcb->totalBlocks * blockSize) /
	                   hfsmp->hfs_logical_block_size);
	...
	if (spare_sectors > (daddr_t)(blockSize / hfsmp->hfs_logical_block_size)) {
		/* 
		...
		hfsmp->hfs_fs_avh_sector = (hfsmp->hfsPlusIOPosOffset / hfsmp->hfs_logical_block_size) +
					   HFS_ALT_SECTOR(hfsmp->hfs_logical_block_size,
						(((daddr_t)vcb->totalBlocks * blockSize) / hfsmp->hfs_logical_block_size));
	} 
	else {
	...
	if (hfs_resize_debug) {
        printf ("hfs_MountHFSPlusVolume: partition_avh_sector=%ld, fs_avh_sector=%ld\n",
	...
	retval = hfs_getnewvnode(hfsmp, NULL, NULL, &cndesc, gnv_dfl, &cnattr, &cfork,
	                         &hfsmp->hfs_extents_vp, &newvnode_flags);
	if (retval)
	{
		if (HFS_MOUNT_DEBUG) {
...
//	}
	...
	/*
	 * We have to ensure that we can proceed to scan the bitmap allocation
	 * file asynchronously. If the catalog file is fragmented such that it
	 * has overflow extents and the volume needs a journal transaction we
	 * cannot scan the bitmap asynchronously. Doing so will cause the mount
	 * thread to block on the journal transaction bitmap lock, while the scan
	 * thread (which holds the bitmap lock) exclusively performs disk I/O
     * (to issue TRIMS to unallocated ranges and build summary table). The
	 * duration of the mount thread block depends on the size of
	 ...
	 *
	 * Please NOTE: Currently this timeout only seems to happen for non SSD
	...
	if (async_bitmap_scan) {
        struct thread *allocator_scanner;
	...
	if (   (vcb->vcbAtrb & kHFSVolumeJournaledMask)
		...
		if (retval != 0) {
			...
			if (!(hfsmp->hfs_flags & HFS_READ_ONLY)) {
				...
				mdb_offset = (daddr_t)((embeddedOffset / blockSize) + HFS_PRI_SECTOR(blockSize));
				...
				if (retval == 0) {
					jvhp = (HFSPlusVolumeHeader *)(bp->b_data + HFS_PRI_OFFSET(hfsmp->hfs_physical_block_size));
            ...
            hfsmp->hfs_mp->mnt_flag |= MNT_SUJ; // idek why MNT_SUJ in a mntopt, it has no special treatment.
		}
...
static int
_pin_metafile(struct hfsmount *hfsmp, struct vnode* vp)
{
	int err;
...
hfs_pin_fs_metadata(struct hfsmount *hfsmp)
{
	...
	if ((hfsmp->hfs_mp)->mnt_flag & MNT_SUJ) {
...
int
hfsUnmount( register struct hfsmount *hfsmp, __unused struct thread *td)
{
...
boolean_t hfs_is_frozen(struct hfsmount *hfsmp)
{
	return (hfsmp->hfs_freeze_state == HFS_FROZEN
			|| (hfsmp->hfs_freeze_state == HFS_FREEZING
				&& curthread != hfsmp->hfs_freezing_thread));
}
...
frozen:
		...
		struct timespec ts = { 0, 500 * /*NSEC_PER_MSEC*/1000000ull };

		while (hfs_is_frozen(hfsmp)) {
			if (hfsmp->hfs_freeze_state == HFS_FROZEN
				&& (hfsmp->hfs_freezing_proc->p_flag & P_WEXIT)) {
				hfs_thaw_locked(hfsmp);
				break;
			}

            msleep(&hfsmp->hfs_freeze_state, &hfsmp->hfs_mutex, PWAIT, "hfs_lock_global (frozen)", ts.tv_nsec);
		}
		hfs_unlock_mount(hfsmp);
	}
...
void hfs_unlock_mount (struct hfsmount *hfsmp) {
	mtx_unlock (&(hfsmp->hfs_mutex));
}

const char* systemfile_names[SFL_VM_PRIV+1] = {
    "INVALID_SYSTEMFILE",
    [SFL_CATALOG]       =       "CATALOG_SYSTEMFILE",
    [SFL_EXTENTS]       =       "EXTENTS_SYSTEMFILE",
    "INVALID_SYSTEMFILE",
    [SFL_BITMAP]        =       "BITMAP_SYSTEMFILE",
    "INVALID_SYSTEMFILE",
    [SFL_ATTRIBUTE]     =       "ATTRIBUTE_SYSTEMFILE",
    "INVALID_SYSTEMFILE",
    [SFL_STARTUP]       =       "STARTUP_SYSTEMFILE",
    "INVALID_SYSTEMFILE",
    [SFL_VM_PRIV]       =       "VM_PRIV_SYSTEMFILE",
};
 ...
 */
int
_hfs_systemfile_lock(struct hfsmount *hfsmp, int flags, enum hfs_locktype locktype, const char *func, const char *file, int line)
{
    printf("[%p]:hfs_systemfile_lock(%s): called by %s():%d\n", curthread, systemfile_names[(flags & SFL_VALIDMASK)], func, line);
	...
	if (flags & SFL_CATALOG) {
		if (hfsmp->hfs_catalog_cp
			&& hfsmp->hfs_catalog_cp->c_lockowner != curthread) {
#ifdef HFS_CHECK_LOCK_ORDER
			if (hfsmp->hfs_attribute_cp && hfsmp->hfs_attribute_cp->c_lockowner == curthread) {
				panic("hfs_systemfile_lock: bad lock order (Attributes before Catalog)");
			}
			if (hfsmp->hfs_startup_cp && hfsmp->hfs_startup_cp->c_lockowner == curthread) {
				panic("hfs_systemfile_lock: bad lock order (Startup before Catalog)");
			}
			if (hfsmp-> hfs_extents_cp && hfsmp->hfs_extents_cp->c_lockowner == curthread) {
				panic("hfs_systemfile_lock: bad lock order (Extents before Catalog)");
			}
#endif /* HFS_CHECK_LOCK_ORDER */

			(void) _hfs_lock(hfsmp->hfs_catalog_cp, locktype, HFS_LOCK_DEFAULT, file, line);
			/*
	...
	if (flags & SFL_ATTRIBUTE) {
		if (hfsmp->hfs_attribute_cp
			&& hfsmp->hfs_attribute_cp->c_lockowner != curthread) {
#ifdef HFS_CHECK_LOCK_ORDER
			if (hfsmp->hfs_startup_cp && hfsmp->hfs_startup_cp->c_lockowner == curthread) {
			...
			(void) _hfs_lock(hfsmp->hfs_attribute_cp, locktype, HFS_LOCK_DEFAULT, file, line);
			/*
	...
	if (flags & SFL_STARTUP) {
		if (hfsmp->hfs_startup_cp
			&& hfsmp->hfs_startup_cp->c_lockowner != curthread) {
#ifdef HFS_CHECK_LOCK_ORDER
			if (hfsmp-> hfs_extents_cp && hfsmp->hfs_extents_cp->c_lockowner == curthread) {
			...
			(void) _hfs_lock(hfsmp->hfs_startup_cp, locktype, HFS_LOCK_DEFAULT, file, line);
			/*
	...
	if (flags & (SFL_BITMAP | SFL_EXTENTS)) {
		if (hfsmp->hfs_allocation_cp) {
            (void) _hfs_lock(hfsmp->hfs_allocation_cp, HFS_EXCLUSIVE_LOCK, HFS_LOCK_DEFAULT, file, line);
			/* 
	...
	if (flags & SFL_EXTENTS) {
		...
		if (hfsmp->hfs_extents_cp) {
			(void) _hfs_lock(hfsmp->hfs_extents_cp, HFS_EXCLUSIVE_LOCK, HFS_LOCK_DEFAULT, file, line);
#if notneeded
            if (vfs_isswapmount(hfsmp->hfs_mp->mnt_stat.f_type = MNT_SWAP)) {
				/*
 ...
 */
void
_hfs_systemfile_unlock(struct hfsmount *hfsmp, int flags, const char *func, const char *file, int line)
{
	if (!flags)
		return;
	...
	if (hfsmp->jnl == NULL) {
		microuptime(&tv);
		lastfsync = (unsigned) tv.tv_sec;
	}
    
    printf("[%p]:hfs_systemfile_unlock(%s): called by %s():%d\n", curthread, systemfile_names[(flags & SFL_VALIDMASK)], func, line);
    
	if (flags & SFL_STARTUP && hfsmp->hfs_startup_cp) {
		_hfs_unlock(hfsmp->hfs_startup_cp, file, line);
	}
	if (flags & SFL_ATTRIBUTE && hfsmp->hfs_attribute_cp) {
		if (hfsmp->jnl == NULL) {
			BTGetLastSync((FCB*)VTOF(hfsmp->hfs_attribute_vp), &lastfsync);
			numOfLockedBuffs = buf_dirty_count_severe();
		...
		_hfs_unlock(hfsmp->hfs_attribute_cp, file, line);
	}
	if (flags & SFL_CATALOG && hfsmp->hfs_catalog_cp) {
		...
		_hfs_unlock(hfsmp->hfs_catalog_cp, file, line);
	}
	if (flags & SFL_BITMAP && hfsmp->hfs_allocation_cp) {
		_hfs_unlock(hfsmp->hfs_allocation_cp, file, line);
	}
	if (flags & SFL_EXTENTS && hfsmp->hfs_extents_cp) {
		...
		_hfs_unlock(hfsmp->hfs_extents_cp, file, line);
...
void RequireFileLock(FileReference vp, int shareable)
{
	...
	locked = VTOC(vp)->c_lockowner == curthread;
...
hfs_owner_rights(struct hfsmount *hfsmp, uid_t cnode_uid, struct ucred* cred,
                 struct thread *td, int invokesuperuserstatus)
{
    if ((cred->cr_uid == cnode_uid) ||                                    /* [1a] */
	    (cnode_uid == UNKNOWNUID) ||  									  /* [1b] */
	    (hfsmp->hfs_ignore_permissions &&          /* [2] */
	      ((cred->cr_uid == hfsmp->hfs_uid) ||                            /* [2a] */
	        (hfsmp->hfs_uid == UNKNOWNUID))) ||                           /* [2b] */
	    (invokesuperuserstatus && (vfs_suser(HFSTOVFS(hfsmp), td) == 0))) {    /* [3] */
		return (0);
	} else {	
		trace_return (EPERM);
	}
...
{
	struct statfs *vfsp;  /* 272 bytes */
	uint64_t vfreeblks;
	struct timeval now;

	hfs_lock_mount(hfsmp);

	struct vnode* backing_vp = hfsmp->hfs_backingvp;
	if (!backing_vp) {
		hfs_unlock_mount(hfsmp);
		return false;
	}
	...
	struct mount* backingfs_mp = backing_vp->v_mount;

	microtime(&now);
	if ((now.tv_sec - hfsmp->hfs_last_backingstatfs) >= 1) {
//		vfs_update_vfsstat(backingfs_mp, vfs_context_kernel(), VFS_KERNEL_EVENT);
		hfsmp->hfs_last_backingstatfs = (u_int) now.tv_sec;
	}

	if (!(vfsp = &(backingfs_mp)->mnt_stat)) {
		vput(backing_vp);
		return false;
	}
...
hfs_freeblks(struct hfsmount * hfsmp, int wantreserve)
{
	...
	if (hfs_get_backing_free_blks(hfsmp, &vfreeblks))
		freeblks = MIN(freeblks, (unsigned int) vfreeblks);
...
hfs_namecmp(const u_int8_t *str1, size_t len1, const u_int8_t *str2, size_t len2)
{
	...
	cmp = FastUnicodeCompare(ustr1, (ItemCount) ulen1>>1, ustr2, (ItemCount) ulen2>>1);
out:
...
journal_open_cb(const char *bsd_dev_name, const char *uuid_str, void *arg)
{
    ...
    struct nameidata jnd;
	char bsd_name[NAME_MAX];
	int error, flag;

	strlcpy(&bsd_name[0], "/dev/", sizeof(bsd_name));
	strlcpy(&bsd_name[5], bsd_dev_name, sizeof(bsd_name)-5);

    NDINIT(&jnd, LOOKUP, NOFOLLOW, UIO_SYSSPACE, bsd_name, td);
    ...
    ji->jvp = jnd.ni_vp; // assign jnl vnode
    
    error = vn_open(&jnd, &flag, FREAD | FWRITE, NULL);
	if (error) {
		ji->jvp = NULL;
		return 1;
	}
    
    NDFREE(&jnd, NDF_ONLY_PNBUF);
    
    // setup gvfs for external jnl devices
    g_topology_lock();
    error = g_vfs_open(ji->jvp, &ji->jcp, "hfs_journal_dev", 1);
    g_topology_unlock();
    ...
    if (error != 0){
        ...
        g_vfs_close(ji->jcp);
        g_topology_unlock();
        vn_close(ji->jvp, FREAD|FWRITE, td->td_ucred, td); // error?
        vput(ji->jvp);
        ji->jvp = NULL;
    }
    
    ji->jbo = &ji->jvp->v_bufobj;
    ji->jbo->bo_private = ji->jcp;
    ji->jbo->bo_ops = g_vfs_bufops;
	...
	if (ji->need_clean && ji->desired_uuid && ji->desired_uuid[0] == '\0') {
		error = journal_is_clean(ji->jvp, ji->jcp, 0, ji->jsize, (void *)1, ji->blksize);
		if (error == EBUSY) {
            vn_close(ji->jvp, FWRITE | FREAD, td->td_ucred, td);
			vput(ji->jvp);
			ji->jvp = NULL;
			return 1;    // keep iterating
...
static struct vnode*
open_journal_dev(struct mount* mp,
...
void hfs_close_jvp(hfsmount_t *hfsmp, struct thread *td)
{
...
hfs_early_journal_init(struct hfsmount *hfsmp, HFSPlusVolumeHeader *vhp,
					   void *_args, off_t embeddedOffset, daddr_t mdb_offset,
					   HFSMasterDirectoryBlock *mdbp, struct ucred* cred)
{
	...
	dev_name = devtoname(devvp->v_rdev);
	...
	retval = (int)bread(devvp,
						(daddr_t)((embeddedOffset/blksize) + 
	...
	if (jib_flags & kJIJournalInFSMask) {
	    ...
	    if (hfsmp->jvp == NULL) {
		    ...
		    if (hfs_get_platform_serial_number(&jibp->machine_serial_num[0], sizeof(jibp->machine_serial_num)) != 0) {
	...
	hfsmp->jnl_start = (u_int) jib_offset / SWAP_BE32(vhp->blockSize);
	hfsmp->jnl_size  = (u_int) jib_size;
	...
	if (jib_flags & kJIJournalNeedInitMask) {
		...
		if (hfsmp->jnl && mdbp) {
			...
			bcopy((char *)bp->b_data + HFS_PRI_OFFSET(hfsmp->hfs_physical_block_size), mdbp, 512);
			brelse(bp);
			bp = NULL;
		}
	}
...
hfs_late_journal_init(struct hfsmount *hfsmp, HFSPlusVolumeHeader *vhp, void *_args)
{
	...
	if ((jib_flags & kJIJournalInFSMask) && (jib_offset / (u_int64_t)vcb->blockSize) != jfork.cf_extents[0].startBlock) {
        printf("hfs: The journal file moved (was: %lu; is: %d).  Fixing up\n",
	...
	if (jib_size != (u_int64_t)jfork.cf_extents[0].blockCount*vcb->blockSize) {
        printf("hfs: The journal file changed size! (was %lu; is %lu).  Fixing up.\n",
...
hfs_metadatazone_init(struct hfsmount *hfsmp, int disable)
{
	...
	if (hfsmp->hfs_flags & HFS_QUOTAS) {
		items = (int) (QF_USERS_PER_GB * (fs_size / GIGABYTE));
		 ...
		 *
		 */
		items = (int) (QF_GROUPS_PER_GB * (fs_size / GIGABYTE));
	...
	hfsmp->hfs_min_alloc_start = (u_int) zonesize / vcb->blockSize;
	/*
	...
	if (hfsmp->hfs_flags & HFS_CS_HOTFILE_PIN) {
		...
		hfsmp->hfs_hotfile_maxblks = (u_int) filesize / vcb->blockSize;
	}

	/* Convert to allocation blocks. */
	blk = (u_int) zonesize / vcb->blockSize;
	...
	if (HFSTOVFS(hfsmp)->mnt_flag & MNT_ROOTFS) {
		hfsmp->hfs_hotfile_start = (u_int) (blk - (filesize / vcb->blockSize));
...
void hfs_syncer_wait(struct hfsmount *hfsmp, struct timespec *ts)
{
    ...
    msleep(&hfsmp->hfs_syncer_thread, &hfsmp->hfs_mutex, PWAIT, "hfs_syncer_wait", ts->tv_nsec);
}
...
{
    uint64_t deadline;
    clock_interval_to_deadline((uint32_t)usecs, /*NSEC_PER_USEC*/ 1000ull, &deadline);
    return deadline;
}
...
void hfs_sync_ejectable(struct hfsmount *hfsmp)
{
    ...
    if (!ISSET(hfsmp->hfs_flags, HFS_RUN_SYNCER)
		|| curthread == hfsmp->hfs_syncer_thread) {
        return;
	}

    hfs_syncer_lock(hfsmp);

    if (hfsmp->hfs_sync_req_oldest == 0)
        hfsmp->hfs_sync_req_oldest = TICKS_2_MSEC(ticks);
    ...
    kthread_add(&hfs_syncer, hfsmp, NULL, &hfsmp->hfs_syncer_thread, 0, 0, "hfs_syncer");
}
...
again:
	...
	if (hfsmp->jnl) {
		if (journal_owner(hfsmp->jnl) != thread) {
			...
			atomic_add_int((volatile uint32_t *)&hfsmp->hfs_active_threads, 1);
			unlock_on_err = 1;
		}
	} else {
		// No journal
...
hfs_end_transaction(struct hfsmount *hfsmp)
{
	...
	hfs_assert(!hfsmp->jnl || journal_owner(hfsmp->jnl) == curthread);
 ...
 *
 */
int hfs_flush(struct hfsmount *hfsmp, hfs_flush_mode_t mode)
{
	int error = 0;
	int options = 0;
	...
	switch (mode) {
		...
		case HFS_FLUSH_FULL:
			...
			if (mode == HFS_FLUSH_JOURNAL_BARRIER && !(hfsmp->hfs_flags & HFS_FEATURE_BARRIER))
				mode = HFS_FLUSH_FULL;
			...
			if (mode == HFS_FLUSH_JOURNAL_BARRIER)
				error = VNOP_IOCTL(hfsmp->hfs_devvp, HFSTOCP(hfsmp), DKIOCSYNCHRONIZE, (caddr_t)&sync_req, 0);
...
check_for_dataless_file(struct vnode *vp, uint64_t op_type)
{
	...
	if (vp == NULL || (VTOC(vp)->c_bsdflags & UF_COMPRESSED) == 0
#if HFS_COMPRESSION
        || VTOCMP(vp) == NULL
#endif
    ) {
...
//
int
hfs_generate_document_id(struct hfsmount *hfsmp, uint32_t *docid, struct thread *td)
{
	struct vnode *rvp;
	struct cnode *cp;
	int error;

	error = hfs_vfs_root(HFSTOVFS(hfsmp), 0, &rvp);
	if (error) {
		return error;
	}
...
int hfs_freeze(struct hfsmount *hfsmp)
{
	...
	while (hfsmp->hfs_freeze_state != HFS_THAWED) {
        if (msleep(&hfsmp->hfs_freeze_state, &hfsmp->hfs_mutex, PWAIT | PCATCH, "hfs freeze 1", 0) == EINTR) {
			hfs_unlock_mount(hfsmp);
			return EINTR;
		}
	}
	...
	while (hfsmp->hfs_syncers) {
        if (msleep(&hfsmp->hfs_freeze_state, &hfsmp->hfs_mutex, PWAIT | PCATCH, "hfs freeze 2", 0) == EINTR) {
			hfs_thaw_locked(hfsmp);
			hfs_unlock_mount(hfsmp);
			return EINTR;				
		}
    ...
    MNT_VNODE_FOREACH_ALL(vp, HFSTOVFS(hfsmp), mvp) {
...
//HFS_SYSCTL(QUAD, _vfs_generic_hfs, OID_AUTO, allocated, CTLFLAG_RD | CTLFLAG_LOCKED, &hfs_allocated, "Memory allocated");

MALLOC_DEFINE(M_HFS, "hfs", "hfs malloc zone");
...
#else
	void *ptr;
	ptr = malloc(size, M_HFS, M_ZERO);
#endif

	atomic_add_long(&hfs_allocated, size);
...
void hfs_free(void *ptr, size_t size)
{
	...
	atomic_add_long(&hfs_allocated, -(int64_t)size);
...
{
	atomic_add_long(&hfs_allocated, hfs_zones[zone].hz_elem_size);
	
	return uma_zalloc(hfs_zones[zone].hz_zone, M_ZERO);
}
