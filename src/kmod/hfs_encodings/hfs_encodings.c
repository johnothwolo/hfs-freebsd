...
hfs_converterinit(void)
{
	...
	encodinglst_lck_grp  = lck_grp_alloc_init("encodinglst", encodinglst_lck_grp_attr);
...
{
	struct hfs_encoding *encp = (struct hfs_encoding *)malloc(sizeof(*encp), M_TEMP, M_ZERO);
...
mac_roman_to_utf8(const Str31 hfs_str, ByteCount maxDstLen, ByteCount *actualDstLen, unsigned char* dstStr)
{
	...
	if (error == 0) {
		...
		if (error == ENAMETOOLONG)
			*actualDstLen = (u_int) utf8_encodelen(uniStr, uniCount * sizeof(UniChar), ':', 0);
		else
			*actualDstLen = (u_int) utf8len;
	}
...
utf8_to_mac_roman(ByteCount srcLen, const unsigned char* srcStr, Str31 dstStr)
{
	...
	if (error == 0)
		error = unicode_to_mac_roman(uniStr, (u_int) (ucslen/sizeof(UniChar)), dstStr);
