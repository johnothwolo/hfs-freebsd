...
#define DTYPE_FSEVENTS  200

#pragma mark -

void fsevents_init(void);
void fsevent_unmount(struct mount *mp, struct thread *td);

void create_fsevent_from_kevent(struct vnode *vp, uint32_t kevents, struct vattr *vap);
...
int   get_fse_info(struct vnode *vp, fse_info *fse, struct thread *td);
int   vnode_get_fse_info_from_vap(struct vnode *vp, fse_info *fse, struct vattr *vap);
...
typedef struct kfs_event {
    ...
    uint16_t       len;        // the length of the path in "str"
...
#define IS_TLD(x) strstr(__DECONST(char *, path), x)
...
//    PE_get_default("kern.maxkfsevents", &max_kfs_events, sizeof(max_kfs_events));
    ...
    event_zone = uma_zcreate("fs-event-buf", sizeof(kfs_event),
                             NULL, NULL, NULL, NULL, 0, UMA_ZONE_NOFREE | UMA_ZONE_ZINIT);
    ...
    uma_zone_set_max(event_zone, max_kfs_events);
...
int
need_fsevent(int type, struct vnode *vp)
{
    if (type >= 0 && type < FSE_MAX_EVENTS && fs_event_type_watchers[type] == 0) {
        return 0;
    }

    // TODO: check is too expensive
    // events in /dev aren't really interesting...
//    if (strncmp(vp->v_mount->mnt_stat.f_mntfromname, "devfs", sizeof("devfs") - 1)) {
//        return 0;
//    }
...
int
add_fsevent(int type, struct thread *td, ...)
{
    ...
    char             *pathbuff = "\0", *pathbuffree = NULL;
    int               pathbuff_len;
    ...
    kfse = uma_zalloc(event_zone, M_NOWAIT);
    if (kfse && (type == FSE_RENAME || type == FSE_EXCHANGE || type == FSE_CLONE)) {
        kfse_dest = uma_zalloc(event_zone, M_NOWAIT);
        if (kfse_dest == NULL) {
            did_alloc = 1;
            uma_zfree(event_zone, kfse);
            kfse = NULL;
        }
    }
    ...
    if (kfse == NULL) {    // yikes! no free events
...

        {
            ...
            if ((current_tv.tv_sec - last_print.tv_sec) > 10) {
                int ii;
                void *junkptr = uma_zalloc(event_zone, M_NOWAIT), *listhead = kfse_list_head.lh_first;
                ...
                if (junkptr) {
                    uma_zfree(event_zone, junkptr);
                }
            }
        }

        if (pathbuffree) {
            free(pathbuff, M_TEMP);
            pathbuffree = NULL;
        }
        return ENOSPC;
    }
    ...
    for (arg_type = va_arg(ap, int32_t); arg_type != FSE_ARG_DONE; arg_type = va_arg(ap, int32_t)) {
        ...
        case FSE_ARG_VNODE: {
            ...
            if ((ret = VOP_GETATTR(vp, &va, td->td_ucred)) != 0) {
            ...
            cur->mode = (int32_t)VTTOIF(vp->v_type) | va.va_mode;
            cur->uid  = va.va_uid;
            cur->gid  = va.va_gid;
            
            if (va.va_nlink > 1) {
                cur->mode |= FSE_MODE_HLINK;
                unsigned long dirlinkcount = va.va_nlink - 2; // va.va_dirlinkcount is nlink without DOT/DOTDOT.
                if ((vp->v_type == VDIR && dirlinkcount == 0) || (vp->v_type == VREG && va.va_nlink == 0)) {
            ...
            if (pathbuffree == NULL) {
                ...
                if ((ret = vn_fullpath(vp, &pathbuff, &pathbuffree)) != 0 || pathbuff[0] == '\0') {
                    ...
                    do {
                        ...
                        if (error == 0 && parentdp != NULL) {
                            ...
                            pathbuff = strdup(vp->v_mount->mnt_stat.f_mntonname, M_TEMP);
                            break;
                        } else {
                            vp = NULL;
                        }
                        ...
                        pathbuff_len = MAXPATHLEN;
                        ret = vn_fullpath(vp, &pathbuff, &pathbuffree);
                    } while (ret == ENOMEM);
...
release_event_ref(kfs_event *kfse)
{
    ...
    old_refcount = atomic_fetchadd_32((volatile u_int*)&kfse->refcount, -1);
    if (old_refcount > 1) {
        return;
    }
    ...
    if (dest_copy.type != FSE_INVALID) {
        ...
        uma_zfree(event_zone, kfse->dest);
    }
...
add_watcher(int8_t *event_list, int32_t num_events, int32_t eventq_size, fs_event_watcher **watcher_out, void *fseh)
{
    ...
    watcher = (fs_event_watcher *)malloc(sizeof(fs_event_watcher) + eventq_size * sizeof(kfs_event *),
                                         M_TEMP, M_WAITOK);
    if (watcher == NULL) {
        return ENOMEM;
    }
    ...
    watcher->fseh         = fseh;
    watcher->pid          = curproc->p_pid;
    
    strncpy(watcher->proc_name, curproc->p_comm, sizeof(curproc->p_comm));
...
static struct proc *event_delivery_timer = NULL;
static int timer_set = 0;


static void
delayed_event_delivery(__unused void *param0)
{
    int i;
...
schedule_event_wakeup(void)
{
    ...
    if (event_delivery_timer == NULL) {
        kproc_create(&delayed_event_delivery, NULL, &event_delivery_timer, 0, 0, "fs_event_delivery_timer");
    }
...
copy_again:
    ...
    if (watcher->flags & WATCHER_WANTS_COMPACT_EVENTS) {
        ...
        finfo_size = sizeof(dev_t) + sizeof(ino_t) + sizeof(int32_t) + sizeof(uid_t) + sizeof(gid_t);
        ...
        error = fill_buff(FSE_ARG_INO, sizeof(ino_t), &cur->ino, evbuff, &evbuff_idx, sizeof(evbuff), uio);
        if (error != 0) {
            goto get_out;
        }
...
//
void
fsevent_unmount(__unused struct mount *mp, __unused struct thread *td)
{
#if CONFIG_EMBEDDED
    ...
    error = add_fsevent(FSE_UNMOUNT_PENDING, td, FSE_ARG_DEV, dev, FSE_ARG_DONE);
...
static int
fseventsf_read(struct file *fp, struct uio *uio,
    __unused int flags, __unused struct thread *td)
{
    fsevent_handle *fseh = (struct fsevent_handle *)fp->f_data;
    int error;
...
static int
fseventsf_write(__unused struct file *fp, __unused struct uio *uio,
    __unused int flags, __unused struct thread *td)
{
    return EIO;
}
...
static int
fseventsf_ioctl(struct file *fp, u_long cmd, caddr_t data, struct thread *td)
{
    fsevent_handle *fseh = (struct fsevent_handle *)fp->f_data;
    int ret = 0;
...
handle_dev_filter:
        {
            ...
            devices_not_to_watch = (dev_t *) malloc(new_num_devices * sizeof(dev_t), M_TEMP, M_WAITOK);
    ...
    case FSEVENTS_UNMOUNT_PENDING_ACK: {
        ...
        if (fsevent_unmount_dev == dev) {
            ...
            printf("unexpected unmount pending ack %lu (%d)\n", dev, fsevent_unmount_dev);
            ret = EINVAL;
        }
        unlock_watch_table();
        break;
...
static int
fseventsf_select(struct file *fp, int which, __unused void *wql, struct thread *td)
{
    fsevent_handle *fseh = (struct fsevent_handle *)fp->f_data;
    int ready = 0;
    ...
    if (!ready) {
        selrecord(td->td_proc, &fseh->si, wql);
    }
...
#if NOTUSED
static int
fseventsf_stat(__unused struct file *fp, __unused struct stat *sb, __unused struct thread *td)
{
    return ENOTSUP;
}
#endif
...
static int
fseventsf_close(struct file *fp, struct thread *td)
{
    fsevent_handle *fseh = (struct fsevent_handle *)fp->f_data;
    fs_event_watcher *watcher;
...
static int
filt_fsevent_touch(struct knote *kn, struct kevent *kev)
{
    int res;
...
static int
filt_fsevent_process(struct knote *kn, struct filt_process_s *data, struct kevent *kev)
{
#pragma unused(data)
    int res;
...
static int
fseventsf_kqfilter(__unused struct file *fp, __unused struct knote *kn,
    __unused struct kevent *kev, __unused struct thread *td)
{
    fsevent_handle *fseh = (struct fsevent_handle *)fp->f_data;
    int res;
...
static int
fseventsf_drain(struct file *fp, __unused struct thread *td)
{
    int counter = 0;
    fsevent_handle *fseh = (struct fsevent_handle *)fp->f_data;
...
static int
parse_buffer_and_add_events(const char *buffer, int bufsize, struct thread *td, long *remainder)
{
    ...
    while ((ptr + sizeof(int) + sizeof(fse_info) + 1) < buffer + bufsize) {
        ...
        if (type != FSE_RENAME && type != FSE_EXCHANGE && type != FSE_CLONE) {
            ...
            err = add_fsevent(type, td, FSE_ARG_STRING, path_len, path, FSE_ARG_FINFO, finfo, FSE_ARG_DONE);
            if (err) {
                break;
            }
            continue;
...
{
    int error = 0, count;
    struct thread *td = vfs_context_current();
    ...
    while (uio_resid(uio)) {
        ...
        error = parse_buffer_and_add_events(write_buffer, offset + count, td, &remainder);
        if (error) {
            break;
        }
...
fsevents_init(void)
{
    ...
    kqueue_add_filteropts(EVFILT_FS, &fsevent_filtops);
...
int
get_fse_info(struct vnode *vp, fse_info *fse, __unused struct thread *td)
{
    struct vattr va;
...
int
vnode_get_fse_info_from_vap(struct vnode *vp, fse_info *fse, struct vattr *vap)
{
    fse->ino  = (ino_t)vap->va_fileid;
...
void
create_fsevent_from_kevent(struct vnode *vp, uint32_t kevents, struct vattr *vap)
{
...
get_pathbuff(void)
{
    char *path = malloc(MAXPATHLEN, M_NAMEI, M_WAITOK);
    return path;
}
...
int
add_fsevent(__unused int type, __unused struct thread *td, ...)
{
    return 0;
}

int
need_fsevent(__unused int type, __unused struct vnode *vp)
{
    return 0;
}
...
// TODO: check this out in FreeBSD source: vfs_notify_upper()
