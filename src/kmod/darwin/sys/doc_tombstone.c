...
extern void  thlocal_put(struct thread *td, void *data);
extern void* thlocal_getdata(struct thread *td);
extern void  thlocal_remove(struct thread *td);
...
{
    struct doc_tombstone *tombstone = thlocal_getdata(curthread);

    if (tombstone == NULL) {
        tombstone = malloc(sizeof(struct doc_tombstone), M_TEMP, M_ZERO);
        thlocal_put(curthread, tombstone);
    }
...
//
void
doc_tombstone_clear(struct doc_tombstone *ut, struct vnode **old_vpp)
{
    uint32_t old_id = (uint32_t)ut->t_lastop_document_id;
    ...
    if (old_vpp) {
        *old_vpp = NULL;
        if (old_id && ut->t_lastop_item
            && (ut->t_lastop_item->v_nchash) == ut->t_lastop_item_vid) {
            int res = vget(ut->t_lastop_item, 0);
            if (!res) {
                // Need to check vid again
                if (ut->t_lastop_item->v_nchash == ut->t_lastop_item_vid
                    && !VN_IS_DOOMED(ut->t_lastop_item)) {
                    *old_vpp = ut->t_lastop_item;
                } else {
                    vput(ut->t_lastop_item);
                }
            }
        }
    }
    ...
    struct componentname *cnp)
{
    ...
    if (ut->t_lastop_document_id && ut->t_lastop_item == vp
        && doc_tombstone_should_ignore_name(cnp->cn_nameptr, (int)cnp->cn_namelen)) {
        return false;
    }
                   ...
                   ino_t file_id)
{
    ...
    ut->t_lastop_parent_vid     = dvp->v_nchash;
    ut->t_lastop_fileid         = file_id;
    ut->t_lastop_item           = vp;
    ut->t_lastop_item_vid       = vp ? vp->v_nchash : 0;
