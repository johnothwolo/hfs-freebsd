...
static UInt32 GetDefaultEncoding(void);
...
#define HOTBAND_MAXIMUM_SIZE  (512*1024*1024)
	...
	finderInfoUUIDPtr->v.high = htobe32(newVolumeUUID.v.high);
	finderInfoUUIDPtr->v.low = htobe32(newVolumeUUID.v.low);
}
...
MarkExtentUsed(const DriveInfo *driveInfo,
	       ...
	       UInt32 blockCount)
{
	size_t bufSize = driveInfo->sectorSize;
	void *buf;
	...
	while (blocksLeft > 0) {
		...
		if (nbytes < (ssize_t)bufSize) {
			if (nbytes == -1)
				err(1, "%s::pread(%d, %p, %zu, %ld)", __FUNCTION__, driveInfo->fd, buf, bufSize, offset);
			goto exit;
		}
...
int keyCompare(const void *l, const void *r) {
	...
	*bmp = ~(0xFF >> temp);
	...
	if (NEWFS_HFS_DEBUG && numOverflowExtents) {
		...
		if (numOverflowExtents > 1) {
            qsort(overflowExtents, numOverflowExtents, sizeof(*overflowExtents), keyCompare);
		}
		 ...
		 void *buffer)
{
    ...
    memset(buffer, 0xdb, driveInfo->sectorSize);
    ...
    WriteBuffer(driveInfo, startingSector, driveInfo->sectorSize, buffer);
...
InitCatalogRoot_HFSPlus(const hfsparams_t *dp, const HFSPlusVolumeHeader *header, void * buffer)
{
    ...
    memcpy(canonicalName, dp->volumeName, sizeof(canonicalName));
	...
	cdp->textEncoding	= SWAP_BE32 (0x7e /* kTextEncodingMacUnicode */);
	...
	const void *buffer)
{
	...
	UInt32 physSectorSize = driveInfo->sectorSize;
	void *tempbuf = NULL;
	int sectorSizeRatio = driveInfo->sectorSize / kBytesPerSector;
...
exit:
	...
	if (1 == status) {
		...
		err(1, "read (sector %lu)", physSector);
	}
	else if (3 == status) {
		err(1, "write (sector %lu)", physSector);
	}
