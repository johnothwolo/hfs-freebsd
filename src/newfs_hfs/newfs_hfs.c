		...
		case 'J':
			...
			if (isdigit(optarg[0])) {
			    ...
			    if (gJournalSize < 512*1024) {
					printf("%s: journal size %ldk too small.  Reset to %dk.\n",
...
#elif __FreeBSD__
		(void) snprintf(rawdevice, sizeof(rawdevice), "%s%s", _PATH_DEV, special);
#endif
...
hfs_newfs(char *device)
{
	...
	if (gPartitionSize) {
		...
		dip.totalSectors = dip.totalSectors = gPartitionSize / kBytesPerSector;
		dip.sectorSize = kBytesPerSector;	/* 512-byte sectors */
		dip.fd = 0;
	} else {
		if (gNoCreate) {
			fso = open( device, O_RDONLY | O_NDELAY | O_DIRECT, 0 );
		} else {
			fso = open( device, O_RDWR | O_NDELAY | O_DIRECT, 0 );
		}
		if (fso == -1) {
			return -1;
		}
		...
		if (fso < 0)
			fatal("%s: %s: %s", __func__, device, strerror(errno));
        ...
        if (ioctl(fso, DIOCGSECTORSIZE, &dip.sectorSize) < 0)
            fatal("%s: %s: %s", __func__, device, strerror(errno));

        if ((dip.sectorSize % kBytesPerSector) != 0)
            fatal("%s: %d is an unsupported sector size\n", __func__, dip.sectorSize);

        if (ioctl(fso, DIOCGMEDIASIZE, &mediaSize) < 0)
            fatal("%s: %s: %s", __func__, device, strerror(errno));
        
        dip.totalSectors = mediaSize / dip.sectorSize;
        
#endif // !__APPLE__
	}

#if __APPLE__
	dip.sectorsPerIO = (1024 * 1024) / dip.physSectorSize;  /* use 1M as default */

	if (fso != -1 && ioctl(fso, DKIOCGETMAXBLOCKCOUNTREAD, &maxPhysPerIO) < 0)
		fatal("%s: %s", device, strerror(errno));

	if (maxPhysPerIO)
		dip.sectorsPerIO = MIN(dip.sectorsPerIO, maxPhysPerIO);
	...
	if (maxPhysPerIO)
		dip.sectorsPerIO = MIN(dip.sectorsPerIO, maxPhysPerIO / dip.physSectorSize);
    ...
    dip.sectorsPerIO = maxPhysPerIO = MAXPHYS / dip.sectorSize;
    dip.sectorsPerIO = MIN(dip.sectorsPerIO, maxPhysPerIO);
...
static void hfsplus_params (const DriveInfo* dip, hfsparams_t *defaults)
{
	...
	if (gJournaled) { 
		...
		if (dip->sectorSize != 0) {
			min_size = dip->sectorSize * (dip->sectorSize / BLOCK_INFO_SIZE);
		}
